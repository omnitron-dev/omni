...
describe('Packet System', () => {
  ...
  describe('Stream Error Packets', () => {
    it('should create stream error packet with factory', () => {
      const packet = createStreamErrorPacket(1, 100, 'Stream processing failed', 'Error: ...\n  at ...');
    ...
    it('should create stream error packet without stack trace', () => {
      const packet = createStreamErrorPacket(2, 200, 'Connection timeout');
    ...
    it('should encode and decode stream error packet', () => {
      const original = createStreamErrorPacket(3, 300, 'Parse error', 'Stack trace here');
      const encoded = encodePacket(original);
      const decoded = decodePacket(encoded);
  ...
  describe('Stream Close Packets', () => {
    it('should create stream close packet with factory', () => {
      const packet = createStreamClosePacket(1, 100, 'Stream completed successfully');
      ...
      expect(packet.data).toEqual({
        streamId: 100,
        reason: 'Stream completed successfully',
      });
    });

    it('should create stream close packet without reason', () => {
      const packet = createStreamClosePacket(2, 200);
    ...
    it('should encode and decode stream close packet', () => {
      const original = createStreamClosePacket(3, 300, 'User requested close');
      const encoded = encodePacket(original);
      const decoded = decodePacket(encoded);
  ...
  describe('Decode Validation', () => {
    it('should reject packets that are too small', () => {
      const tooSmall = new Uint8Array([0x00, 0x00, 0x00]); // Only 3 bytes

      expect(() => decodePacket(tooSmall)).toThrow('Packet too small');
    });

    it('should reject packets with invalid type', () => {
      // Create a packet with invalid type (0x08, outside valid range 0x00-0x07)
      const packet = new Packet(1);
      packet.flags = 0x08; // Invalid type in lower bits
      packet.data = null;

      const encoded = encodePacket(packet);
      // Manually corrupt the flags byte to have invalid type
      encoded[4] = 0x08; // Set type to 0x08 (invalid)

      expect(() => decodePacket(encoded)).toThrow('Invalid packet type');
    });

    it('should reject stream packets with incomplete metadata', () => {
      // Create a stream packet
      const packet = createStreamPacket(1, 100, 0, false, true, 'test data');
      const encoded = encodePacket(packet);

      // Truncate the buffer to remove stream metadata
      const truncated = encoded.slice(0, encoded.length - 7); // Remove most of stream metadata

      expect(() => decodePacket(truncated)).toThrow('Incomplete stream metadata');
    });

    it('should accept valid minimum packet', () => {
