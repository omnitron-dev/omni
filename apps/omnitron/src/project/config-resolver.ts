...
export function resolvedConfigToEnv(
  ...
  if (config['pricingRedis']) {
    const pr = config['pricingRedis'] as { host: string; port: number; db: number; password?: string };
    const password = pr.password ? `:${pr.password}@` : '';
    env['PRICING_REDIS_URL'] = `redis://${password}${pr.host}:${pr.port}/${pr.db}`;
  }
...
export function resolveStack(
  ...
  for (const entry of enabledApps) {
    ...
    if (omnitronCfg) {
      ...
      // Cross-app Redis — pricing price data (read-through from payments/swap)
      if (omnitronCfg.services?.pricing) {
        // Pricing app's Redis DB — look up from allocation or use known offset
        const pricingDb = redisAllocation.get('pricing') ?? (redisDbOffset + 2);
        resolved['pricingRedis'] = {
          host: addresses.redis.host,
          port: addresses.redis.port,
          db: pricingDb,
          password: addresses.redis.password,
