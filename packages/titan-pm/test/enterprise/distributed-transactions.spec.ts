...
import {
  ...
  TransactionPhase,
} from '../../src/enterprise/distributed-transactions.js';
...
describe('Distributed Transactions', () => {
  ...
  describe('Two-Phase Commit', () => {
    it('should successfully commit when all participants vote yes', async () => {
      ...
      vi.spyOn(participant1, 'prepare').mockResolvedValue(true);
      vi.spyOn(participant2, 'prepare').mockResolvedValue(true);

      // Spy on commit methods
      const commit1Spy = vi.spyOn(participant1, 'commit').mockResolvedValue();
      const commit2Spy = vi.spyOn(participant2, 'commit').mockResolvedValue();
    ...
    it('should abort when any participant votes no', async () => {
      ...
      vi.spyOn(participant1, 'prepare').mockResolvedValue(true);
      vi.spyOn(participant2, 'prepare').mockResolvedValue(false);

      // Spy on rollback methods
      const rollback1Spy = vi.spyOn(participant1, 'rollback').mockResolvedValue();
      const rollback2Spy = vi.spyOn(participant2, 'rollback').mockResolvedValue();
    ...
    it('should handle participant timeouts', async () => {
      ...
      vi.spyOn(participant1, 'prepare').mockResolvedValue(true);
      vi.spyOn(participant2, 'prepare').mockImplementation(
  ...
  describe('Transaction Recovery', () => {
    it('should log transactions for recovery', async () => {
      ...
      const participant = new TransactionParticipant('test', mockLogger);
      vi.spyOn(participant, 'prepare').mockResolvedValue(true);
      vi.spyOn(participant, 'commit').mockResolvedValue();
  ...
  describe('Event Emissions', () => {
    ...
    it('should emit abort events on failure', async () => {
      ...
      const participant = new TransactionParticipant('test', mockLogger);
      vi.spyOn(participant, 'prepare').mockResolvedValue(false);
