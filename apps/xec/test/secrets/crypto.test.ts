...
describe('Crypto Module', () => {
  ...
  describe('encrypt/decrypt', () => {
    ...
    it('should encrypt and decrypt with passphrase', async () => {
      const { encrypted, salt, iv, authTag } = await encrypt(testValue, testMachineId, testPassphrase);

      const decrypted = await decrypt(encrypted, salt, iv, authTag, testMachineId, testPassphrase);
    ...
    it('should fail to decrypt with wrong passphrase', async () => {
      ...
      await expect(decrypt(encrypted, salt, iv, authTag, testMachineId, 'wrong-passphrase')).rejects.toThrow();
    });

    it('should fail to decrypt with wrong machine ID', async () => {
      const { encrypted, salt, iv, authTag } = await encrypt(testValue, testMachineId);

      await expect(decrypt(encrypted, salt, iv, authTag, 'wrong-machine-id')).rejects.toThrow();
    });
    ...
    it('should fail to decrypt with tampered data', async () => {
      ...
      await expect(decrypt(encrypted, salt, iv, authTag, testMachineId)).rejects.toThrow();
    });
