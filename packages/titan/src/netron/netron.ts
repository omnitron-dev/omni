...
export class Netron extends EventEmitter implements INetron {
   ...
   * Provides industrial-grade connection management.
   * @public
   */
   ...
   * Optional - only present when authentication is configured via configureAuth().
   * @internal
   */
  public authenticationManager?: IAuthenticationManager;

  /**
   * Authorization manager for method-level access control.
   * Optional - only present when authorization is configured via configureAuth().
   * @internal
   */
  ...
  constructor(logger: ILogger, options: NetronOptions = {}) {
    ...
    const connectionManagerConfig: ConnectionManagerConfig = {
      ...
      poolSize: options.connectionPoolSize,
      idleTimeout: options.idleConnectionTimeout,
      healthCheckInterval: options.healthCheckInterval,
      maxMissedHeartbeats: options.maxMissedHeartbeats,
      heartbeatTimeout: options.heartbeatTimeout,
      reconnect: options.reconnect,
      enableMetrics: options.enableConnectionMetrics ?? true,
      cleanupInterval: options.connectionCleanupInterval,
    };
    this.connectionManager = new ConnectionManager(this.baseLogger, connectionManagerConfig);
  }
  ...
  /**
   * Configure authentication and authorization for this Netron instance.
  ...
  private async startTransportServer(name: string, transport: ITransport, config: TransportConfig): Promise<void> {
    try {
      ...
      server.on('connection', async (connection: ITransportConnection) => {
        ...
        // (WebSocket auth happens at connection time, context stored on connection)
        if ((connection as any).getAuthContext) {
        ...
        // Listen for client ID BEFORE init() to prevent the init message handler
        // from seeing the handshake message and logging "non-binary message" warnings.
        // peer.init() is deferred until after handshake completes.
          ...
          if (!isBinary) {
            try {
              ...
              if (message.type === 'client-id') {
                ...
                // Initialize peer AFTER handshake completes — this registers
                // the binary packet handler without interfering with text handshake
        ...
        connection.on('disconnect', async () => {
          ...
          // Guard: only delete from peers map if THIS peer object is still the active one.
          // When a client reconnects with the same ID, the new peer overwrites the old entry
          // in the map. The old connection's delayed disconnect must NOT delete the new peer.
  ...
  async connect(address: string, reconnect = true): Promise<RemotePeer | HttpRemotePeer> {
    ...
    const jitterFactor = RECONNECT_JITTER_FACTOR;
              ...
              try {
                ...
                if (message.type === 'id') {
                  ...
                  connection.once('disconnect', async () => {
                    ...
                    // Clean up services exposed by this peer
                    await this.cleanupPeerServices(peer);
    ...
    /**
     * Implements the reconnection logic with exponential backoff and jitter.
     ...
     *
     * The jitter helps prevent thundering herd problem when many clients
     * try to reconnect simultaneously after a server restart.
     */
    const attemptReconnect = () => {
      ...
      if (maxAttempts && reconnectAttempts >= maxAttempts) {
        this.logger.error(
          { address, attempts: reconnectAttempts, maxAttempts },
          'Reconnect attempts exceeded. Giving up.'
        );
        return;
      }

      // Use the improved delay calculation with jitter
      const delay = calculateReconnectDelay(reconnectAttempts);
      this.logger.info(
        { address, delay, attempt: reconnectAttempts + 1, maxAttempts: maxAttempts ?? 'unlimited' },
      ...
      setTimeout(async () => {
        ...
        try {
          await connectPeer();
          this.logger.info({ address, attempts: reconnectAttempts }, 'Successfully reconnected');
        } catch (error) {
          this.logger.warn(
            { address, error, attempt: reconnectAttempts, maxAttempts: maxAttempts ?? 'unlimited' },
  ...
  private async connectHttp(address: string, useDirectHttp = true): Promise<HttpRemotePeer> {
    this.logger.info({ address }, `Connecting to HTTP server (v1.0 native mode)`);
    ...
    try {
      ...
      this.emitSpecial(NETRON_EVENT_PEER_CONNECT, getPeerEventName(peer.id), { peerId: peer.id });
      this.logger.info({ peerId: peer.id, address }, 'HTTP peer connected (v1.0 stateless)');
  ...
  async disconnect(peerId: string) {
    ...
    if (peer) {
      // Delete peer from map BEFORE calling disconnect() so the connection handler's
  ...
  /**
   * Clean up services that were exposed by a disconnected peer via exposeRemoteService().
   * Iterates the services map and unexposes any service whose definition is owned by the peer.
   */
  private async cleanupPeerServices(peer: IPeer): Promise<void> {
    const localPeer = this.peer as ILocalPeerInternal;
    const servicesToCleanup: string[] = [];

    for (const [name, stub] of this.services) {
      if (stub.definition?.peerId === peer.id) {
        servicesToCleanup.push(name);
      }
    }
    ...
    this.logger.info(
      { peerId: peer.id, services: servicesToCleanup },
      'Cleaning up services from disconnected peer'
    );

    for (const name of servicesToCleanup) {
      try {
        await localPeer.unexposeRemoteService(peer as RemotePeer, name);
      } catch {
        // Service may already be gone — safe to ignore
      }
    }
  }
  ...
  /**
   * Returns metadata for all exposed services, including name, version, and method names.
   * Public API for service introspection — avoids accessing ServiceStub internals.
   */
  getServiceMetadata(): Array<{ name: string; version?: string; methods: string[] }> {
    const result: Array<{ name: string; version?: string; methods: string[] }> = [];
    for (const [, stub] of this.services) {
      const meta = stub.definition?.meta;
      if (!meta) continue;
      const methods: string[] = meta.methods instanceof Map
        ? Array.from(meta.methods.keys())
        : Object.keys(meta.methods ?? {});
      result.push({ name: meta.name, version: meta.version, methods });
    }
    return result;
  }
