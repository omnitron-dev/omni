/**
 * Example Order Repository with Relationships (Golden Path)
 *
 ...
 * - Uses runInTransaction() for multi-table operations
 * - Demonstrates joins, aggregations, and relationship handling
 * - Complex queries with filter expressions and grouping
 */

import { z } from 'zod';
import { Repository, Timestamps } from '../../src/modules/database/database.decorators.js';
...
export class OrderRepository extends TransactionAwareRepository<Database, 'orders'> {
  /**
  ...
  async createOrder(input: CreateOrderInput): Promise<OrderWithItems> {
    return runInTransaction(this.db, async () => {
      ...
      const products = await this.executor
        .selectFrom('products')
        .where(
          'id',
          'in',
  ...
  async findWithItems(orderId: number): Promise<OrderWithItems | null> {
    ...
    const items = await this.executor.selectFrom('order_items').where('order_id', '=', orderId).selectAll().execute();
  ...
  async getOrderStatistics(startDate: Date, endDate: Date) {
    const results = await this.executor
      ...
      .select([
        (eb) => eb.fn.count('id').as('totalOrders'),
        (eb) => eb.fn.sum('total').as('totalRevenue'),
        (eb) => eb.fn.avg('total').as('averageOrderValue'),
        (eb) => eb.fn.countAll().filterWhere('status', '=', OrderStatus.DELIVERED).as('deliveredOrders'),
        (eb) => eb.fn.countAll().filterWhere('status', '=', OrderStatus.CANCELLED).as('cancelledOrders'),
      ])
      .executeTakeFirst();
  ...
  async getTopCustomers(limit: number = 10) {
    const results = await this.executor
      ...
      .select([
        ...
        (eb) => eb.fn.sum('orders.total').as('totalSpent'),
      ])
  ...
  async getBestSellingProducts(limit: number = 10) {
    const results = await this.executor
      ...
      .select([
        ...
        (eb) => eb.fn.sum('order_items.total').as('totalRevenue'),
        (eb) => eb.fn.count('order_items.id').as('orderCount'),
      ])
