/**
 * LRU (Least Recently Used) Cache - Synchronous Implementation
 *
 * A lightweight synchronous LRU cache with generic key types. Use this when you need:
 * - **Synchronous API** - No async/await required
 * - **Generic key types** - Any hashable key, not just strings
 * - **Simpler interface** - No compression, partitioning, or advanced features
 *
 * For advanced features (compression, WeakRef, wheel timer, partitioning), use
 * the async cache module instead: `@omnitron-dev/titan/module/cache`
 *
 * @example
 * ```typescript
 * // Synchronous API with any key type
 * import { LRUCache } from '@omnitron-dev/titan/utils';
 * const cache = new LRUCache<number, Definition>({ maxSize: 100, ttl: 60000 });
 * cache.set(123, definition);
 * const value = cache.get(123); // Synchronous!
 *
 * // Async API with advanced features
 * import { LRUCache } from '@omnitron-dev/titan/module/cache';
 * const cache = new LRUCache({ maxSize: 100, ttl: 60, compressionThreshold: 1024 });
 * await cache.set('key', value);
 * const value = await cache.get('key');
 * ```
 *
 * @module titan/utils/lru-cache
 */
...
/**
 * Configuration options for synchronous LRUCache
 */
...
/**
 * Synchronous LRU Cache with TTL support
 *
 * A lightweight, synchronous cache with generic key types.
 ...
 *
 * // With numeric keys
 * const peerCache = new LRUCache<number, Peer>({ maxSize: 500 });
  ...
  private deleteWithCallback(key: K, value: V, reason: 'size' | 'ttl' | 'manual'): void {
    ...
    if (this.onEvict) {
      try {
        ...
        this.logger?.error({ err: error as Error, key, reason }, 'LRU cache eviction callback error');
      }
    }
  }
}
 ...
 * Type alias for backward compatibility
 */
export type SimpleLRUCacheOptions<K, V> = LRUCacheOptions<K, V>;
