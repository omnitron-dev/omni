...
import {
  ...
  TenantContext,
  type ITenantContext,
} from '../../src/enterprise/multi-tenancy.js';
import { Process, Public } from '../../src/decorators.js';
...
class MockProcessManager {
  ...
  async spawn(ProcessClass: any, options?: any): Promise<any> {
    ...
    const proxy = {
      ...
      store: instance.store?.bind(instance) || vi.fn().mockResolvedValue(undefined),
      get: instance.get?.bind(instance) || vi.fn().mockResolvedValue(undefined),
      listKeys: instance.listKeys?.bind(instance) || vi.fn().mockResolvedValue([]),
      getAllTenantIds: instance.getAllTenantIds?.bind(instance) || vi.fn().mockResolvedValue([]),
    };
...
type _ProcessManager = MockProcessManager;
