...
export class DevToolsServer {
  ...
  constructor(config?: { port?: number; host?: string; logger?: ILogger } | number) {
  ...
  async start(): Promise<void> {
    ...
    if (typeof global !== 'undefined' && global.process) {
      try {
        ...
        this.logger.info({ host: this.host, port: this.port }, 'Nexus DevTools server listening');
      } catch (error) {
        this.logger.warn({ error }, 'WebSocket server not available');
...
export class DevToolsPlugin {
  ...
  constructor(config?: {
    ...
    this.logger = config?.logger ?? createNullLogger();
    this.server = config?.server || new DevToolsServer({ port: config?.port, logger: this.logger });
    this.enabled = config?.enabled !== false;

    // Only auto-start if explicitly requested
    if (config?.autoStart === true && this.enabled && !config?.server) {
      this.server?.start().catch((err) => this.logger.error({ err }, 'Failed to start DevTools server'));
    }
  }
