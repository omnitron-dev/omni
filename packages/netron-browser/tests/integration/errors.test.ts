...
describe.skip('Error Serialization (WebSocket)', () => {
  ...
  describe('Error Propagation', () => {
    ...
    it('should propagate user not found error', async () => {
      await expect(client.invoke('user@1.0.0', 'getUser', ['nonexistent'])).rejects.toThrow(
  ...
  describe('Multiple Error Scenarios', () => {
    it('should handle different error types from different services', async () => {
      const errorTests = [
        ...
        },
        {
          ...
          method: 'getUser',
          args: ['999'],
          expectedMessage: 'User not found: 999',
        },
        {
          service: 'echo@1.0.0',
          method: 'throwError',
    ...
    it('should handle errors in concurrent requests', async () => {
      ...
      expect((results[1] as any).reason.message).toContain('User not found: 999');
...
describe('TitanError Serialization', () => {
  ...
  describe('Error Code Handling', () => {
    it('should handle all standard error codes', () => {
      // Use actual HTTP status codes to avoid enum alias issues
      // ErrorCode has aliases (e.g., INTERNAL_ERROR and INTERNAL_SERVER_ERROR both = 500)
      // When deserializing, we get the numeric value, not the specific enum member
      const codes = [
