...
import {
  ...
  type ResourceUsage,
  type BudgetAction,
} from '../../src/enterprise/cost-optimization.js';
...
describe('CostOptimizer', () => {
  ...
  describe('Budget Management', () => {
    ...
    it('should execute budget actions at thresholds', () => {
      const budgetActions: BudgetAction[] = [];
      optimizer.on('budget:action', (action: BudgetAction) => {
        budgetActions.push(action);
      });

      // Track high usage to trigger budget action
      // Cost calculation: cpu * 0.01 + (memory / 1024) * 0.005
      // Projected cost = hourly cost * 730
      // For 95% of 1000 budget = 950, need hourly cost > 1.3
      // Using cpu=150, memory=20480: 1.5 + 0.1 = 1.6, projected = 1168 > 950
      optimizer.trackUsage('high-cost-resource', {
        cpu: 150,
        memory: 20480,
        network: 1000,
        storage: 100,
      ...
      // Should trigger multiple budget actions (0.9 and 0.95 thresholds)
      expect(budgetActions.length).toBeGreaterThan(0);
      expect(budgetActions[0].action).toBeDefined();
      expect(budgetActions[0].threshold).toBeDefined();
    });

    it('should throttle resources on budget exceeded', () =>
      new Promise<void>((done) => {
    ...
    it('should stop resources on critical budget exceeded', () =>
      new Promise<void>((done) => {
  ...
  describe('Cost Analysis', () => {
    ...
    it('should detect over-provisioned resources', () => {
      // Over-provisioned detection: cpu < 30 && memory < 50
      // The values represent percentages of capacity being used
      optimizer.trackUsage('over-provisioned', {
        cpu: 20, // 20% CPU usage
        memory: 30, // 30% memory usage (treated as percentage in the condition)
        network: 100,
        storage: 10,
      });
  ...
  describe('Instance Selection', () => {
    ...
    it('should select cheapest instance that meets requirements', () => {
      // Without spot: true, the cheapest instance meeting cpu>=2, memory>=1024
      // is t3.micro-spot (cost: 0.003), not t3.micro (cost: 0.01)
      const instance = optimizer.selectOptimalInstance({
        cpu: 2,
        memory: 1024,
      });
      ...
      expect(instance).toBeDefined();
      expect(instance?.id).toBe('t3.micro-spot'); // Cheapest option meeting requirements
      expect(instance?.cost).toBe(0.003);
    });
  });
  ...
  describe('Resource Optimization', () => {
    ...
    it('should perform predictive scaling during business hours', () => {
      ...
      Date.prototype.getHours = vi.fn().mockReturnValue(13); // 1 PM
  ...
  describe('Idle Shutdown', () => {
    it('should schedule shutdown for idle resources', () =>
      new Promise<void>((done) => {
        vi.useFakeTimers();
    ...
    it('should not shutdown active resources', () =>
      new Promise<void>((done) => {
        vi.useFakeTimers();
  ...
  describe('Resource Management', () => {
    it('should downsize over-provisioned resources', () =>
      new Promise<void>((done) => {
    ...
    it('should convert to spot instances', () =>
      new Promise<void>((done) => {
