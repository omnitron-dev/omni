...
import { safeJsonParseWithFallback } from '@omnitron-dev/titan/utils';
 ...
 */
@Injectable()
export class DiscoveryService implements IDiscoveryService, ILifecycle {
  private redis: Redis;
  ...
  /**
   * Initialize the discovery service (ILifecycle)
   */
  async onInit(): Promise<void> {
    // Initialization is done in constructor
    this.logger.debug('DiscoveryService initialized');
  }
  ...
  /**
   * Destroy the discovery service (ILifecycle)
   */
  ...
  async findNodes(): Promise<NodeInfo[]> {
    ...
    try {
      ...
      // Use Redis pipeline for batch operations - 90% latency reduction
      const pipeline = this.redis.pipeline();
      for (const nodeId of nodeIds) {
        pipeline.hgetall(`${prefix}:nodes:${nodeId}`);
        pipeline.exists(`${prefix}:heartbeat:${nodeId}`);
      }
      ...
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        if (!nodeDataResult || !heartbeatResult) continue;

        const [nodeErr, nodeData] = nodeDataResult as [Error | null, Record<string, string> | null];
        const [hbErr, isActive] = heartbeatResult as [Error | null, number];

        if (nodeErr || hbErr) continue;

        if (nodeData && nodeData['address'] && isActive === 1) {
          nodes.push({
            nodeId: nodeIds[i]!,
            address: nodeData['address'],
            services: safeJsonParseWithFallback<ServiceInfo[]>(nodeData['services'] || '[]', []),
  ...
  async getAllNodes(): Promise<NodeInfo[]> {
    ...
    try {
      ...
      for (let i = 0; i < nodeIds.length; i++) {
        ...
        const [err, nodeData] = result as [Error | null, Record<string, string> | null];
        if (err) continue;
