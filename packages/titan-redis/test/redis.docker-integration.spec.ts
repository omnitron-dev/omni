    ...
    it(
      ...
      async () => {
        await withDockerRedisCluster(async (fixture) => {
          // Use hash tags to ensure keys map to the same slot in cluster
          const pipeline = fixture.client.pipeline();
          pipeline.set('{test}key1', 'value1');
          pipeline.set('{test}key2', 'value2');
          pipeline.get('{test}key1');
          pipeline.get('{test}key2');
...
describeOrSkip('Integration: Real-world Scenarios', () => {
