...
import type { IMigration, IDatabaseManager } from '../src/index.js';
...
describe('Migration System', () => {
  describe('SQLite (in-memory)', () => {
    ...
    beforeEach(async () => {
      ...
      // Use a unique database name for each test to avoid lock conflicts
      const uniqueDbName = `test_${Date.now()}_${Math.random().toString(36).substring(7)}`;
        ...
        imports: [
          TitanDatabaseModule.forRoot({
            connection: {
              dialect: 'sqlite',
              connection: `:memory:`,
              // Each test gets isolated in-memory database by using filename mode
              // SQLite creates separate databases when using different filenames
            },
            migrations: {
              tableName: `migrations_${uniqueDbName}`,
              lockTableName: `lock_${uniqueDbName}`,
  ...
  describeOrSkip('PostgreSQL (Docker)', () => {
    ...
    beforeEach(async () => {
      ...
      // Use unique table names for each test to avoid lock conflicts
      const uniqueDbName = `test_${Date.now()}_${Math.random().toString(36).substring(7)}`;
        ...
        imports: [
          TitanDatabaseModule.forRoot({
            ...
            migrations: {
              ...
              lockTableName: `lock_${uniqueDbName}`,
              transactional: true,
              lockTimeout: 60000, // 60 seconds for tests (to handle concurrent attempts)
            },
            autoMigrate: false,
            isGlobal: true,
          }),
