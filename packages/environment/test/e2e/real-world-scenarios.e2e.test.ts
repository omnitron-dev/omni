...
describe('E2E: Real-World Scenarios', () => {
  ...
  it('should simulate CI/CD pipeline workflow', async () => {
    ...
    expect(executionOrder.indexOf('unit-test')).toBeLessThan(executionOrder.indexOf('integration-test'));
    expect(executionOrder.indexOf('build')).toBeGreaterThan(executionOrder.indexOf('lint'));
    expect(executionOrder.indexOf('build')).toBeGreaterThan(executionOrder.indexOf('integration-test'));
  ...
  it('should handle development workflow lifecycle', async () => {
    ...
    const startTasks = ['start-db', 'start-redis', 'start-rabbitmq', 'migrate-db', 'seed-db', 'start-app'];
