...
describe('containerRemedy', () => {
  ...
  it('says recreate, not restart, for a stale network endpoint', () => {
    const remedy = containerRemedy(
      'acme-dev-monero-daemon',
      'failed to set up container networking: endpoint with name acme-dev-monero-daemon already exists in network acme-dev_default',
      false
    );

    expect(remedy).toMatch(/docker rm -f acme-dev-monero-daemon/);
  });
