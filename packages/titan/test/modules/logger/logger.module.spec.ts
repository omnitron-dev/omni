...
describe('LoggerModule', () => {
  ...
  describe('forRoot() Static Method', () => {
    ...
    it('should configure with custom log level', () => {
      ...
      const optionsProvider = result.providers.find((p: any) => Array.isArray(p) && p[0] === LOGGER_OPTIONS_TOKEN);
    ...
    it('should configure with transports', () => {
      ...
      const transportProvider = result.providers.find((p: any) => Array.isArray(p) && p[0] === LOGGER_TRANSPORTS_TOKEN);
      expect(transportProvider).toBeDefined();
    });

    it('should configure with processors', () => {
      const mockProcessor: ILogProcessor = {
        process: vi.fn((log) => log),
      };
      ...
      const processorProvider = result.providers.find((p: any) => Array.isArray(p) && p[0] === LOGGER_PROCESSORS_TOKEN);
    ...
    it('should configure with all options', () => {
      const options: ILoggerModuleOptions = {
        level: 'warn',
        prettyPrint: false,
        transports: [{ name: 'test', write: vi.fn() }],
    ...
    it('should include LoggerService factory in providers', () => {
      ...
      const serviceProvider = result.providers.find((p: any) => Array.isArray(p) && p[0] === LOGGER_SERVICE_TOKEN);
  ...
  describe('ConsoleTransport', () => {
    ...
    beforeEach(() => {
      ...
      transport = new ConsoleTransport(mockLogger);
    });
    ...
    it('should write log entry using logger', () => {
      const logEntry = {
        level: 30,
        time: Date.now(),
        msg: 'Test message',
      ...
      expect(mockLogger.info).toHaveBeenCalledWith({ log: logEntry }, 'Console transport log');
    });
    ...
    it('should handle empty log object', () => {
      transport.write({});
      expect(mockLogger.info).toHaveBeenCalledWith({ log: {} }, 'Console transport log');
    });
