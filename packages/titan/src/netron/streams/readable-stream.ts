...
/**
 * Maximum number of concurrent readable streams per peer.
...
export class NetronReadableStream extends Readable {
  ...
  constructor({ peer, streamId, isLive = false, ...opts }: NetronReadableStreamOptions) {
    ...
    if (this.peer.readableStreams.size >= MAX_STREAMS_PER_PEER) {
      const errorMessage = `Maximum readable streams (${MAX_STREAMS_PER_PEER}) reached for peer`;
      this.peer.logger.error({ streamId: this.id, count: this.peer.readableStreams.size }, errorMessage);
      throw NetronErrors.streamBackpressure(String(this.id), this.peer.readableStreams.size);
    }
   ...
   *
   * @param {IStreamPeer} peer - The remote peer for this stream
   ...
   */
  public static create(peer: IStreamPeer, streamId: number, isLive: boolean = false): NetronReadableStream {
