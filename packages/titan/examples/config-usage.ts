...
function advancedValidation() {
  ...
  const CustomConfigSchema = z.object({
    port: z
      ...
      .int()
      .positive()
      .refine((port) => port >= 3000 && port <= 9999, { message: 'Port must be between 3000 and 9999' }),

    email: z
      .string()
      .email()
      .refine((email) => email.endsWith('@company.com'), { message: 'Email must be a company email address' }),
    ...
    timeRange: z
      ...
      .refine((data) => new Date(data.end) > new Date(data.start), { message: 'End time must be after start time' }),
  });
...
export { AppConfigSchema, type AppConfig, createConfigModule, createTypedConfig };
