...
describe('SmartBuffer', () => {
  describe('base', () => {
    it('allocate', () => {
      ...
      for (const i in bb) {
        if (
          Object.prototype.hasOwnProperty.call(bb, i) &&
          !['roffset', 'woffset', 'markedOffset', 'limit', 'noAssert', 'buffer', 'view', 'ateos_tag'].includes(i)
        ) {
  ...
  describe('types', () => {
    const types: [string, number, any, any, string][] = [
      ...
      [
        'UInt64',
        8,
        new Long(0xfffffffe, 0xffffffff, false),
        new Long(0xfffffffe, 0xffffffff, true),
        'fffffffffffffffe',
      ],
      ['UBigInt', 8, 18446744073709551614n, 18446744073709551614n, 'fffffffffffffffe'],
      ['BigInt', 8, -9223372036854775808n, -9223372036854775808n, '8000000000000000'],
      ['BigInt', 8, 9223372036854775807n, 9223372036854775807n, '7fffffffffffffff'],
    ...
    it('bitset', () => {
      ...
      bb.reset(true);
      bb.writeBitSet([undefined, null, '', 0, 42, 'hello world', new Date(0), {}, []]);
    ...
    it('zigZagVarint', () => {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        expect((dec as SNumber).value).toEqual(values[i]);
        expect(encLen).toEqual((dec as SNumber).length);
        bb.reset(true);
      }
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        expect(values[i].toString()).toEqual((dec as SLong).value.toString());
        expect(encLen).toEqual((dec as SLong).length);
      }
    });
