 ...
 *
 * omnitron install http.tar.gz
 * omnitron uninstall http
 *
 * - the first and only folder in the tarball must be called module (tar zcvf http module/)
 * - a package.json must be present with attribute "name", "version" and "omnitron" to declare apps to run
 */

function install(OMNITRON, module_filepath, opts, cb) {
  ...
  if (module_filepath.includes('http') === true) {
    ...
    return retrieveRemote(module_filepath, target_filepath, (err) => {
      ...
      installLocal(OMNITRON, target_filepath, opts, cb);
    });
  }

  // Local install
  installLocal(OMNITRON, module_filepath, opts, cb);
}
...
function installLocal(OMNITRON, module_filepath, opts, cb) {
  ...
  getModuleName(module_filepath, function (err, module_name) {
    ...
    install_instance.on('close', function (code) {
      ...
      if (code == 0) return runInstall(OMNITRON, install_path, module_name, opts, cb);
      return OMNITRON.exitCli(1);
    });
...
function runInstall(OMNITRON, target_path, module_name, opts, cb) {
  ...
  if (opts.install) {
    ...
    sexec(`cd ${target_path} ; yarn install`, { silent: false }, function (code) {
      // Start apps under "apps" or "omnitron" attribute
      Common.logMod(`Starting ${target_path}`);
      OMNITRON.start(conf, opts, function (err, data) {
        if (err) return cb(err);
...
function start(OMNITRON, module_name, cb) {
 ...
 */
function uninstall(OMNITRON, module_name, cb) {
  var module_path = path.join(cst.DEFAULT_MODULE_PATH, module_name);
  ...
  var apps = pkg.apps || pkg.omnitron;
  apps = [].concat(apps);
  ...
  forEachLimit(
    ...
    (app, next) => {
      ...
      else if (apps.length == 1 && pkg.name != apps[0].name) app_name = `${module_name}:${app.name}`;
      else app_name = app.name;

      OMNITRON._operate('deleteProcessId', app_name, () => {
...
function publish(OMNITRON, folder, cb) {
  ...
  if (!pkg.omnitron && !pkg.apps) throw new Error('Attribute apps should be present');
  ...
  packager(current_path, target_path, (err, res) => {
    ...
    var uri = `${OMNITRON.omnitron_configuration.registry}/api/v1/modules`;
    Common.logMod(`Sending Package to remote ${pkg.name} ${uri}`);

    require('needle').post(uri, data, { multipart: true }, function (err, res, body) {
      if (err) {
        Common.errMod(err);
        process.exit(1);
      }
...
function needPrefix(conf) {
  if (
    ...
    (conf.omnitron && conf.omnitron.length > 1) ||
