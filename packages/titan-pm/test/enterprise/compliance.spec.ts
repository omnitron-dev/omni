...
} from '../../src/enterprise/compliance.js';
...
describe('ComplianceManager', () => {
  ...
  describe('Edge Cases', () => {
    it('should handle unknown request types', async () => {
      ...
      await expect(complianceManager.processDataSubjectRequest(request)).rejects.toThrow();
    });
  ...
  describe('Integration Tests', () => {
    it('should maintain audit trail for data subject requests', async () => {
      ...
      const _auditConfig: AuditConfig = {
        level: 'full',
        retention: '7y',
        encryption: false,
        immutable: false,
    ...
    it('should handle multiple concurrent data subject requests', async () => {
      ...
      const responses = await Promise.all(requests.map((req) => complianceManager.processDataSubjectRequest(req)));
