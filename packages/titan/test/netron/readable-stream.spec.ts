...
import { NetronReadableStream } from '../../src/netron/streams/readable-stream.js';
import { createStreamPacket } from '../../src/netron/packet/index.js';
...
describe('NetronReadableStream', () => {
  ...
  describe('Stream Creation and Initialization', () => {
    ...
    it('should log stream creation', () => {
      ...
      expect(peer.logger.info).toHaveBeenCalledWith({ streamId: 5, isLive: false }, 'Creating readable stream');
    });
  ...
  describe('Packet Processing and Ordering', () => {
    it('should handle ordered packets correctly', () =>
    ...
    it('should handle severely out-of-order packets', () =>
      new Promise<void>((done) => {
    ...
    it('should ignore packets for closed stream', () => {
      ...
      expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 13 }, 'Received packet for closed stream');
    });

    it('should handle packets with null data for last chunk', () =>
      new Promise<void>((done) => {
    ...
    it('should handle various data types', () =>
      new Promise<void>((done) => {
        ...
        stream.on('data', (chunk) => receivedChunks.push(chunk));
        stream.on('end', () => {
          expect(receivedChunks).toEqual(['string', 123, { nested: { obj: true } }, [1, 2, 3], Buffer.from('binary')]);
          done();
        });
  ...
  describe('Buffer Management', () => {
    ...
    it('should detect buffer overflow at MAX_BUFFER_SIZE (10000)', () =>
      new Promise<void>((done) => {
    ...
    it('should log buffer overflow error', () =>
      new Promise<void>((done) => {
    ...
    it('should process buffered packets when gap is filled', () =>
      new Promise<void>((done) => {
  ...
  describe('Backpressure Handling', () => {
    it('should log backpressure when push returns false', () => {
      ...
      expect(peer.logger.debug).toHaveBeenCalledWith({ streamId: 30 }, 'Stream backpressure detected');
    });

    it('should resume delivery when drained', () =>
      new Promise<void>((done) => {
  ...
  describe('Timeout Handling', () => {
    it('should timeout inactive non-live streams', () =>
      new Promise<void>((done) => {
  ...
  describe('Stream Closing', () => {
    ...
    it('should not close live stream without force flag', () => {
      ...
      expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 51 }, 'Attempt to close live stream');
      expect(stream.destroyed).toBe(false);
    });
    ...
    it('should handle multiple closeStream calls on already closed stream', () =>
      new Promise<void>((done) => {
        ...
        stream.on('close', () => {
          ...
          expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 53 }, 'Attempt to close already closed stream');
          done();
        });
      }));

    it('should force close with reason', () =>
      new Promise<void>((done) => {
    ...
    it('should handle forceClose on already closed stream', () => {
      ...
      expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 55 }, 'Attempt to force close already closed stream');
    });
  });

  describe('Error Handling', () => {
    it('should handle destroy with error', () =>
      new Promise<void>((done) => {
    ...
    it('should log error on error event', () =>
      new Promise<void>((done) => {
        ...
        stream.on('error', () => {});
        stream.on('close', () => {
          expect(peer.logger.error).toHaveBeenCalledWith({ streamId: 61, error: testError }, 'Stream error occurred');
          done();
        });
    ...
    it('should handle multiple destroy calls', () => {
      ...
      expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 63 }, 'Attempt to destroy already closed stream');
    });
  });
  ...
  describe('Cleanup', () => {
    ...
    it('should clear timeout on cleanup', () =>
      new Promise<void>((done) => {
        ...
        stream.on('close', () => {
          expect(peer.logger.debug).toHaveBeenCalledWith({ streamId: 71 }, 'Cleaning up stream resources');
          done();
        });
  ...
  describe('Edge Cases', () => {
    it('should handle empty stream (only end packet)', () =>
      new Promise<void>((done) => {
