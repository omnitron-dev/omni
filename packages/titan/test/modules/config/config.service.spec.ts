...
describe('ConfigService', () => {
  ...
  afterEach(async () => {
    if (configService) await configService.onDestroy();
  });
  ...
  describe('initialize', () => {
    it('should load configuration from sources', async () => {
      ...
      configService = new TestConfigService({ sources: [{ type: 'file', path: 'config.json' }] }, loader, validator);
    ...
    it('should throw on validation failure', async () => {
      ...
      await expect(configService.onInit()).rejects.toThrow();
    });
    ...
    it('should only initialize once', async () => {
      ...
      loader.load = async (sources) => {
        loadCount++;
        return originalLoad(sources);
      };
