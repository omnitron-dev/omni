...
export class CommandRegistry {
  ...
  findSimilarCommands(input: string, maxSuggestions = 3): CommandSuggestion[] {
    const allCommands = [...this.getAllCommands(), ...Array.from(this.aliases.keys())];
   ...
   */
  formatSuggestions(input: string, suggestions: CommandSuggestion[], options: { color?: boolean } = {}): string {
 ...
 */
export function checkForCommandTypo(input: string, registry: CommandRegistry = defaultCommandRegistry): string | null {
 ...
 */
export function getCommandCompletions(partial: string, registry: CommandRegistry = defaultCommandRegistry): string[] {
  const allCommands: string[] = [
    ...registry.getAllCommands(),
    // Note: aliases are internal, we only return main commands
  ];

  return allCommands.filter((cmd: string) => cmd.toLowerCase().startsWith(partial.toLowerCase())).sort();
}
