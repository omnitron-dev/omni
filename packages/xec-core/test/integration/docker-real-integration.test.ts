...
describeIfDocker('Docker Adapter Real Integration Tests', () => {
  ...
  describe('Working Directory and Environment', () => {
    ...
    it('should handle environment variables', async () => {
      ...
      try {
        const $docker = $.docker({ container: containerName }).env({ NEW_VAR: 'new_value', ANOTHER_VAR: 'another' });
  ...
  describe('Multi-Container Scenarios', () => {
    it('should handle multiple containers', async () => {
      ...
      try {
        ...
        expect(pingResult.stdout).toMatch(
          /1 packets transmitted.*1 received|1 packets transmitted.*1 packets received/
        );
      } finally {
