// RocksDB storage layer for progress tracking
...
/// Storage operations for progress tracking
...
mod tests {
    ...
    async fn create_test_storage() -> (ProgressStorage, TempDir) {
        let temp_dir = TempDir::new().unwrap();
        let db = RocksDBStorage::new(temp_dir.path()).unwrap();
        let storage = ProgressStorage::new(Arc::new(db));
        (storage, temp_dir)
    }
