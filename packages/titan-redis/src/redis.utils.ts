...
import { InternalRedisClient, InternalRedisClientOptions, isClientReady } from './redis.types.js';

export function isCluster(client: InternalRedisClient): client is Cluster {
  return client instanceof Cluster;
}

/**
 * Extract the namespace from Redis client options.
 * Works with both IRedisClientOptions (public) and InternalRedisClientOptions.
 * @param options - Options object containing an optional namespace property
 * @returns The namespace string or 'default' if not specified
 */
export function getClientNamespace(options: { namespace?: string } | undefined | null): string {
...
export function mergeOptions(
  ...
  clientOptions?: InternalRedisClientOptions
...
export async function waitForConnection(client: InternalRedisClient, timeout = 5000): Promise<boolean> {
