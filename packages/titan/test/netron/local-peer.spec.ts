...
describe('LocalPeer', () => {
  ...
  it('should throw if expose service with same name', async () => {
    ...
    await expect(async () => peer.exposeService(ctx2)).rejects.toThrow(/Service already exposed/);
  });
  ...
  it('should throw if unexpose service two times', async () => {
    ...
    await peer.exposeService(ctx1);
    await peer.unexposeService('service1');
    await expect(async () => peer.unexposeService('service1')).rejects.toThrow(/Service.*not found/);
  });

  it('should throw if unexpose service with unknown name', async () => {
    await expect(async () => await netron.peer.unexposeService('unknown')).rejects.toThrow(/Service.*not found/);
  });
  ...
  it('should throw if exposed instance is not service', async () => {
    ...
    const peer = netron.peer;
    await expect(async () => await peer.exposeService(new TestNonService())).rejects.toThrow(/Invalid service/);
    await expect(async () => await peer.exposeService({})).rejects.toThrow(/Invalid service/);
  });
  ...
  it('should call methods and access properties', async () => {
    // Mock fetch to avoid real HTTP calls to external services
    ...
    expect(await iface.fetchData('https://jsonplaceholder.typicode.com/todos/1')).toEqual(mockResponse);
    ...
    expect(await iface.fetchDataWithDelay('https://jsonplaceholder.typicode.com/todos/1', 500)).toEqual(mockResponse);
  ...
  it('when unexpose service all child services should be released', async () => {
    ...
    const iService3 = await peer.queryInterface<IService3>('service3');
    const iService2 = await iService3.getService2();
    const _iService1 = await iService2.getService1();
