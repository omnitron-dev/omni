  ...
  describe('Fine-grained Reactivity (Spec Section 2.1)', () => {
    it('should track dependencies automatically without explicit subscription', () => {
      ...
      createRoot((d) => {
        ...
        firstName.set('Jane');
        expect(updates).toEqual(['greeting-computed', 'fullName-computed', 'effect: Hello, Jane Doe!']);

        updates.length = 0;

        // Changing lastName should also trigger the chain
        lastName.set('Smith');
        expect(updates).toEqual(['greeting-computed', 'fullName-computed', 'effect: Hello, Jane Smith!']);
      });
    });
    ...
    it('should group updates for optimization', () => {
      ...
      createRoot((d) => {
        ...
        const filtered = computed(() => {
          ...
          let result = allItems.filter((item) => item.toLowerCase().includes(searchTerm.toLowerCase()));
