...
function displayMonitoringResult(result: any, verbose: boolean, consecutiveFailures: number): void {
  ...
  const statusColor = result.status === 'healthy' ? prism.green :
                      result.status === 'degraded' ? prism.yellow : prism.red

  console.log(`Status: ${statusIcon} ${statusColor(result.status.toUpperCase())}`)

  // Show latency
  if (result.metrics?.checkLatency) {
    const latencyColor = result.metrics.checkLatency < 10 ? prism.green :
                        result.metrics.checkLatency < 50 ? prism.yellow :
                        prism.red
  ...
  // Connection status
  if (result.checks) {
    const connCheck = result.checks.find((c: any) =>
