...
describe('Netron Browser Features - Standalone', () => {
  ...
  describe('Core Tasks Type Safety', () => {
    it('should create and validate query interface requests', () => {
      ...
      const latestRequest = createQueryInterfaceRequest('UserService');
      expect(latestRequest.serviceName).toBe('UserService');

      const versionedRequest = createQueryInterfaceRequest('DataService@2.1.0');
      expect(versionedRequest.serviceName).toBe('DataService@2.1.0');
    });
    ...
    it('should create and validate authenticate requests', () => {
      ...
      expect(request.credentials).toBeDefined();
      expect(request.credentials.username).toBe('testuser');
      expect(request.credentials.password).toBe('testpass');
    });
