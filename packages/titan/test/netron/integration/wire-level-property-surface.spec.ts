 ...
 *
 * Measured on a running downstream stand (messaging, Netron WS on 3006), where the
...
async function boot(): Promise<{ server: Netron; port: number }> {
  ...
  // which is the posture the downstream backends actually run.
...
describe('the condition NET-14b cited', () => {
  it('leaves meta.properties empty while methods are still extracted', async () => {
    ...
    // this map. Measured on the real downstream backends, which show exactly this
