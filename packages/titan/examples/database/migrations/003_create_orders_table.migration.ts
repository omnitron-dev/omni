...
export class CreateOrdersSystemMigration implements IMigration {
  async up(db: Kysely<any>): Promise<void> {
    ...
    await db.schema
      .createIndex('idx_orders_customer')
      .on('orders')
      .column('customer_id')
      .execute();
    ...
    await db.schema
      .createIndex('idx_orders_payment_status')
      .on('orders')
    ...
    await db.schema
      .createIndex('idx_order_items_order')
      .on('order_items')
      .column('order_id')
      .execute();

    await db.schema
      .createIndex('idx_order_items_product')
      .on('order_items')
      .column('product_id')
      .execute();

    await db.schema
      .createIndex('idx_payments_order')
      .on('payments')
      .column('order_id')
      .execute();
