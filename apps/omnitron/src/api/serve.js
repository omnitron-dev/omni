...
var options = {
  port: process.env.OMNITRON_SERVE_PORT || process.argv[3] || 8080,
  host: process.env.OMNITRON_SERVE_HOST || process.argv[4] || '0.0.0.0',
  path: path.resolve(process.env.OMNITRON_SERVE_PATH || process.argv[2] || '.'),
  spa: process.env.OMNITRON_SERVE_SPA === 'true',
  homepage: process.env.OMNITRON_SERVE_HOMEPAGE || '/index.html',
  basic_auth:
    process.env.OMNITRON_SERVE_BASIC_AUTH === 'true'
      ? {
          username: process.env.OMNITRON_SERVE_BASIC_AUTH_USERNAME,
          password: process.env.OMNITRON_SERVE_BASIC_AUTH_PASSWORD,
        }
      : null,
  monitor: process.env.OMNITRON_SERVE_MONITOR,
};
...
if (typeof options.monitor === 'string' && options.monitor !== '') {
  try {
    let fileContent = fs.readFileSync(path.join(process.env.OMNITRON_HOME, 'agent.json5')).toString();
...
}(window,document,'https://apm.omnitron.io/omnitron-io-apm-browser.v1.js','omnitronReady','script'))

omnitronReady(function(apm) {
