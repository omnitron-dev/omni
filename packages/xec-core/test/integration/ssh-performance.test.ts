...
describeSSH(
  'SSH Performance and Stress Tests',
  () => {
    describe('Connection Performance', () => {
      testEachPackageManager('should establish connections quickly', async (container) => {
        ...
        console.log(
          `Connection times for ${container.name} - Avg: ${avgTime.toFixed(2)}ms, Min: ${minTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms`
        );
    ...
    describe('Command Execution Performance', () => {
      testEachPackageManager('should handle rapid command execution', async (container) => {
        ...
        console.log(
          `${container.name}: Executed ${commandCount} commands in ${duration.toFixed(2)}ms (${commandsPerSecond.toFixed(2)} commands/sec)`
        );
    ...
    describe('Data Transfer Performance', () => {
      testEachPackageManager('should handle large stdout efficiently', async (container) => {
        ...
        for (const size of sizes) {
          ...
          console.log(
            `${container.name}: Transfer ${size} bytes: ${duration.toFixed(2)}ms (${throughputMBps.toFixed(2)} MB/s)`
          );
        }
      ...
      testEachPackageManager('should handle many small outputs efficiently', async (container) => {
        ...
        console.log(
          `${container.name}: Processed ${lineCount} lines in ${duration.toFixed(2)}ms (${linesPerSecond.toFixed(0)} lines/sec)`
        );
    ...
    describe('Concurrent Connection Stress Test', () => {
      it(
        'should handle multiple concurrent SSH connections',
        async () => {
          ...
          try {
            ...
            console.log(
              `Created and used ${connectionCount} connections in ${duration.toFixed(2)}ms (${successes} succeeded, ${failures} failed)`
            );
        ...
        try {
          ...
          for (let batch = 0; batch < commandCount / batchSize; batch++) {
            const promises = Array(batchSize)
              .fill(null)
              .map((_, i) => $ssh`echo "Batch ${batch}, Command ${i}"`);
      ...
      it('should compare performance across different containers', async () => {
        ...
        for (const container of containers) {
          ...
          try {
            ...
            console.log(
              `${container.name}: Failed to connect - ${error instanceof Error ? error.message : String(error)}`
            );
          } finally {
            // No disconnect method on $ proxy
          }
