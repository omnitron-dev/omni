...
describe('SSHAdapter - Mocked Integration Tests', () => {
  ...
  describe('SFTP Operations', () => {
    it('should reject SFTP operations without SSH options', async () => {
      ...
      await expect(adapter.uploadFile('/local/file.txt', '/remote/file.txt', {} as any)).rejects.toThrow();
    });
    ...
    it('should reject SFTP operations with invalid paths', async () => {
      ...
      // Empty local path
      await expect(adapter.uploadFile('', '/remote/file.txt', sshOptions)).rejects.toThrow();

      // Empty remote path
      await expect(adapter.uploadFile('/local/file.txt', '', sshOptions)).rejects.toThrow();
    });
    ...
    it('should handle download operations', async () => {
      ...
      await expect(
        adapter.downloadFile('/remote/file.txt', '/local/file.txt', {
          type: 'ssh',
          host: 'non-existent-host',
          username: 'test',
          password: 'test',
    ...
    it('should handle directory upload operations', async () => {
      ...
      await expect(
        adapter.uploadDirectory('/local/dir', '/remote/dir', {
          type: 'ssh',
          host: 'non-existent-host',
          username: 'test',
          password: 'test',
