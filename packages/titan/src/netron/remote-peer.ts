...
/** Regex pattern for validating UUID format (used for definition IDs) */
const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * Regex pattern for validating property/method names.
 * Matches valid JavaScript identifier names:
 * - Must start with a letter, underscore, or dollar sign
 * - Can contain letters, digits, underscores, or dollar signs
 * - Prevents injection attacks by rejecting special characters
 *
 * Note: \w includes [a-zA-Z0-9_], \d is redundant but kept for clarity
 */
const PROPERTY_NAME_REGEX = /^[\w$][\w\d_$]*$/;

/**
 * Regex pattern for validating service name format (e.g., "serviceName@1.0.0" or "serviceName").
 * - Allows alphanumeric characters, hyphens, underscores in service name
 * - Optional version part after @
 */
const SERVICE_NAME_REGEX = /^[\w-]+(@[\w.-]+)?$/;

/**
 * Validates and resolves a definition ID or service name to a UUID.
 * Supports both UUID format (legacy) and service names (e.g., "calculator@1.0.0").
 * @param defIdOrServiceName - The definition ID (UUID) or service name to resolve
 * @param netron - The Netron instance for service lookups
 * @returns The resolved UUID definition ID
 * @throws Error if the input is invalid or service not found
 */
const resolveDefId = (defIdOrServiceName: unknown, netron: INetron): string => {
  if (typeof defIdOrServiceName !== 'string') {
    throw Errors.badRequest('Definition ID must be a string');
  }

  // If it's already a UUID, validate and return it
  if (UUID_REGEX.test(defIdOrServiceName)) {
    return defIdOrServiceName;
  }

  // If it looks like a service name, resolve it
  if (SERVICE_NAME_REGEX.test(defIdOrServiceName)) {
    const stub = netron.services.get(defIdOrServiceName) as ServiceStub | undefined;
    if (!stub || !stub.definition) {
      throw Errors.notFound('Service', defIdOrServiceName);
    }
    return stub.definition.id;
  }

  // Invalid format
  throw Errors.badRequest('Invalid definition ID format');
};

/**
 * Validates that a property/method name is safe for use in RPC calls.
 * This function is called in the hot path for every GET, SET, and CALL packet.
 *
 * Validation rules:
 * - Must be a non-empty string
 * - Maximum length of 255 characters (prevents DoS via long names)
 * - Must match valid JavaScript identifier pattern (letters, digits, _, $)
 * - First character must be a letter, underscore, or dollar sign
 *
 * @param name - The property or method name to validate
 * @returns true if the name is valid, false otherwise
 */
const isValidPropertyName = (name: unknown): name is string =>
  typeof name === 'string' && name.length > 0 && name.length < 256 && PROPERTY_NAME_REGEX.test(name);
...
export class RemotePeer extends AbstractPeer {
  ...
  /**
   * Maximum number of concurrent streams per direction.
   * Prevents memory exhaustion from malicious or runaway stream creation.
   * Adjust based on expected workload (default: 1000 streams per direction).
   */
  private static readonly MAX_STREAMS_PER_DIRECTION = 1000;

  /**
   * Maximum number of service definitions to cache.
   * Services are typically limited in number, but this prevents unbounded growth
   * from dynamic service registration.
   */
  private static readonly MAX_SERVICE_DEFINITIONS = 500;

  /** Map of writable streams indexed by stream ID (bounded by MAX_STREAMS_PER_DIRECTION) */
  public writableStreams = new Map<number, NetronWritableStream>();

  /** Map of readable streams indexed by stream ID (bounded by MAX_STREAMS_PER_DIRECTION) */
   ...
   * Bounded by MAX_SERVICE_DEFINITIONS - oldest entries evicted when limit reached.
   */
   ...
   * @param {INetronOptions} [options] - Configuration options for the local Netron instance
  ...
  async exposeService(instance: any) {
    ...
    const stub = new ServiceStub(localPeer, instance, meta);
    localPeer.stubs.set(def.id, stub);
    localPeer.serviceInstances.set(instance, stub);
  ...
  async unexposeService(serviceName: string) {
    ...
    // Clean up stubs
    const localPeer = this.netron.peer as ILocalPeerInternal;
    const stub = localPeer.stubs.get(defId);
    if (stub) {
      localPeer.serviceInstances.delete(stub.instance);
      localPeer.stubs.delete(defId);
    }
   ...
   */
  private sendResponse(packet: Packet, data: unknown) {
    packet.setImpulse(0);
    packet.data = data;
    return this.sendPacket(packet);
  }
   ...
   */
  private sendErrorResponse(packet: Packet, error: unknown) {
    packet.setImpulse(0);
    packet.setError(1);
    packet.data = error;
    return this.sendPacket(packet);
   ...
   */
  sendStreamChunk(streamId: number, chunk: unknown, index: number, isLast: boolean, isLive: boolean) {
  ...
  private handleResponse(packet: Packet) {
    ...
    if (handlers) {
      ...
      this.logger.debug({ packetId: id }, 'Response for unknown packet — handler timed out or already completed');
    }
  }
  /**
  ...
  /**
   * Evicts the oldest entry from a Map when size limit is reached.
   * Uses FIFO eviction (first key in iteration order is oldest due to Map ordering).
   *
   * @param {Map<string, any>} map - The map to evict from
   * @param {number} maxSize - Maximum allowed size
   * @param {string} mapName - Name of the map for logging
   * @private
   */
  private evictOldestIfNeeded(map: Map<string, any>, maxSize: number, mapName: string): void {
    if (map.size >= maxSize) {
      const firstKey = map.keys().next().value;
      if (firstKey) {
        map.delete(firstKey);
        this.logger.debug({ mapName, evictedKey: firstKey, size: map.size }, 'Evicted oldest entry due to size limit');
      }
    }
  }
  ...
  refService(def: Definition, parentDef: Definition) {
    ...
    // Evict oldest if at capacity (FIFO eviction to bound memory)
    this.evictOldestIfNeeded(this.definitions, RemotePeer.MAX_SERVICE_DEFINITIONS, 'definitions');
  ...
  protected async queryInterfaceRemote(qualifiedName: string): Promise<Definition> {
    ...
    // Apply FIFO eviction to bound memory growth
    this.evictOldestIfNeeded(this.definitions, RemotePeer.MAX_SERVICE_DEFINITIONS, 'definitions');
