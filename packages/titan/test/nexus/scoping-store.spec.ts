 ...
 * shared caches, lifecycle, and the `createInstance` hook from its injected
 * ContainerStore rather than from positional arguments — only `context` is
 * passed per call.
 */
...
function makeStore(createInstance: (r: Registration) => any): ContainerStore {
  return {
    instances: new Map(),
...
describe('ScopingService + ContainerStore (NX-9)', () => {
  it('caches a singleton in store.instances (created once)', () => {
    ...
    const a = svc.resolveWithScope(r, {} as any);
    const b = svc.resolveWithScope(r, {} as any);
  ...
  it('isolates scoped instances per scopeId in store.scopedInstances', () => {
    ...
    const a1 = svc.resolveWithScope(r, ctxA);
    const a2 = svc.resolveWithScope(r, ctxA); // same scope → cached
    const b1 = svc.resolveWithScope(r, ctxB); // other scope → fresh
  ...
  it('request scope with no request context falls back to transient', () => {
    ...
    svc.resolveWithScope(r, { metadata: {} } as any);
    svc.resolveWithScope(r, { metadata: {} } as any);
  ...
  it('resolveRegistration caches a singleton in store.instances', () => {
    ...
    const a = svc.resolveRegistration(r, { scope: Scope.Singleton } as any);
    const b = svc.resolveRegistration(r, { scope: Scope.Singleton } as any);
