 ...
 *
 * Uses globalThis to guarantee the SAME token object is returned for the same name
 * regardless of which module path loaded this file. This prevents the dual-package
 * hazard where tsx loads the same file from both src/ and dist/ paths, creating
 * separate module instances with separate registries.
 *
 * @internal
 * @since 0.1.0
 */
const GLOBAL_REGISTRY_KEY = Symbol.for('nexus:global-token-registry');
const tokenRegistry: Map<string, EnhancedToken<any>> =
  (globalThis as any)[GLOBAL_REGISTRY_KEY] ??= new Map<string, EnhancedToken<any>>();
...
export function createToken<T = any>(name: string, metadata: Partial<TokenMetadata> = {}): EnhancedToken<T> {
  ...
  const token: EnhancedToken<T> = {
    ...
    toJSON() {
      const { name: _metadataName, ...cleanMetadata } = token.metadata;
      return {
        name: token.name,
        type: 'Token',
 ...
 */
export function isOptionalToken(token: Token | string | symbol | object): boolean {
  ...
  return (
    tokenObj.metadata?.optional === true ||
    ('isOptional' in token && (token as { isOptional?: boolean }).isOptional === true)
  );
}
...
export class TokenRegistry {
  private static readonly INSTANCE_KEY = Symbol.for('nexus:token-registry-instance');
  private tokens = new Map<symbol, Token>();

  private constructor() {}

  static getInstance(): TokenRegistry {
    const existing = (globalThis as any)[TokenRegistry.INSTANCE_KEY];
    if (existing) return existing;
    const instance = new TokenRegistry();
    (globalThis as any)[TokenRegistry.INSTANCE_KEY] = instance;
    return instance;
  }
   ...
   */
  static reset(): void {
    const instance = (globalThis as any)[TokenRegistry.INSTANCE_KEY];
    if (instance) {
      instance.clear();
      (globalThis as any)[TokenRegistry.INSTANCE_KEY] = undefined;
    }
  }
}
