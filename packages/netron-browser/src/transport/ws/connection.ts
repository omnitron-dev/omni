...
export interface WebSocketConnectionOptions {
  ...
  /** Maximum queue size */
  maxQueueSize?: number;
  /** Path prefix for WebSocket URL (e.g., '/api/v1', '/core') */
  pathPrefix?: string;
}
...
/**
 * Build WebSocket URL with optional path prefix.
 * If the provided URL is HTTP(S), converts it to WS(S) and appends pathPrefix.
 *
 * @param url - The base URL (can be ws://, wss://, http://, or https://)
 * @param pathPrefix - Optional path prefix to append
 * @returns WebSocket URL with pathPrefix appended
 *
 * @example
 * buildWebSocketUrl('http://localhost:3000', '/api/v1')  // 'ws://localhost:3000/api/v1'
 * buildWebSocketUrl('https://example.com', '/core')      // 'wss://example.com/core'
 * buildWebSocketUrl('ws://localhost:3000/path', '')      // 'ws://localhost:3000/path'
 */
function buildWebSocketUrl(url: string, pathPrefix: string): string {
  // Convert HTTP(S) to WS(S) if needed
  let wsUrl = url;
  if (url.startsWith('http://')) {
    wsUrl = 'ws://' + url.slice(7);
  } else if (url.startsWith('https://')) {
    wsUrl = 'wss://' + url.slice(8);
  }
...
export class WebSocketConnection extends EventEmitter {
  ...
  private keepAlivePongTimeout?: ReturnType<typeof setTimeout>;
  ...
  constructor(url: string, options?: WebSocketConnectionOptions) {
    ...
    // Build the WebSocket URL with pathPrefix applied
    this.url = buildWebSocketUrl(url, this.pathPrefix);
