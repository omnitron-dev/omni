...
import { MiddlewareStage, type ClientMiddlewareContext, type IMiddlewareManager } from '../../middleware/index.js';
...
/**
 * HTTP Remote Peer options
 */
export interface HttpRemotePeerOptions {
...
export class HttpRemotePeer extends AbstractPeer {
  ...
  /** In-memory cache for response hint-based caching (fallback when no cache manager) */
  private hintCache = new Map<string, { data: any; expiresAt: number; etag?: string }>();

  /** Middleware pipeline for request/response processing */
  public middleware?: IMiddlewareManager;

  constructor(baseUrl: string, options?: HttpRemotePeerOptions) {
   ...
   * - Middleware pipeline execution (pre-request, post-response, error stages)
   * - HTTP response caching via the cache manager
  ...
  override async call(defId: string, method: string, args?: any[]): Promise<any> {
    ...
    // Create middleware context if middleware is configured
    let ctx: ClientMiddlewareContext | undefined;
    if (this.middleware) {
      ctx = {
        service: serviceName,
        method,
    ...
    try {
      ...
      if (ctx && this.middleware) {
        await this.middleware.execute(ctx, MiddlewareStage.PRE_REQUEST);

        // Check if middleware wants to skip remaining
        if (ctx.skipRemaining && ctx.response?.data !== undefined) {
          return ctx.response.data;
        }
      }

      // Build cache key for HTTP caching
      const cacheKey = `${serviceName}:${method}:${JSON.stringify(callArgs)}`;

      // Check cache first if cache manager is configured
      if (this.cacheManager) {
        const cachedData = await this.cacheManager.get(
          cacheKey,
          async () => {
            // Cache miss - fetch from server
            const message = createRequestMessage(serviceName, method, callArgs, {
          ...
          {
            maxAge: this.globalOptions.cache?.maxAge || 60000,
            staleWhileRevalidate: this.globalOptions.cache?.staleWhileRevalidate,
            tags: [serviceName, `${serviceName}:${method}`],
          }
        );

        // Store response in context for middleware
        if (ctx) {
          ctx.response = { data: cachedData, headers: {}, metadata: {} };
        }
        ...
        return ctx?.response?.data ?? cachedData;
      }

      // Check hint cache (populated from server response cache hints)
      if (this.hintCache.size > 0) {
        const hintCacheKey = `${serviceName}:${method}:${JSON.stringify(callArgs)}`;
        const hintEntry = this.hintCache.get(hintCacheKey);
        if (hintEntry && hintEntry.expiresAt > Date.now()) {
          this.logger.debug('Hint cache hit', { key: hintCacheKey });

          if (ctx) {
            ctx.response = { data: hintEntry.data, headers: {}, metadata: {} };
          }
          if (ctx && this.middleware) {
            await this.middleware.execute(ctx, MiddlewareStage.POST_RESPONSE);
          }
          return ctx?.response?.data ?? hintEntry.data;
        }
        // Remove expired entry
        if (hintEntry) {
          this.hintCache.delete(hintCacheKey);
        }
      }

      // No cache manager - direct request
      const message = createRequestMessage(serviceName, method, callArgs, {
        context: this.buildRequestContext(),
        hints: this.buildRequestHints(),
      });
      ...
      if (response.hints?.cache) {
        this.handleCacheHints(serviceName, method, callArgs[0], response.data, response.hints.cache);
      }

      // Store response in context for middleware
      if (ctx) {
        ctx.response = { data: response.data, headers: {}, metadata: {} };
      }
      ...
      return ctx?.response?.data ?? response.data;
    } catch (error: any) {
      // Store error in context and execute error middleware
      if (ctx && this.middleware) {
        ctx.error = error;
        try {
  ...
  private buildRequestContext(): HttpRequestContext {
    ...
    const traceId = crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
    context.traceId = traceId;
    context.spanId = crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;

    // Propagate user context from default headers (set by auth interceptors)
    if (this.defaultOptions.headers?.['x-user-id']) {
      context.userId = this.defaultOptions.headers['x-user-id'];
    }
    if (this.defaultOptions.headers?.['x-tenant-id']) {
      context.tenantId = this.defaultOptions.headers['x-tenant-id'];
    }
  ...
  private buildRequestHints(): HttpRequestHints {
    ...
    // Propagate cache hints from global options
    if (this.globalOptions.cache) {
      hints.cache = {
        maxAge: this.globalOptions.cache.maxAge,
        staleWhileRevalidate: this.globalOptions.cache.staleWhileRevalidate,
        tags: this.globalOptions.cache.tags,
      };
    }

    // Propagate retry hints from global options
    if (this.globalOptions.retry) {
      hints.retry = {
        attempts: this.globalOptions.retry.attempts,
        backoff: this.globalOptions.retry.backoff,
        maxDelay: this.globalOptions.retry.maxDelay,
        initialDelay: this.globalOptions.retry.initialDelay,
      };
    }

    // Propagate priority and timeout from global options
    if (this.globalOptions.priority) {
      hints.priority = this.globalOptions.priority;
    }
    if (this.globalOptions.timeout) {
      hints.timeout = this.globalOptions.timeout;
    }
  ...
  private handleCacheHints(
    ...
    if (!cacheHints || cacheHints.noStore) {
      return;
    }

    const maxAge = cacheHints.maxAge;
    if (!maxAge || maxAge <= 0) {
      return;
    }

    const cacheKey = `${service}:${method}:${JSON.stringify(input)}`;
    const tags = [service, `${service}:${method}`, ...(cacheHints.tags ?? [])];

    if (this.cacheManager) {
      // Use the existing cache manager infrastructure
      this.cacheManager.set(cacheKey, output, {
        maxAge,
        tags,
      });
      this.logger.debug('Cached response via cache manager', { cacheKey, maxAge, tags });
    } else {
      // Use the in-memory hint cache as fallback
      this.hintCache.set(cacheKey, {
        data: output,
        expiresAt: Date.now() + maxAge,
        etag: cacheHints.etag,
      });
      ...
      this.logger.debug('Cached response in hint cache', { cacheKey, maxAge });
    }
  }
  ...
  private getOrCreateHttpClient(): HttpTransportClient {
    // Create a new HttpTransportClient with the base URL and pathPrefix
   ...
   * @returns Object with total count and breakdown by cache type
   *
   * @example
   * // Clear all cache
   * const result = await peer.invalidateCache();
   * console.log(`Cleared ${result.count} entries`);
   *
   * @example
   * // Clear specific service
   * const result = await peer.invalidateCache('UserService@1.0.0');
   *
   * @example
   * // Clear all services starting with "User"
   * const result = await peer.invalidateCache('User*');
   *
   * @example
   * // Clear only HTTP cache
   * const result = await peer.invalidateCache(undefined, 'http');
   */
  async invalidateCache(
    pattern?: string,
    cacheType: 'service' | 'http' | 'all' = 'all'
  ): Promise<{ count: number; breakdown?: { service?: number; http?: number } }> {
    let totalCount = 0;
    const breakdown: { service?: number; http?: number } = {};
    ...
    if (cacheType === 'service' || cacheType === 'all') {
      ...
      breakdown.service = serviceCount;
   ...
   *
   * Returns statistics for both service definition cache and HTTP response cache.
   *
   * @returns Cache statistics object with counts and hits for both cache types
   *
   * @example
   * const stats = peer.getCacheStats();
   * console.log(`HTTP cache: ${stats.http.size} entries, ${stats.http.hits} hits`);
   * console.log(`Definitions cache: ${stats.definitions.size} entries`);
   */
  getCacheStats(): {
    http: { size: number; hits: number; misses: number };
    definitions: { size: number };
  } {
    const httpStats = this.cacheManager ? this.cacheManager.getStats() : { entries: 0, hits: 0, misses: 0 };

    return {
      http: {
        size: httpStats.entries,
        hits: httpStats.hits,
        misses: httpStats.misses,
      },
      definitions: {
        size: this.services.size + this.interfaces.size,
      },
    };
  }
