...
export class DatabasePersistenceProvider implements IPersistenceProvider {
  ...
  async saveJob(job: IScheduledJob): Promise<void> {
    ...
    await this.db
      ...
      .onConflict((oc) => oc.column('id').doUpdateSet(serializedJob as any))
      .execute();
  }

  async loadJob(id: string): Promise<IScheduledJob | null> {
    const row = await this.db.selectFrom('scheduler_jobs').selectAll().where('id', '=', id).executeTakeFirst();
  ...
  async loadAllJobs(): Promise<IScheduledJob[]> {
    const rows = await this.db.selectFrom('scheduler_jobs').selectAll().execute();
  ...
  async deleteJob(id: string): Promise<void> {
    // Delete job
    await this.db.deleteFrom('scheduler_jobs').where('id', '=', id).execute();

    // Delete execution history
    await this.db.deleteFrom('scheduler_execution_history').where('job_id', '=', id).execute();
  }
  ...
  private serializeJobForDb(job: IScheduledJob): Partial<SchedulerJobsTable> {
    const { instance: _instance, target, lastError, options, ...rest } = job;
