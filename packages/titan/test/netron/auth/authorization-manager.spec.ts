...
describe('AuthorizationManager', () => {
  ...
  describe('validateScopes', () => {
    it('should return valid when no scopes required', () => {
      const result = authzManager.validateScopes([], ['read', 'write']);
      expect(result.valid).toBe(true);
      expect(result.missing).toEqual([]);
    });

    it('should return valid when user has all required scopes', () => {
      const result = authzManager.validateScopes(['read', 'write'], ['read', 'write', 'delete']);
      expect(result.valid).toBe(true);
      expect(result.missing).toEqual([]);
    });

    it('should return invalid with missing scopes', () => {
      const result = authzManager.validateScopes(['read', 'write', 'admin'], ['read']);
      expect(result.valid).toBe(false);
      expect(result.missing).toEqual(['write', 'admin']);
    });

    it('should handle undefined user scopes', () => {
      const result = authzManager.validateScopes(['read'], undefined);
      expect(result.valid).toBe(false);
      expect(result.missing).toEqual(['read']);
    });

    it('should handle empty user scopes', () => {
      const result = authzManager.validateScopes(['read'], []);
      expect(result.valid).toBe(false);
      expect(result.missing).toEqual(['read']);
    });
  });
  ...
  describe('validateAccess', () => {
    ...
    it('should allow access when user has required role (ANY logic)', () => {
      const authContext = {
        userId: 'user1',
        roles: ['admin', 'user'],
        permissions: [],
    ...
    it('should deny access when user lacks all required roles', () => {
      ...
      expect(result.details?.missingRoles).toEqual(['admin', 'superadmin']);
    });

    it('should allow access when user has all required permissions (ALL logic)', () => {
      const authContext = {
        userId: 'user1',
        roles: [],
    ...
    it('should deny access when user lacks some required permissions', () => {
      ...
      expect(result.details?.missingPermissions).toEqual(['write']);
    });
    ...
    it('should deny access when user lacks required scopes', () => {
      const authContext = {
        userId: 'user1',
        roles: [],
        permissions: [],
    ...
    it('should check roles first, then permissions, then scopes', () => {
      const authContext = {
        userId: 'user1',
        roles: ['user'],
        permissions: ['read'],
      ...
      // Fails on permissions (roles pass)
      ...
      // Fails on scopes (roles and permissions pass)
