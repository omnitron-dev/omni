...
/**
 * Simplified Zod schema type that avoids deep type instantiation issues.
 * Using 'any' for type parameters prevents TypeScript from recursively
 * resolving complex nested Zod schemas, which can cause TS2589 errors
 * and SIGSEGV crashes during compilation.
 */
 ...
 */
export const CONFIG_INJECT_METADATA_KEY = Symbol.for('titan:config:inject');
export const CONFIG_SCHEMA_METADATA_KEY = Symbol.for('titan:config:schema');
...
export interface IConfigValidator {
   ...
   */
  validate(config: Record<string, any>, schema?: AnyZodSchema): IConfigValidationResult;

  /**
   * Validate a specific path
   */
  validatePath(path: string, value: any, schema?: AnyZodSchema): IConfigValidationResult;
}
