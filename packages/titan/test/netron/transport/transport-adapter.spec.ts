...
describe('BinaryTransportAdapter', () => {
  ...
  beforeEach(() => {
    ...
    mockConnection.send = vi.fn().mockResolvedValue(undefined);
    mockConnection.sendPacket = vi.fn().mockResolvedValue(undefined);
    mockConnection.close = vi.fn().mockResolvedValue(undefined);
    mockConnection.ping = vi.fn().mockResolvedValue(10);
  ...
  describe('Data Conversion in send()', () => {
    ...
    it('should convert ArrayBuffer to Buffer', async () => {
      ...
      const sentBuffer = (mockConnection.send as vi.Mock).mock.calls[0][0];
  ...
  describe('Close and Terminate', () => {
    ...
    it('should handle close error', async () => {
      ...
      (mockConnection.close as vi.Mock).mockRejectedValue(error);
  ...
  describe('Ping and Pong', () => {
    ...
    it('should handle ping error with callback', async () => {
      ...
      (mockConnection.ping as vi.Mock).mockRejectedValue(error);
...
describe('NativeWebSocketWrapper', () => {
  ...
  beforeEach(() => {
    ...
    mockWs.send = vi.fn((data, callback) => callback?.());
    mockWs.close = vi.fn();
    mockWs.ping = vi.fn((callback) => callback?.());
    mockWs._socket = {
  ...
  describe('Send Methods', () => {
    ...
    it('should reject on send error', async () => {
      const error = new Error('Send failed');
      mockWs.send = vi.fn((data, callback) => callback(error));
    ...
    it('should send packet with encoding', async () => {
      ...
      const sentData = (mockWs.send as vi.Mock).mock.calls[0][0];
  ...
  describe('Ping Method', () => {
    ...
    it('should reject on ping error', async () => {
      const error = new Error('Ping failed');
      mockWs.ping = vi.fn((callback) => callback(error));
