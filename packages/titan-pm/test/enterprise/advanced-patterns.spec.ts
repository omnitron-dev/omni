...
} from '../../src/enterprise/advanced-patterns.js';
...
describe('Bulkhead', () => {
  ...
  describe('Drain', () => {
    it('should reject all queued requests on drain', async () => {
      ...
      // Queue some tasks
      const queuedTasks = [bulkhead.execute(async () => 'q1'), bulkhead.execute(async () => 'q2')];
...
describe('ResilienceBuilder', () => {
  ...
  describe('Combined Execution', () => {
    ...
    it('should apply circuit breaker', async () => {
      ...
      await expect(builder.execute(async () => 'success')).rejects.toThrow('Circuit breaker is open');
    });
