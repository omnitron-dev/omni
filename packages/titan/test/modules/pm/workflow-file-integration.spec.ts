...
describe('Workflow File Integration - Performance and Scalability', () => {
  ...
  it('should handle high-volume concurrent workflow execution', async () => {
    ...
    const workflows = await Promise.all(Array.from({ length: 20 }, () => pm.workflow(workflowPath)));

    // Execute all concurrently
    const startTime = Date.now();
    const results = await Promise.all(workflows.map((wf, i) => (wf as any).run(`input-${i}`)));
  ...
  it('should efficiently cache workflow modules', async () => {
    ...
    // Use a minimum baseline of 1ms to avoid division by zero issues in fast environments
    // Allow 5x tolerance for mock environment variability and system load fluctuations
    const baselineTime = Math.max(loadTime1, 1);
    expect(loadTime2).toBeLessThanOrEqual(baselineTime * 5);
    expect(loadTime3).toBeLessThanOrEqual(baselineTime * 5);
  });
});
