...
// Performance tests can be enabled with ENABLE_PERF_TESTS=true
// They are skipped in CI due to memory constraints
const skipTests = process.env.ENABLE_PERF_TESTS !== 'true' || process.env.CI === 'true';
if (skipTests) {
  console.log(
    'Skipping auth-performance.spec.ts - ' +
      (process.env.CI === 'true' ? 'performance tests disabled in CI' : 'set ENABLE_PERF_TESTS=true to run locally')
  );
}
...
describeOrSkip('Auth Performance Tests', () => {
  ...
  describe('Authentication Throughput', () => {
    it('should handle > 10K authentications per second', async () => {
      ...
      const mockAuth = vi.fn(async () => authContext);
