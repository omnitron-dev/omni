...
describe('MockAdapter', () => {
  ...
  describe('execute', () => {
    ...
    it('should throw mocked error', async () => {
      ...
      await expect(adapter.execute({ command: 'error' })).rejects.toThrow(AdapterError);
    });
    ...
    it('should preserve CommandError and TimeoutError types', async () => {
      ...
      await expect(adapter.execute({ command: 'cmd-error' })).rejects.toThrow(CommandError);

      await expect(adapter.execute({ command: 'timeout-error' })).rejects.toThrow(TimeoutError);
    });
  });
});
