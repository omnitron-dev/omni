...
import type { ILogger } from '../../../../src/modules/logger/index.js';
...
describeOrSkip('Redis Manager - Infrastructure Tests', () => {
    ...
    it('should initialize with single client configuration', async () => {
      const options: RedisModuleOptions = {
        config: {
          ...
          namespace: 'default',
          lazyConnect: false, // Explicitly connect on init
        },
      };
      ...
      manager = new RedisManager(
        {
          config: {
            host: testContainer.host,
            port: testContainer.port,
            lazyConnect: false, // Explicitly connect on init for tests expecting ready status
          },
        },
        mockLogger
      );
  ...
  describe('Health Checks', () => {
      ...
      manager = new RedisManager(
        {
          config: {
            host: testContainer.host,
            port: testContainer.port,
            lazyConnect: false, // Explicitly connect on init for health checks
          },
        },
        mockLogger
      );
    ...
    it('should return false for disconnected client', async () => {
      ...
      // Health check for destroyed client should return false (not throw)
      const isHealthy = await manager.isHealthy('temp');
  ...
  describe('Script Management', () => {
    ...
    it('should throw error for non-existent script', async () => {
      ...
      await expect(manager.runScript('nonexistent', [], [])).rejects.toThrow();
    });
  ...
  describe('Lifecycle Management', () => {
    ...
    it('should call onModuleDestroy', async () => {
      ...
      // After destroy, clients should be removed from manager
      expect(manager.getClients().size).toBe(0);
      // Give ioredis time to transition status (quit() sends command, status updates async)
      await delay(50);
      // Verify the client status changed (either 'end' or 'close')
