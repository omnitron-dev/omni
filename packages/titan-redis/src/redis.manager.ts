...
import type { IRedisClient, IRedisClientOptions } from './redis.interfaces.js';
...
export class RedisManager {
  ...
  private async initializeClients(): Promise<void> {
    const clientConfigs: InternalRedisClientOptions[] = [];
  ...
  private async createAndRegisterClient(options: InternalRedisClientOptions): Promise<InternalRedisClient> {
  ...
  private async connectClient(
    ...
    namespace: string,
    options: InternalRedisClientOptions
  ): Promise<void> {
    try {
  ...
  private setupEventListeners(client: InternalRedisClient, namespace: string): void {
   ...
   * Returns the public IRedisClient interface type.
  ...
  /**
   * @internal
   * Get the internal Redis client for operations that need direct ioredis access.
   * This should only be used within the redis module implementation.
   * @param namespace - Optional namespace for the Redis client
   * @returns The internal Redis client instance
   */
  getInternalClient(namespace?: string): InternalRedisClient {
    const ns = namespace || 'default';
    const client = this.clients.get(ns);
  ...
  /**
   * Get all registered Redis clients.
   * @returns Map of namespace to client instances
   */
  getClients(): Map<string, IRedisClient> {
    const result = new Map<string, IRedisClient>();
    for (const [ns, client] of this.clients) {
      result.set(ns, client as unknown as IRedisClient);
    }
    return result;
  }
   ...
   * @returns The created Redis client instance
   */
  async createClient(options: IRedisClientOptions): Promise<IRedisClient> {
    const internalOptions = toInternalClientOptions(options);
    const namespace = getClientNamespace(internalOptions);

    if (this.clients.has(namespace)) {
      throw Errors.conflict(`Redis client with namespace "${namespace}" already exists`);
    }

    const mergedOptions = mergeOptions(this.options.commonOptions, internalOptions);
    const client = await this.createAndRegisterClient(mergedOptions);
    return client as unknown as IRedisClient;
  }
  ...
  async runScript<T = any>(
    ...
    const ns = namespace || 'default';
    const client = this.getInternalClient(ns);
