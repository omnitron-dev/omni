...
export class LoadBalancer {
  ...
  private responseTime(): ServiceInstance {
    ...
    const noDataInstances: ServiceInstance[] = [];
    let instancesWithData = 0;

    for (const instance of this.instances) {
      const times = this.responseTimes.get(instance.id) || [];
      if (times.length === 0) {
        // Track instances with no data
        noDataInstances.push(instance);
        continue;
      }
    ...
    // If there are endpoints with no data and only one endpoint with data,
    // prefer exploring the no-data endpoints
    if (noDataInstances.length > 0 && instancesWithData === 1) {
      return noDataInstances[0]!;
    }

    // If we found an instance with data, return it (prefer lowest average when multiple have data)
    if (selected) {
      return selected;
    }

    // Otherwise, return the first instance with no data (all instances have no data)
    return noDataInstances[0] || this.instances[0]!;
  }
