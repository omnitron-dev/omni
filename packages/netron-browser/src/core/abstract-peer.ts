/**
 * Browser-compatible AbstractPeer implementation
 * Adapted from server version with browser-specific optimizations:
 * - LRU cache with TTL for definition caching
 * - Pattern regex cache with ReDoS prevention
...
/**
 * Configuration options for definition cache
 */
...
export abstract class AbstractPeer implements IPeer {
   ...
   * Features:
   * - LRU eviction when max size reached (default 500)
  ...
  /**
   * Flag to disable caching entirely (useful for testing or debugging)
   */
  protected definitionCacheDisabled: boolean;

  /**
   * Cache of compiled regex patterns for pattern matching.
   * Prevents regex recompilation and provides LRU eviction.
   */
   ...
   */
  private static readonly MAX_PATTERN_CACHE_SIZE = 100;
  ...
  constructor(
    ...
    this.definitionCacheDisabled = cacheOptions?.disabled ?? DEFAULT_DEFINITION_CACHE_OPTIONS.disabled;

    this.definitionCache = new LRUCache<string, Definition>({
   ...
   * communication with the remote peer (e.g., via WebSocket or HTTP request).
   *
  ...
  async queryInterface<T>(qualifiedName: string): Promise<T> {
    ...
    // Check definition cache first (skip if caching is disabled)
  ...
  /**
   * Retrieves names of all services available.
   *
   * @returns {string[]} Array of service names
   * @abstract
   */
  protected abstract getServiceNames(): string[];
  ...
  protected findLatestServiceVersion(serviceName: string): Definition {
    ...
    const candidates = Array.from(this.getServiceNames())
      ...
      // Sort versions in descending order using simplified semver comparison
   ...
   * Uses a pattern regex cache with LRU eviction to prevent ReDoS attacks
   * and improve performance for repeated pattern matching.
   *
  ...
  private matchesPattern(serviceName: string, pattern: string): boolean {
    ...
    // No wildcards - no match (fast path)
    if (!pattern.includes('*')) {
      return false;
    }
    ...
    if (regex) {
      // LRU promotion: delete and re-add to move to end (most recently used)
      this.patternRegexCache.delete(pattern);
      this.patternRegexCache.set(pattern, regex);
    } else {
      // SECURITY: Escape special regex characters to prevent ReDoS attacks.
      // Without escaping, a malicious pattern like "test.*+?^" could cause
      // exponential backtracking in the regex engine.
      ...
      if (this.patternRegexCache.size >= AbstractPeer.MAX_PATTERN_CACHE_SIZE) {
        const firstKey = this.patternRegexCache.keys().next().value;
        if (firstKey !== undefined) {
          this.patternRegexCache.delete(firstKey);
        }
      }
      this.patternRegexCache.set(pattern, regex);
    }
   ...
   *
   * @returns {LRUCacheStats} Cache statistics including hits, misses, and hit rate
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  getDefinitionCacheStats(): LRUCacheStats {
  ...
  /**
   * Dispose of the cache and stop background cleanup timer.
   * Call this when the peer is being destroyed to prevent memory leaks.
   */
