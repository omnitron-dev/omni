...
const copydirSync = require('../../tools/copydirSync');
 ...
 * - Always ON, a module is always up along OMNITRON, to stop it, you need to uninstall it
 * - Install a runnable module from NPM/Github/HTTP (require a package.json only)
 * - Some modules add internal OMNITRON depencencies (like typescript, profiling...)
 ...
 * - Generate sample module via omnitron module:generate <module_name>
 */

function localStart(OMNITRON, opts, cb) {
  var proc_path = '',
    cmd = '',
    conf = {};
  ...
  return StartModule(OMNITRON, opts, function (err, dt) {
...
function generateSample(app_name, cb) {
  ...
  function samplize(module_name) {
    var cmd1 =
      'git clone https://github.com/omnitron-hive/sample-module.git ' +
      module_name +
      '; cd ' +
      module_name +
      '; rm -rf .git';
    ...
    sexec(cmd1, function (err) {
      ...
      sexec(cmd2, function (err) {
        ...
        sexec(cmd3, function (err) {
          ...
          Common.printOut('$ omnitron install . ');
          console.log('');

          Common.printOut('Module Log: ');
          Common.printOut('$ omnitron logs ' + module_name);
          console.log('');
          Common.printOut('Uninstall module: ');
          Common.printOut('$ omnitron uninstall ' + module_name);
          console.log('');
          Common.printOut('Force restart: ');
          Common.printOut('$ omnitron restart ' + module_name);
          return cb ? cb() : false;
        });
      });
    });
    ...
    fs.writeFile(package_file, JSON.stringify(package_json, null, 2), function (err, data) {
      ...
      sexec('npm publish', function (code) {
        ...
        sexec('git add . ; git commit -m "' + package_json.version + '"; git push origin master', function (code) {
          Common.printOut(cst.PREFIX_MSG_MOD + 'Installable with omnitron install %s', package_json.name);
    ...
    Configuration.set(
      ...
      function (err, data) {
        ...
        StartModule(CLI, opts, function (err, dt) {
          ...
          if (process.env.OMNITRON_PROGRAMMATIC === 'true') return cb(null, dt);

          CLI.conf(canonic_module_name, function () {
            Common.printOut(cst.PREFIX_MSG_MOD + 'Module successfully installed and launched');
            Common.printOut(cst.PREFIX_MSG_MOD + 'Checkout module options: `$ omnitron conf`');
            return cb(null, dt);
          });
        });
      }
...
function start(OMNITRON, modules, module_name, cb) {
  ...
  StartModule(OMNITRON, opts, function (err, dt) {
...
function StartModule(CLI, opts, cb) {
   ...
   * 1- *apps* field (default omnitron json configuration)
   * 2- *bin* field
   * 3- *main* field
   */
  if (!package_json.apps && !package_json.omnitron) {
    package_json.apps = {};
    ...
    if (opts.safe) {
      ...
      return setTimeout(function () {
        CLI.describe(package_json.name, function (err, apps) {
          if (err || apps[0].omnitron_env.restart_time > 2) {
