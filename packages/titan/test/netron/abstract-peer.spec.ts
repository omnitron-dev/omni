...
import { isNetronPeer } from '../../src/netron/predicates.js';
...
function createMockNetron(overrides: Partial<INetron> = {}): INetron {
  ...
  const mockPeer = {
    ...
    expose: vi.fn(),
    unexpose: vi.fn(),
    exposeRemoteService: vi.fn(),
    unexposeRemoteService: vi.fn(),
    refService: vi.fn(),
    unrefService: vi.fn(),
    getStubByDefinitionId: vi.fn(),
    queryInterfaceByDefId: vi.fn(),
  };
  ...
  const netron: INetron = {
    ...
    peers,
    transportServers,
    getLocalPeer: vi.fn(() => mockPeer as any),
    findPeer: vi.fn(),
    trackTask: vi.fn(),
    emitSpecial: vi.fn(),
...
describe('AbstractPeer', () => {
  ...
  describe('LRU Cache Behavior', () => {
    it('should cache definitions when querying interface', async () => {
      const _def = peer.addService('testService', '1.0.0');
    ...
    it('should not cache definitions when caching is disabled', async () => {
      ...
      const _def = disabledPeer.addService('testService', '1.0.0');
  ...
  describe('Interface Lifecycle', () => {
    ...
    it('should decrement reference count on release', async () => {
      ...
      const iface1 = await peer.queryInterface('testService');
      const _iface2 = await peer.queryInterface('testService');
    ...
    it('should release child interfaces when parent is released', async () => {
      ...
      const _childIface = await peer.queryInterface('childService');
  ...
  describe('Service Version Resolution', () => {
    ...
    it('should handle service name with @ character', async () => {
      const _def = peer.addService('test', '1.0.0');
  ...
  describe('isNetronPeer Predicate', () => {
    ...
    it('should return true for duck-typed object with peer shape', () => {
      // Note: With duck-typing (used to break circular deps), objects with
      // the correct shape are considered peers. This is by design.
      const fakePeer = {
        id: 'fake',
        netron: createMockNetron(),
        queryInterface: () => {},
      ...
      expect(isNetronPeer(fakePeer)).toBe(true);
    });
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('should handle concurrent interface queries', async () => {
      const _def = peer.addService('concurrentService', '1.0.0');

      const promises = Array(10)
        .fill(null)
        .map(() => peer.queryInterface('concurrentService'));
