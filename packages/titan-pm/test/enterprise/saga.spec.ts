...
describe('Saga Orchestrator', () => {
  ...
  describe('Saga Execution', () => {
    it('should execute simple saga successfully', async () => {
      ...
      expect(results).toEqual(['inventory-reserved', 'payment-charged', 'order-shipped']);
    ...
    it('should pass data between steps', async () => {
      const steps: ISagaStep[] = [
        {
          name: 'step1',
          handler: async (input) => ({ value: input.initial * 2 }),
        },
        {
          name: 'step2',
          handler: async (input) => ({ value: input.value * 3 }),
          dependsOn: ['step1'],
        },
      ];
    ...
    it('should timeout long-running steps', async () => {
      ...
      await expect(orchestrator.execute('timeout-saga')).rejects.toThrow();
    });
  });
  ...
  describe('Saga Compensation', () => {
    it('should compensate completed steps on failure', async () => {
      ...
      await expect(orchestrator.execute('compensation-saga')).rejects.toThrow();
    ...
    it('should not compensate failed steps', async () => {
      ...
      await expect(orchestrator.execute('partial-compensation')).rejects.toThrow();
    ...
    it('should log compensation failures but continue', async () => {
      ...
      await expect(orchestrator.execute('compensation-error-saga')).rejects.toThrow('Step failed');
  ...
  describe('Edge Cases', () => {
    it('should throw error for non-existent saga', async () => {
      await expect(orchestrator.execute('non-existent-saga')).rejects.toThrow('not found');
    });
    ...
    it('should detect circular dependencies', () => {
      ...
      expect(orchestrator.execute('circular-saga')).rejects.toThrow('Circular dependency');
    });
  });
});
