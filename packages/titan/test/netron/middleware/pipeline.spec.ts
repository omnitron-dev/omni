...
describe('MiddlewarePipeline', () => {
  ...
  describe('Middleware Registration', () => {
    it('should register global middleware', () => {
      const middleware: MiddlewareFunction = vi.fn(async (ctx, next) => {
        await next();
      });
    ...
    it('should sort middleware by priority', () => {
      const middleware1: MiddlewareFunction = vi.fn(async (ctx, next) => {
        await next();
      });
      const middleware2: MiddlewareFunction = vi.fn(async (ctx, next) => {
        await next();
      });
      const middleware3: MiddlewareFunction = vi.fn(async (ctx, next) => {
        await next();
      });
