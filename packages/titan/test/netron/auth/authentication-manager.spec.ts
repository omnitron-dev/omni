...
describe('AuthenticationManager', () => {
  ...
  let mockLogger: any;
  let mockAuthenticateFn: vi.Mock;
  let mockValidateTokenFn: vi.Mock;
  ...
  describe('async authentication providers', () => {
    ...
    it('should handle sync authentication function', async () => {
      ...
      const syncAuthFn = vi.fn(() => authContext);
  ...
  describe('token validation caching', () => {
    it('should cache successful token validations', async () => {
      ...
      expect(mockValidateTokenFn).toHaveBeenCalledTimes(1); // Still 1 (cached)
    });
    ...
    it('should return cache statistics', async () => {
      ...
      // Second validation of same token - hit
      await authManager.validateToken('token1');
      stats = authManager.getCacheStats();
    ...
    it('should work without cache when disabled', async () => {
      ...
      // Should call validator each time (no caching)
      expect(mockValidateTokenFn).toHaveBeenCalledTimes(2);
    });
    ...
    it('should use custom TTL from configuration', async () => {
      ...
      // After TTL expiry - should call validator again
      await authManager.validateToken('token1');
      expect(mockValidateTokenFn).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('async audit logging', () => {
    it('should use non-blocking audit by default', async () => {
      const mockAuditLogger = {
        logAuth: vi.fn().mockResolvedValue(undefined),
      };

      const manager = new AuthenticationManager(mockLogger, undefined, mockAuditLogger as any);
      ...
      // Audit should be called but non-blocking
      expect(mockAuditLogger.logAuth).toHaveBeenCalled();
    });

    it('should handle audit logging errors gracefully in async mode', async () => {
      const mockAuditLogger = {
        logAuth: vi.fn().mockRejectedValue(new Error('Audit failed')),
      };
