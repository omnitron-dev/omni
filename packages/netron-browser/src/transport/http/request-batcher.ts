...
export interface BatchOptions {
  ...
  maxRetries?: number;
  /** Batch endpoint URL (relative path, default: '/netron/batch') */
...
export class RequestBatcher extends EventEmitter {
   ...
   *
   * @param endpoint - The endpoint path (e.g., '/netron/batch')
  ...
  private async processBatch(batch: BatchEntry[], reason: string): Promise<void> {
    ...
    try {
      ...
      // Send batch request
      const response = await fetch(this.buildUrl(this.batchEndpoint), {
        method: 'POST',
