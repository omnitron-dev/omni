...
describe('SecurePasswordHandler - Comprehensive Coverage', () => {
  ...
  describe('Askpass Script Creation', () => {
    ...
    it('should set correct permissions on script', async () => {
      ...
      const { mode } = (await import('fs').then(
        (fs) =>
          new Promise((resolve, reject) => fs.stat(scriptPath, (err, stats) => (err ? reject(err) : resolve(stats))))
      )) as any;
    ...
    it('should track created scripts for cleanup', async () => {
      ...
      await Promise.all(scripts.map((script) => expect(access(script, constants.F_OK)).resolves.toBeUndefined()));

      // After cleanup, all should be removed
      await handler.cleanup();

      await Promise.all(scripts.map((script) => expect(access(script, constants.F_OK)).rejects.toThrow()));
    });
  ...
  describe('Cleanup Operations', () => {
    it('should clean up all temporary files', async () => {
      const scripts = await Promise.all([handler.createAskPassScript('pass1'), handler.createAskPassScript('pass2')]);
  ...
  describe('Static Methods', () => {
    ...
    describe('validatePassword', () => {
      it('should validate strong passwords', () => {
        const strongPasswords = ['StrongP@ss123', 'MySecure#Pass2023', 'C0mpl3x!Password', 'Test@123Password'];
  ...
  describe('createSecureEnv', () => {
    ...
    it('should throw after disposal', async () => {
      await handler.dispose();
      expect(() => handler.createSecureEnv('/tmp/askpass-123abc.sh')).toThrow(
        'SecurePasswordHandler has been disposed'
      );
    });
  });
});
