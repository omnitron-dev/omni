 ...
 */
export interface DecoratorContext<TOptions = unknown> {
  target: any;
 ...
 */
export type DecoratorTransform<TOptions = unknown> = (
  context: DecoratorContext<TOptions>
) => void | PropertyDescriptor | unknown;
...
export class CustomDecoratorBuilder<TOptions = any> {
  ...
  build(): (options?: TOptions) => any {
    ...
    return function decoratorFactory(optionsOrTarget?: any, propertyKey?: string | symbol, descriptorOrIndex?: any) {
      ...
      if (isDecoratorFactory) {
        ...
        return function decorator(target: any, propKey?: string | symbol, descOrIndex?: any) {
...
export function combineDecorators(...decorators: Array<(...args: any[]) => any>): (...args: any[]) => any {
  return function combinedDecorator(target: any, propertyKey?: string | symbol, descriptor?: any) {
...
export const Memoize = createMethodInterceptor(
  ...
  (() => {
    ...
    return function memoizeInterceptor(this: any, originalMethod, args, _context) {
...
export const Deprecated = createDecorator<{ message?: string; version?: string }>()
  ...
  .forMethod((context) => {
    ...
    context.descriptor!.value = function deprecatedMethod(this: unknown, ...args: unknown[]) {
      const warning =
        message ||
        `${context.target.constructor.name}.${String(context.propertyKey)} is deprecated` +
          (version ? ` since version ${version}` : '');

      // Check if instance has a logger property
      const instance = this as Record<string, unknown> | null;
      const hasLogger = instance && (instance['logger'] || instance['_logger'] || instance['log']);

      if (hasLogger) {
        // Use instance logger for structured logging
        const logger = getInstanceLogger(instance);
        logger.warn({ deprecated: true }, warning);
      } else {
        // Fallback to structured JSON when no logger is available
        fallbackLog('warn', warning);
      }
