  ...
  async check(): Promise<HealthIndicatorResult> {
    ...
    if (this.thresholds.rssLimit > 0 && memoryInfo.rss > this.thresholds.rssLimit) {
      return {
        ...this.unhealthy(
          'RSS memory (' +
            this.formatBytes(memoryInfo.rss) +
            ') exceeds limit (' +
            this.formatBytes(this.thresholds.rssLimit) +
            ')',
          details
        ),
        latency,
    ...
    if (this.thresholds.externalLimit > 0 && memoryInfo.external > this.thresholds.externalLimit) {
      return {
        ...this.unhealthy(
          'External memory (' +
            this.formatBytes(memoryInfo.external) +
            ') exceeds limit (' +
            this.formatBytes(this.thresholds.externalLimit) +
            ')',
          details
        ),
        latency,
