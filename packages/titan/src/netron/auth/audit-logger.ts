 ...
 * Stores audit events in memory using a circular buffer (useful for testing)
 *
 * Performance characteristics:
 * - save(): O(1) - constant time insertion using circular buffer
 * - query(): O(n) - single pass filtering with compound predicate
 * - clear(): O(1) - resets buffer pointers
 */
export class MemoryAuditAdapter implements AuditStorageAdapter {
  /** Fixed-size circular buffer for O(1) insertion */
  private buffer: (EnhancedAuditEvent | undefined)[];
  ...
  async save(event: AuditEvent): Promise<void> {
    ...
    // O(1) circular buffer insertion - overwrites oldest when full
    this.buffer[this.writeIndex] = enhancedEvent;
  ...
  async query(filter: AuditFilter): Promise<AuditEvent[]> {
    const results: EnhancedAuditEvent[] = [];

    // Iterate through buffer in chronological order (oldest to newest)
    // Start position depends on whether buffer has wrapped
    const startIndex = this.count < this.maxSize ? 0 : this.writeIndex;

    // Single-pass filtering with compound predicate - O(n) instead of O(n*m)
    for (let i = 0; i < this.count; i++) {
      const idx = (startIndex + i) % this.maxSize;
      const e = this.buffer[idx];
      if (!e) continue;

      // Compound predicate - all conditions evaluated in single pass
      if (filter.userId !== undefined && e.userId !== filter.userId) continue;
      if (filter.service !== undefined && e.service !== filter.service) continue;
      if (filter.method !== undefined && e.method !== filter.method) continue;
      if (filter.success !== undefined && e.success !== filter.success) continue;
      if (filter.startTime && e.timestamp < filter.startTime) continue;
      if (filter.endTime && e.timestamp > filter.endTime) continue;
  ...
  async clear(): Promise<void> {
    // O(1) clear - just reset pointers, GC handles the rest
  ...
  getAll(): EnhancedAuditEvent[] {
    const events: EnhancedAuditEvent[] = [];
    const startIndex = this.count < this.maxSize ? 0 : this.writeIndex;
...
export class FileAuditAdapter implements AuditStorageAdapter {
  ...
  async query(filter: AuditFilter): Promise<AuditEvent[]> {
    ...
    const results = events.filter((e) => {
      ...
      if (filter.service !== undefined && e.service !== filter.service) return false;
      if (filter.method !== undefined && e.method !== filter.method) return false;
