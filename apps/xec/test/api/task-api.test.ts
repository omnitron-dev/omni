...
describe('Task API', () => {
  ...
  describe('update()', () => {
    ...
    it('should throw when updating non-existent task', async () => {
      await expect(api.update('nonexistent', { command: 'test' })).rejects.toThrow("Task 'nonexistent' not found");
    });
  });
  ...
  describe('delete()', () => {
    ...
    it('should throw when deleting non-existent task', async () => {
      await expect(api.delete('nonexistent')).rejects.toThrow("Task 'nonexistent' not found");
    });
  });
    ...
    it('should show commands without executing', async () => {
      const config = {
        ...
        tasks: {
          ...
          multi: {
            steps: [{ command: 'echo "Step 1"' }, { command: 'echo "Step 2"' }, { task: 'simple' }],
          },
        },
      };
      ...
      expect(multiCommands).toEqual(['echo "Step 1"', 'echo "Step 2"', '[Task: simple]']);
