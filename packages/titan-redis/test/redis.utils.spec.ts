...
import { RedisClientOptions } from '../src/redis.types.js';
import {
  withDockerRedis,
...
const skipDockerTests = process.env.USE_MOCK_REDIS === 'true' || process.env.SKIP_DOCKER_TESTS === 'true' || !isDockerAvailable();
...
describe('Redis Utils', () => {
  ...
  describeIfRealRedis('waitForConnection', () => {
    let client: Redis;
    let redisConfig: ReturnType<typeof getTestRedisConfig>;
    ...
    beforeEach(() => {
      ...
      redisConfig = getTestRedisConfig(15);
      client = new Redis({ host: redisConfig.host, port: redisConfig.port, db: 15, lazyConnect: true });
      testClients.push(client);
    });
    ...
    it('should timeout if connection takes too long', async () => {
      // Use a non-existent port to ensure timeout
      ...
      disconnectedClient.connect().catch(() => {}); // Ignore connection error
    ...
    it('should clean up event listeners', async () => {
      const removeListenerSpy = vi.spyOn(client, 'removeListener');
