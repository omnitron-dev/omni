...
describe('LoadBalancer', () => {
  ...
  describe('Response Time Strategy', () => {
    ...
    it('should prefer endpoint with no data', () => {
      // Only record data for ONE endpoint to trigger exploration mode
      loadBalancer.recordResponseTime('endpoint-0', 100);

      // When only 1 endpoint has data, algorithm prefers exploring endpoints with no data
      const next = loadBalancer.next();
      expect(next?.id).toBe('endpoint-1'); // First endpoint without data
    });
