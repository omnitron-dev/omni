...
export class EventEmitter {
  ...
  emit(event: string | symbol, ...args: any[]): boolean {
    ...
    if (!Array.isArray(handlers)) {
      ...
      // Multiple handlers - only copy array if once listeners exist
      const needsCopy = this.hasOnceListener.get(event);
      const handlersList = needsCopy ? handlers.slice() : handlers;
      for (let i = 0; i < handlersList.length; i++) {
        const handler = handlersList[i];
  ...
  addListener(event: string | symbol, fn: Function, context?: any, once: boolean = false): this {
    ...
    // Track if this event has any once listeners
    if (once) {
      this.hasOnceListener.set(event, true);
    }
  ...
  removeListener(event: string | symbol, fn: Function): this {
    ...
    if (!Array.isArray(handlers)) {
      ...
      if (handlers.fn === fn) {
        ...
        this._eventsCount--;
        this.hasOnceListener.delete(event);
      }
    } else {
      // Multiple handlers
      ...
      if (filtered.length === 0) {
        ...
        // Check if the remaining handler is a once listener
        if (!filtered[0].once) {
          this.hasOnceListener.delete(event);
        }
      } else {
        this._events.set(event, filtered);
        // Check if any remaining handlers are once listeners
        const hasOnce = filtered.some((h) => h.once);
        if (!hasOnce) {
          this.hasOnceListener.delete(event);
        }
      }
