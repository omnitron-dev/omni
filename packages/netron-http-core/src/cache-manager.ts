 ...
 * @omnitron-dev/netron-browser (previously a drifting per-package copy). Logging
 * is environment-neutral via {@link HttpCoreLogger} + a `debug` console fallback.
 */
...
export class HttpCacheManager extends EventEmitter {
  ...
  private revalidationTimers = new Map<string, ReturnType<typeof setTimeout>>();
  ...
  /**
   * Log debug message (via injected logger, else console when debug enabled)
   */
  ...
  invalidate(pattern: string | RegExp | string[]): void {
    ...
    this.log(`[Cache] INVALIDATED: ${keysToInvalidate.size} entries`);
  ...
  private async revalidateInBackground(key: string, fetcher: () => Promise<any>, options: CacheOptions): Promise<void> {
    ...
    const revalidationPromise = fetcher()
      .then((data) => {
        ...
        this.log(`[Cache] REVALIDATED: ${key}`);
      ...
      .catch((error) => {
        ...
        this.log(`[Cache] REVALIDATION FAILED: ${key}`);
