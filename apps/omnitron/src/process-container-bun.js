...
const omnitron_env = JSON.parse(process.env.omnitron_env);
for (const k in omnitron_env) {
  process.env[k] = omnitron_env[k];
}

// Rename process
process.title = process.env.PROCESS_TITLE || 'bun ' + omnitron_env.pm_exec_path;
    ...
    ['warn', 'error'].forEach((method) => {
      console[method] = (...args) => {
        ...
        if (omnitron_env.log_type && omnitron_env.log_type === 'json') {
          ...
          log_data = `${dayjs().format(omnitron_env.log_date_format)}: ${msg}`;
