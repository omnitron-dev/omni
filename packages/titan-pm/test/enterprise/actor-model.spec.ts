...
} from '../../src/enterprise/actor-model.js';
...
describe('Actor Model', () => {
  ...
  describe('Actor System', () => {
    ...
    it('should prevent duplicate actor IDs', async () => {
      ...
      await expect(system.actorOf(SimpleActor, 'duplicate')).rejects.toThrow('Actor duplicate already exists');
    });
    ...
    it('should stop actors', async () => {
      const _actor = await system.actorOf(SimpleActor, 'stoppable');
  ...
  describe('Actor Lifecycle', () => {
    ...
    it('should call restart lifecycle hooks', async () => {
      const preRestartSpy = vi.fn();
      const postRestartSpy = vi.fn();
