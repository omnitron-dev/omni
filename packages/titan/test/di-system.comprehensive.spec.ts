...
import { Container, createToken, Scope, CircularDependencyError } from '@nexus';
import { Injectable, Inject, Optional, Singleton, Transient } from '@/decorators';
import { Service, OnInit, OnDestroy } from '@/index';
...
describe('Comprehensive DI System Tests', () => {
  ...
  describe('Lifecycle Hooks', () => {
    it('should call OnInit when service is created', async () => {
      ...
      container.register(ServiceWithInit, { useClass: ServiceWithInit, scope: Scope.Singleton });
      const _service = container.resolve(ServiceWithInit);
    ...
    it('should call OnDestroy when container is disposed', async () => {
      ...
      const _service = container.resolve(ServiceWithDestroy);
    ...
    it('should handle initialization order based on dependencies', async () => {
      ...
      container.register(AToken, { useClass: A, scope: Scope.Singleton });
      container.register(BToken, { useClass: B, scope: Scope.Singleton });
  ...
  describe('Error Handling', () => {
    it('should provide clear error messages for missing dependencies', () => {
      ...
      expect(() => container.resolve(ServiceWithMissingDep)).toThrow(/Failed to resolve|Token (is )?not registered/);
    });
    ...
    it('should handle errors in OnInit gracefully', async () => {
      const onInitSpy = vi.fn().mockRejectedValue(new Error('Init failed'));
      ...
      container.register(ServiceWithFailingInit, { useClass: ServiceWithFailingInit, scope: Scope.Singleton });
      container.resolve(ServiceWithFailingInit);

      // initialize() catches onInit errors gracefully (doesn't crash the app)
      // — this is by design: one failing service shouldn't block all others
      await container.initialize();
      expect(onInitSpy).toHaveBeenCalled();
    });
  });
  ...
  describe('Advanced Patterns', () => {
    it('should support decorator composition', () => {
      ...
      function TestDecorator1() {
        return function testDecorator1Impl(target: any) {
          metadata.push('TestDecorator1');
        };
      }

      function TestDecorator2() {
        return function testDecorator2Impl(target: any) {
