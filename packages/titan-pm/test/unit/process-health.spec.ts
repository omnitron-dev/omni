...
import type { IHealthStatus, ServiceProxy } from '../../src/types.js';
...
const createMockProxy = (healthStatus: IHealthStatus['status'] = 'healthy'): ServiceProxy<any> =>
  ({
    ...
    __getMetrics: vi.fn().mockResolvedValue({ cpu: 50, memory: 512, requests: 100, errors: 0 }),
...
describe('ProcessHealthChecker', () => {
  ...
  describe('Start Monitoring', () => {
    ...
    it('should not start if already monitoring', async () => {
      ...
      expect((proxy.__getHealth as vi.Mock).mock.calls.length).toBeLessThanOrEqual(3);
    });
    ...
    it('should log debug message when starting', () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith({ processId: 'proc-1' }, 'Starting health monitoring');
    });
  });
  ...
  describe('Stop Monitoring', () => {
    it('should stop health monitoring', async () => {
      ...
      const callsBefore = (proxy.__getHealth as vi.Mock).mock.calls.length;
    ...
    it('should log debug message when stopping', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith({ processId: 'proc-1' }, 'Stopped health monitoring');
    });
  });
  ...
  describe('Health Check with Retries', () => {
    it('should retry on health check failure', async () => {
      const proxy = createMockProxy();
      let attempts = 0;
      (proxy.__getHealth as vi.Mock).mockImplementation(async () => {
        attempts++;
        if (attempts < 3) {
    ...
    it('should mark unhealthy after retries exhausted', async () => {
      const proxy = createMockProxy();
      (proxy.__getHealth as vi.Mock).mockRejectedValue(new Error('Persistent failure'));
  ...
  describe('Fallback Health Check', () => {
    it('should use basic health check when __getHealth not available', async () => {
      const proxy = {
        __processId: 'test-process',
        __destroy: vi.fn(),
        __getMetrics: vi.fn().mockResolvedValue({ cpu: 50, memory: 512 }),
      } as any;
    ...
    it('should return unhealthy when basic check fails', async () => {
      const proxy = {
        __processId: 'test-process',
        __destroy: vi.fn(),
        __getMetrics: vi.fn().mockRejectedValue(new Error('Connection failed')),
      } as any;
  ...
  describe('Health Change Events', () => {
    it('should emit health:change on status transition', async () => {
      ...
      expect(handler).toHaveBeenCalledWith('proc-1', expect.objectContaining({ status: 'unhealthy' }));
    });
  ...
  describe('Error Handling', () => {
    it('should handle health check errors gracefully', async () => {
      const proxy = createMockProxy();
      (proxy.__getHealth as vi.Mock).mockRejectedValue(new Error('Network error'));
    ...
    it('should record error health status', async () => {
      const proxy = createMockProxy();
      (proxy.__getHealth as vi.Mock).mockRejectedValue(new Error('Check failed'));
