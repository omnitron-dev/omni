 ...
 */
// Vitest types workaround for build
...
export class TestContainer extends Container {
  ...
  spy<T>(token: InjectionToken<T>, method: keyof T): any {
    ...
    const spy = vi.spyOn(instance as any, method as string);
    ...
    for (const mock of this.mocks.values()) {
      if (mock && typeof mock === 'object') {
        for (const key in mock) {
          ...
          if (value && vi.isMockFunction && vi.isMockFunction(value)) {
            value.mockReset();
          }
        }
      }
      ...
      for (const [method, spy] of spyMap) {
        ...
        for (const expectation of expectations) {
          if (!expectation.verify(spy)) {
            const tokenName = typeof token === 'string' ? token : String(token);
            violations.push(`Expectation failed for ${tokenName}.${method as string}: ${expectation.getMessage()}`);
          }
        }
      }
    }
  ...
  partialStub<T>(token: InjectionToken<T>, stubs: Partial<T>): this {
    ...
    for (const key in stubs) {
      ...
      if (typeof value === 'function') {
        ...
        stubObject[key] = vi.fn().mockReturnValue(value);
      }
    }
