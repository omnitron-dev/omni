...
describeOrSkip('Rotif - NotificationManager Infrastructure Tests', () => {
  ...
  beforeAll(async () => {
    // Wait for global Redis to be ready
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      // Connection may have been closed, recreate it
      redis = helper.createClient('rotif-comprehensive-test-' + Date.now());
      await redis.flushdb();
    }
  ...
  describe('Initialization and Lifecycle', () => {
    ...
    it('should handle graceful shutdown', async () => {
      const _sub = await manager.subscribe('test.shutdown', async () => {});
      await manager.stopAll();
    ...
    // Note: destroy() test moved to the end of this file to avoid affecting subsequent tests
  });
  ...
  describe('Publishing Messages', () => {
    ...
    it('should handle delayed messages', async () => {
      ...
      const messageId = await manager.publish(
        'test.delay',
        { data: 'test' },
        {
          delayMs: 1000,
    ...
    it('should handle exact-once publishing', async () => {
      ...
      const result1 = await manager.publish(
        'test.exactonce',
        { id: 'unique123' },
        {
          exactlyOnce: true,
      ...
      const result2 = await manager.publish(
        'test.exactonce',
        { id: 'unique123' },
        {
          exactlyOnce: true,
  ...
  describe('Subscription Management', () => {
    ...
    it('should support wildcard patterns', async () => {
      ...
      await delay(100); // Allow subscription to be ready
      await manager.publish('test.one', { data: 'one' });
      await manager.publish('test.two', { data: 'two' });
      await delay(500); // Increased delay for message processing
  ...
  describe('Message Processing', () => {
    it('should process messages in order', async () => {
      ...
      await delay(100); // Let subscription be ready
  ...
  describe('Retry Logic', () => {
    it('should retry failed messages', async () => {
      ...
      await delay(1000); // Increased delay to allow retries
    ...
    it('should move to DLQ after max retries', async () => {
      ...
      await delay(1000); // Increased delay to allow retries and DLQ move
    ...
    it('should use custom retry strategies', async () => {
      ...
      await delay(5000); // Increased delay to allow multiple retries with exponential backoff
  ...
  describe('DLQ Management', () => {
    it('should subscribe to DLQ', async () => {
      ...
      await delay(1500); // Increased delay to allow retries and DLQ subscription processing

      expect(dlqMessages.length).toBeGreaterThan(0);
    }, 5000); // Increased test timeout
  ...
  describe('Performance and Scalability', () => {
    it('should handle high message throughput', async () => {
      ...
      await delay(5000); // Increased delay to allow message processing
  ...
  describe('Middleware System', () => {
    ...
    it('should execute afterProcess middleware', async () => {
      ...
      await manager.subscribe('test.afterprocess', async (msg) => {
        // Message will be auto-acked by framework after successful processing
      });

      await delay(100); // Let subscription be ready

      await manager.publish('test.afterprocess', { data: 'test' });
      await delay(1000); // Increased delay to ensure middleware execution
  ...
  // This test must run last as it calls destroy() which closes Redis connections
  describe('Destroy Cleanup (must run last)', () => {
