...
export abstract class BaseConnection extends EventEmitter implements ITransportConnection {
  public readonly id: string = generateUuidV7();
  ...
  protected handleData(data: Buffer | ArrayBuffer): void {
    ...
    // Fast path: if we're in binary mode, skip text detection heuristic
    if (this.binaryMode) {
      ...
      try {
        ...
        // Not a valid packet, emit as raw data (unlikely in binary mode)
        this.emit('data', data);
      }
      return;
    }
  ...
  /**
   * Reset binary mode flag (for testing or reconnection)
   */
  protected resetBinaryMode(): void {
  ...
  protected handleDisconnect(reason?: string): void {
    ...
    // Reset binary mode on disconnect (will re-detect on next connection)
