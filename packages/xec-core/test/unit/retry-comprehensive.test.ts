...
import { RetryError } from '../../src/utils/retry-adapter.js';
...
describe('Comprehensive Retry Functionality Tests', () => {
  ...
  describe('Retry callbacks', () => {
    test('should call onRetry with ExecutionResult', async () => {
      let attempts = 0;
      const retryCallbacks: Array<{ attempt: number; result: any; delay: number }> = [];
      ...
      expect(retryCallbacks[1]?.delay).toBe(200); // 100 * 2^1
    });
  });
