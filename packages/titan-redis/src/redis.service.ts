...
export class RedisService {
   ...
   */
  getOrThrow(namespace?: string): IRedisClient {
   ...
   */
  getOrNil(namespace?: string): IRedisClient | null {
    try {
   ...
   */
  createSubscriber(namespace?: string): IRedisClient {
    const client = this.manager.getInternalClient(namespace);

    if (isCluster(client)) {
      return client.duplicate() as unknown as IRedisClient;
    }

    const redis = client as Redis;
    return redis.duplicate() as unknown as IRedisClient;
  }
   ...
   */
  multi(namespace?: string): IRedisPipeline {
    const client = this.getClient(namespace);
   ...
   * @returns The result of the script execution
   */
  async eval<T = unknown>(script: string, numkeys: number, ...args: (string | number)[]): Promise<T> {
    const client = this.getClient();
    return client.eval(script, numkeys, ...args) as Promise<T>;
  }
   ...
   * @returns The result of the script execution
   */
  async evalsha<T = unknown>(sha: string, numkeys: number, ...args: (string | number)[]): Promise<T> {
    const client = this.getClient();
    return client.evalsha(sha, numkeys, ...args) as Promise<T>;
  }
  ...
  async xgroup(
    ...
    const client = this.manager.getInternalClient(namespace) as Redis;
    // Use type assertion to handle ioredis strict overload types
    // ioredis has separate overloads for each command type, so we use a wider type here
    type XGroupMethod = (
      command: string,
      key: string,
      group: string,
    ...
    if (mkstream) {
      return (client.xgroup as XGroupMethod)(command, key, group, id, mkstream);
    }
    return (client.xgroup as XGroupMethod)(command, key, group, id);
  }
  ...
  async xreadgroup(
    ...
    // Use type assertion to handle ioredis strict overload types for dynamic arguments
    type XReadGroupMethod = (
      ...args: (string | number)[]
    ) => Promise<Array<[string, Array<[string, string[]]>]> | null>;
    const result = await (client.xreadgroup as XReadGroupMethod)(...xreadArgs);
  ...
  async xread(
    ...
    // Use type assertion to handle ioredis strict overload types for dynamic arguments
    type XReadMethod = (...args: (string | number)[]) => Promise<Array<[string, Array<[string, string[]]>]> | null>;
    const result = await (client.xread as XReadMethod)(...args);
   ...
   */
  async subscribeClient(client: IRedisClient, channels: string | string[]): Promise<void> {
    const channelArray = Array.isArray(channels) ? channels : [channels];
    await (client as unknown as Redis).subscribe(...channelArray);
  }
   ...
   */
  async unsubscribeClient(client: IRedisClient, channels: string | string[]): Promise<void> {
    const channelArray = Array.isArray(channels) ? channels : [channels];
    await (client as unknown as Redis).unsubscribe(...channelArray);
  }
   ...
   * @returns Promise that resolves when subscribed
   */
  async psubscribeClient(client: IRedisClient, patterns: string | string[]): Promise<void> {
    const patternArray = Array.isArray(patterns) ? patterns : [patterns];
    await (client as unknown as Redis).psubscribe(...patternArray);
  }
   ...
   * @returns Promise that resolves when unsubscribed
   */
  async punsubscribeClient(client: IRedisClient, patterns: string | string[]): Promise<void> {
    const patternArray = Array.isArray(patterns) ? patterns : [patterns];
    await (client as unknown as Redis).punsubscribe(...patternArray);
  }
