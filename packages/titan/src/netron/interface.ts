...
import { StreamReference, isNetronStream } from './streams/index.js';
import { isNetronService } from './service-utils.js';
// Use type-only import to break circular dependency
import type { IPeer } from './interfaces/core-types.js';
import type { NetronReadableStream, NetronWritableStream } from './streams/index.js';
...
export class Interface {
   ...
   */
  private $methodCache = new Map<string, (...args: unknown[]) => Promise<unknown>>();
  ...
  constructor(
    ...
    return new Proxy(this, {
       ...
       * @returns {unknown} The property value or a function for method calls
      ...
      get: (target: Interface, prop: string) => {
        ...
        if (target.$def.meta.methods[prop]) {
          // Return cached method proxy or create and cache a new one
          let cachedMethod = target.$methodCache.get(prop);
          if (!cachedMethod) {
            // Capture definition ID when method proxy is created to avoid non-null assertion in async closure
            const defId = target.$def.id;
            cachedMethod = async function methodProxy(...args: unknown[]) {
              const processedArgs = target.$processArgs(args);
              if (!target.$peer) {
                throw Errors.badRequest('Invalid interface: Peer is missing');
              }
              if (!target.$def) {
                throw Errors.badRequest('Invalid interface: Service definition was unset');
              }
              return target.$peer.call(defId, prop, processedArgs);
            };
        ...
        if (target.$def.meta.properties[prop]) {
          if (!target.$peer) {
            throw Errors.badRequest('Invalid interface: Peer is missing');
          }
          return target.$peer.get(target.$def.id, prop);
        }
       ...
       * @param {unknown} value - The value being assigned
       ...
       */
      set: (target: Interface, prop: string, value: unknown) => {
        ...
        if (target.$def.meta.properties[prop]?.readonly) {
        ...
        target.$pendingPromises.set(prop, promise);

        // Capture definition ID before async closure to avoid non-null assertion
        const defId = target.$def.id;
        (async () => {
          try {
            const processedValue = await target.$processValue(value);
            if (!target.$peer) {
              throw Errors.badRequest('Invalid interface: Peer is missing');
            }
            if (!target.$def) {
            ...
            await target.$peer.set(defId, prop, processedValue);
            resolvePromise();
            target.$pendingPromises.delete(prop);
          } catch (error) {
            rejectPromise(error);
          }
        })();
  ...
  private $processValue(value: unknown): unknown {
    if (value instanceof Interface) {
      ...
      if (existingStub) {
        ...
        // Use type assertion for internal stub structure
        return new Reference((existingStub as { definition: { id: string } }).definition.id);
      }
      ...
      try {
        const metadata = Reflect.getMetadata('service:metadata', (value as object).constructor);
        if (!metadata) {
        ...
        throw Errors.badRequest(`Failed to expose service: ${message}`);
      }
    } else if (isNetronStream(value)) {
      // Type assertion is safe here - isNetronStream is a type guard
      return StreamReference.from(value as NetronReadableStream | NetronWritableStream);
    }
    return value;
  }
   ...
   *
   * @private
   * @param {unknown[]} args - The array of arguments to process
   * @returns {unknown[]} The array of processed arguments
   */
