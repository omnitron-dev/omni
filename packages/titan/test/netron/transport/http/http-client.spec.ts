/**
 * Tests for HTTP Connection implementation (v1.0 Native Protocol)
...
describe('HttpConnection (v1.0 Native Protocol)', () => {
  ...
  describe('Connection Lifecycle', () => {
    ...
    it('should emit disconnect event on close', () =>
      new Promise<void>((done) => {
        connection = new HttpConnection(baseUrl);
  ...
  describe('State Transitions', () => {
    it('should be CONNECTED and emit connect event', () =>
      new Promise<void>((done) => {
        connection = new HttpConnection(baseUrl);
  ...
  describe('queryInterface Method', () => {
    it('should return proxy object with $def property', async () => {
      connection = new HttpConnection(baseUrl);
      await new Promise((resolve) => connection.once('connect', resolve));

      // queryInterface creates a minimal definition for unknown services
    ...
    it('should return proxy that can call methods', async () => {
      connection = new HttpConnection(baseUrl);
      await new Promise((resolve) => connection.once('connect', resolve));

      // Mock fetch for the method call
      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        headers: { get: vi.fn() },
      ...
      expect(typeof service.someMethod).toBe('function');
    });
  });
  ...
  describe('ping Method', () => {
    it('should measure round-trip time', async () => {
      ...
      // ping() calls GET /discover endpoint
      mockFetch.mockResolvedValue({
        ok: true,
        status: 200,
        headers: { get: vi.fn() },
      ...
      expect(mockFetch).toHaveBeenCalledWith(`${baseUrl}/discover`, expect.objectContaining({ method: 'GET' }));
    }, 5000);
    ...
    it('should handle ping failure', async () => {
      ...
      // ping() calls GET /discover endpoint - mock failure response
      mockFetch.mockResolvedValue({
        ok: false,
        status: 500,
    ...
    it('should handle ping network error', async () => {
      ...
      // ping() calls GET /discover endpoint - mock network error
