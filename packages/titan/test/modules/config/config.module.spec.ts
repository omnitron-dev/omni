...
import { registerModuleProviders } from '@omnitron-dev/testing/titan';
...
describe('ConfigModule', () => {
  ...
  describe('Module lifecycle', () => {
    it('should initialize on module start', async () => {
      ...
      const mockApp = {
        get: vi.fn(),
        resolve: vi.fn().mockReturnValue(configService),
        on: vi.fn(),
      };
  ...
  describe('Error handling', () => {
    it('should handle initialization errors gracefully', async () => {
      ...
      // Mock onInit to throw an error
      const originalOnInit = configService.onInit.bind(configService);
      configService.onInit = vi.fn().mockRejectedValue(new Error('Initialization failed'));

      // Now onInit should throw
      await expect(configService.onInit()).rejects.toThrow('Initialization failed');

      // Restore original
      configService.onInit = originalOnInit;
    });
  });
});
