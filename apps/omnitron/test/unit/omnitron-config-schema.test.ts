 ...
 * keys behind: `infrastructure` and `services.pricing`.
 *
...
describe('omnitron app config schema', () => {
  ...
  it('covers the cross-app service flags the daemon acts on', () => {
    // `services.pricing` was the one that was missing, and the daemon
    ...
    expect(Object.keys(services!).sort()).toEqual(['discovery', 'notifications', 'pricing']);
  });
});
