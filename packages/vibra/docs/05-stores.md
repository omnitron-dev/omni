...
```javascript
import { store, effect } from '@omnitron-dev/vibra';
...
```javascript
import { store } from '@omnitron-dev/vibra';
...
Stores are perfect for application state, forms, and complex data structures. Next, let's explore [Resources](./06-resources.md) to see how Vibrancy handles asynchronous data fetching and management.