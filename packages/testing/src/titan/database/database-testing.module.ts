...
import { DynamicModule, IModule } from '@omnitron-dev/titan/nexus';
...
import { DatabaseManager } from '@omnitron-dev/titan-database';
import { MigrationRunner } from '@omnitron-dev/titan-database';
...
// These tokens were removed from titan's public exports but are still needed
// for optional DI injection in the testing module. Using Symbol.for() ensures
// they match any other code that creates the same symbol.
const DATABASE_MIGRATION_SERVICE = Symbol.for('titan:DATABASE_MIGRATION_SERVICE');
const DATABASE_TRANSACTION_MANAGER = Symbol.for('titan:DATABASE_TRANSACTION_MANAGER');
export const DATABASE_TESTING_SERVICE = Symbol.for('titan:DATABASE_TESTING_SERVICE');
...
export class DatabaseTestingService {
  ...
  constructor(
    ...
    @Inject(DATABASE_TRANSACTION_MANAGER) private transactionManager?: unknown
  ) {}
  ...
  async beforeEach(): Promise<void> {
    const _db = await this.manager.getConnection();
   ...
   */
  private convertBooleansForSqlite(
    db: Kysely<Record<string, unknown>> | Transaction<unknown>,
    data: Record<string, unknown>
  ): Record<string, unknown> {
    const dialect = this.getDialect(db as Kysely<Record<string, unknown>>);
    if (dialect !== 'sqlite') {
      return data;
    }
    ...
    for (const seed of seeds) {
      ...
      try {
        ...
        for (const record of records) {
          const convertedRecord = this.convertBooleansForSqlite(
            db as Kysely<Record<string, unknown>>,
            record as Record<string, unknown>
          );
          await (db as Kysely<Record<string, unknown>>).insertInto(seed.table).values(convertedRecord).execute();
        }
      } catch (error) {
        if (this.options.verbose) {
   ...
   */
  private async getAllTables(db: Kysely<Record<string, unknown>>, dialect: string): Promise<string[]> {
    interface TableRow {
      table_name: string;
    }
  ...
  private getDialect(db: Kysely<Record<string, unknown>>): string {
    // First, try to get dialect from the DatabaseManager (most reliable)
    const managerDialect = this.manager.getDialect();
    if (managerDialect) {
      return managerDialect;
    }

    // Fallback: Try to determine dialect from the Kysely instance structure
    ...
    // If we can't determine the dialect, throw an error instead of defaulting to sqlite
    throw Errors.conflict('Unable to determine database dialect. Ensure DatabaseManager is properly configured.');
  }
   ...
   */
  async getTestConnection(): Promise<Kysely<Record<string, unknown>> | Transaction<unknown>> {
    return (this.currentTransaction || (await this.manager.getConnection())) as
  ...
  async factory<T extends Record<string, unknown>>(
    ...
    const db = await this.getTestConnection();
    const results: T[] = [];

    for (let i = 0; i < count; i++) {
      const data = await generator();
      const convertedData = this.convertBooleansForSqlite(
        db as Kysely<Record<string, unknown>>,
  ...
  async assertDatabaseHas(table: string, data: Record<string, unknown>): Promise<boolean> {
    const db = (await this.getTestConnection()) as Kysely<Record<string, unknown>>;
   ...
   * ```
   */
  async runInTransaction<T>(fn: (trx: Transaction<unknown>) => Promise<T>): Promise<void> {
    const db = await this.manager.getConnection();
   ...
   * ```
   */
  async runWithSavepoints<T>(fn: (trx: Transaction<unknown>) => Promise<T>): Promise<void> {
    const db = await this.manager.getConnection();
   ...
   * ```
   */
  async cleanWithKysera(strategy: CleanupStrategy = 'delete', tables?: string[]): Promise<void> {
    const db = await this.manager.getConnection();
   ...
   * ```
   */
  async seedWithKysera(fn: (trx: Transaction<unknown>) => Promise<void>): Promise<void> {
    const db = await this.manager.getConnection();
   ...
   * ```
   */
  createDataFactory<T extends Record<string, unknown>>(defaults: { [K in keyof T]: T[K] | (() => T[K]) }): (
    overrides?: Partial<T>
  ) => T {
...
export class DatabaseTestingModule {
  ...
  static async createIsolatedModule(options: DatabaseTestingOptions = {}): Promise<{
    ...
    return {
      ...
      set service(instance: DatabaseTestingService | undefined) {
