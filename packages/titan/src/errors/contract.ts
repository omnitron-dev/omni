  ...
  >(
    ...
    if (!result.success) {
      throw Errors.validation([{ field: 'payload', message: result.error.message }], {
        message: `Invalid error payload for contract method ${String(method)}`,
      });
    }
...
export function ContractMethod<TContract extends Contract, TMethod extends keyof TContract['definition']>(
  ...
  return function contractMethodDecorator(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    const originalMethod = descriptor.value;

    descriptor.value = async function contractMethodHandler(...args: any[]) {
      try {
