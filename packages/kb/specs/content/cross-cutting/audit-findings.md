...
| **pricing** | PricingError (not TitanError) | All createToken() | All covered | Mixed @Service | None | **pricing** (unscoped!) |
| **payments** | **TitanError (CORRECT)** | **2 raw Symbol()** | All covered | Correct | None | **payments** (unscoped!) |

**Reference implementation: payments** — best error handling, TransactionAwareRepository, typed interfaces.
...
| pricing | `PricingError extends Error` | Same |
| storage | `StorageError extends Error` | Same |
| **payments** | **`extends TitanError`** | **CORRECT — reference implementation** |

**Fix**: All error base classes must extend `TitanError` from `@omnitron-dev/titan/errors`. Follow payments pattern.
...
### 3. Raw Symbol() instead of Symbol.for() (main + payments)

```
main:   users.service.ts → Symbol('FollowRepository'), Symbol('BlockRepository')
payments: cache.decorator.ts → Symbol('cache:metadata'), Symbol('cache:invalidate')
```
...
### 8. Unscoped package names (pricing, payments)

```json
"name": "pricing"  // Should be @omnitron-dev/pricing
"name": "payments"      // Should be @omnitron-dev/payments
```

### 9. @Service shorthand vs object form (pricing)
...
### 10. require() in ESM context (payments)
...
### 11. `any` types in DTO service interfaces (payments)
...
### Error Handling (follow payments)
```typescript
...
### Service Interface (follow payments)
```typescript
...
1. **CRITICAL**: Fix raw Symbol() (main, payments) — dual-package hazard
2. **CRITICAL**: Fix require() in ESM (payments) — runtime crash
...
6. **HIGH**: Refactor error base classes to extend TitanError (main, storage, messaging, pricing)
7. **HIGH**: Scope package names (pricing, payments)
8. **HIGH**: Replace raw throw new Error() with typed errors (main, storage)
9. **HIGH**: Fix @Service shorthand (pricing)
10. **HIGH**: Fix any types in DTO interfaces (payments)
