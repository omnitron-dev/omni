...
export async function initializeSchema(db: Kysely<any>, type: DatabaseType): Promise<void> {
  ...
  if (type === 'sqlite') {
    ...
    postsTable = postsTable.addForeignKeyConstraint('posts_user_id_fk', ['user_id'], 'users', ['id'], (cb) =>
      cb.onDelete('cascade')
    );
  }
  ...
  if (type === 'sqlite') {
    ...
    await db.schema
      .createTable('post_categories')
      .addColumn('post_id', 'integer', (col) => col.notNull().references('posts.id').onDelete('cascade'))
      .addColumn('category_id', 'integer', (col) => col.notNull().references('categories.id').onDelete('cascade'))
      .addPrimaryKeyConstraint('pk_post_categories', ['post_id', 'category_id'])
      .execute();
  }
  ...
  if (type === 'sqlite') {
    auditTable = auditTable.addColumn('changed_at', 'text', (col) => col.notNull().defaultTo('CURRENT_TIMESTAMP'));
  } else {
  ...
  await db.schema.createIndex('idx_audit_logs_table_name').on('audit_logs').column('table_name').execute();

  await db.schema.createIndex('idx_audit_logs_entity_id').on('audit_logs').column('entity_id').execute();
}
