...
describe('Streaming Integration (WebSocket)', () => {
  ...
  describe('Stream Performance', () => {
    ...
    it('should maintain consistent performance under load', async () => {
      ...
      // For very fast operations (< 5ms avg), allow up to 50ms variance due to JS event loop jitter
      // For slower operations, variance should be within 3x the average
      const allowedVariance = avgDuration < 5 ? 50 : avgDuration * 3;
      expect(maxDuration - minDuration).toBeLessThan(allowedVariance);
    });
  });
