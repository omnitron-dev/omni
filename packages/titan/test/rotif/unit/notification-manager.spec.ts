...
if (skipTests) {
  console.log('⏭️ Skipping notification-manager.spec.ts - requires real Redis');
}
...
describeOrSkip('Rotif - NotificationManager Integration', () => {
  ...
  describe('initialization', () => {
    ...
    it('should respect disableDelayed option', async () => {
      const customManager = new NotificationManager(createTestConfig(7, { disableDelayed: true }));
  ...
  describe('publish', () => {
    ...
    it('should store and deliver message with correct payload', async () => {
      const received: any[] = [];

      // Replace the generic subscription from beforeEach with a tracking one
      await manager.stopAll();
      manager = await createTestNotificationManager(6, {
        checkDelayInterval: 100,
        blockInterval: 100,
    ...
    it('should publish delayed message', async () => {
      const _result = await manager.publish('test.channel', { msg: 'delayed' }, { delayMs: 5000 });
  ...
  describe('subscribe', () => {
    ...
    it('should support custom group names', async () => {
      const sub = await manager.subscribe('test.channel', async () => {}, { groupName: 'custom-group' });
  ...
  describe('pause/resume', () => {
    ...
    it('should resume message processing', async () => {
      ...
      sub.resume();
      await delay(2000); // Allow time for pending message redelivery

      // Message should be processed after resume (from PEL redelivery)
