...
describeOrSkip('Netron Streams Integration Tests', () => {
  ...
  it('should correctly send and receive a simple stream', async () => {
    ...
    // Set up stream listener BEFORE connecting to avoid race condition.
    // Wait for peer connect on server side and register stream listener.
    ...
    // Wait for the server to register the stream listener
    await serverReady;
  ...
  it('should handle error during stream sending after initial chunk and close both streams', async () => {
    ...
    peerB.sendStreamChunk = vi.fn((streamId, chunk, index, isLast, isLive) => {
      if (index === 1) {
  ...
  it('should correctly send and receive a large amount of data', async () => {
    const receivedChunks: string[] = [];
    const chunkCount = 1000; // large number of packets
    ...
    await new Promise((resolve) => writableStream.on('finish', resolve));
    await delay(500); // allow time for data transmission
