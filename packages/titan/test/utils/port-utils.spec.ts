...
describe('Port Utilities', () => {
  ...
  describe('waitForPort', () => {
    ...
    it('should timeout when port does not become available', async () => {
      ...
      await expect(waitForPort(port, '127.0.0.1', 500, 50)).rejects.toThrow(/timed out after|Timeout waiting for port/);
    }, 10000);
    ...
    it('should throw timeout error when time exceeded', async () => {
      ...
      await expect(waitForPort(port, '127.0.0.1', 300, 100)).rejects.toThrow(
        /timed out after|Timeout waiting for port/
      );
    }, 10000);
