...
import { ProcessManager, ProcessSpawnerFactory } from '@omnitron-dev/titan-pm';
import { ProcessStatus } from '@omnitron-dev/titan-pm';
import type { IProcessOptions, IProcessInfo, ServiceProxy, IProcessManagerConfig } from '@omnitron-dev/titan-pm';
...
export class TestProcessManager extends ProcessManager {
  ...
  constructor(config: ITestProcessManagerConfig = {}) {
    ...
    // Register MockProcessSpawner before creating ProcessManager
    // This ensures the factory can use it when useMockSpawner is true
  ...
  private setupTestHandlers(): void {
    this.on('process:crash', (info: IProcessInfo, error: Error) => {
    ...
    this.on('process:ready', (info: IProcessInfo) => {
   ...
   */
  spyOnMethod<T>(proxy: ServiceProxy<T>, methodName: keyof T): any {
