...
if (skipTests) {
  console.log('⏭️ Skipping consumer-loop.spec.ts - requires real Redis');
}
...
describeOrSkip('Rotif - Consumer Loop', () => {
  ...
  describe('consumer group creation', () => {
    ...
    it('should handle existing consumer group', async () => {
      ...
      // Subscribe should not throw even if group already exists
      await expect(manager.subscribe('test.existing', async () => {})).resolves.not.toThrow();
    });
  });
