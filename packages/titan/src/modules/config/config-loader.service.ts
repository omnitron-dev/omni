...
export class ConfigLoaderService implements IConfigLoader {
  ...
  private loadEnvironment(source: IEnvironmentConfigSource): Record<string, any> {
    ...
    const keyTransform = typeof source.transform === 'string' ? source.transform : undefined;
    ...
    for (const [key, value] of Object.entries(process.env)) {
      if (!prefix || key.startsWith(prefix)) {
        ...
        // Split by separator, lowercase each segment, then apply key transform (e.g. camelCase)
        const configPath = configKey
          .split(new RegExp(separator))
          .map((segment) => this.transformKey(segment.toLowerCase(), keyTransform))
          .join('.');
