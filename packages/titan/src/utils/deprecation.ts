...
export function Deprecated(
  ...
  return function deprecatedDecorator<T>(
    ...
    if (propertyKey !== undefined && descriptor?.value !== undefined) {
      ...
      descriptor.value = function deprecatedMethodHandler(this: any, ...args: any[]): any {
        if (!hasWarned) {
          hasWarned = true;
          const className = this?.constructor?.name || 'Unknown';
          const oldName = `${className}.${methodName}`;
          const newName = replacement ? `${className}.${replacement}` : `an alternative method`;
          deprecate(oldName, newName, removeVersion);
        }
    ...
    if (propertyKey !== undefined && descriptor && (descriptor.get || descriptor.set)) {
      ...
      const warn = (instance: any) => {
        if (!hasWarned) {
          ...
          const newName = replacement ? `${className}.${replacement}` : `an alternative property`;
          deprecate(oldName, newName, removeVersion);
        }
      };
      ...
      if (descriptor.get) {
        ...
        descriptor.get = function deprecatedGetter(this: any) {
          warn(this);
      ...
      if (descriptor.set) {
        ...
        descriptor.set = function deprecatedSetter(this: any, value: any) {
          warn(this);
    ...
    if (typeof target === 'function') {
      ...
      for (const key of Object.getOwnPropertyNames(OriginalClass)) {
        if (key !== 'length' && key !== 'name' && key !== 'prototype') {
          const propertyDescriptor = Object.getOwnPropertyDescriptor(OriginalClass, key);
          if (propertyDescriptor) {
            Object.defineProperty(DeprecatedClass, key, propertyDescriptor);
          }
        }
      }
...
export const DEPRECATIONS: DeprecationEntry[] = [
  ...
  },
  {
    ...
    removeIn: '1.0.0',
    notes:
      'The @Method decorator has been renamed to @Public for clarity, ' +
      'as it works for both methods and properties. ' +
  ...
  },
  {
    ...
    removeIn: '1.0.0',
    notes:
      'Interface naming convention changed. The "I" prefix has been dropped. ' + 'Import from "@omnitron-dev/titan".',
  },
  {
    old: 'IOnDestroy',
    new: 'OnDestroy',
  ...
  },
  {
    ...
    removeIn: '1.0.0',
    notes:
      'Renamed for clarity to indicate these are application-level features. ' + 'Import from "@omnitron-dev/titan".',
  },
];
