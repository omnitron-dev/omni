...
import type { INetron, IPeer, EventSubscriber } from './interfaces/core-types.js';
...
export abstract class AbstractPeer implements IPeer {
  ...
  /**
   * Cache for compiled regex patterns used in wildcard matching.
   * Key is the pattern string, value is the compiled RegExp.
   * This avoids recompiling the same pattern on every match check.
   */
  private patternRegexCache = new Map<string, RegExp>();

  /**
   * Maximum number of patterns to cache to prevent unbounded memory growth.
   */
  ...
  protected findLatestServiceVersion(serviceName: string): Definition {
    ...
    // SECURITY: Escape special regex characters in serviceName to prevent ReDoS attacks.
    // Without escaping, a malicious service name like "test.*+?^" could cause
    ...
    const regex = new RegExp(`^${escapedServiceName}@([^@]+)$`);
  ...
  private matchesPattern(serviceName: string, pattern: string): boolean {
    ...
    if (regex) {
      ...
      // Cache the compiled regex (with LRU eviction)
