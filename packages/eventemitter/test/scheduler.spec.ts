...
describe('EventScheduler', () => {
  ...
  let emitFn: Mock<(event: string, data: any) => void | Promise<void>>;

  beforeEach(() => {
    scheduler = new EventScheduler();
    emitFn = vi.fn<(event: string, data: any) => void | Promise<void>>();
    vi.useFakeTimers();
  });
  ...
  describe('getScheduledEvents', () => {
    ...
    it('should update status when executing', async () => {
      const mockEmitFn = vi.fn<(event: string, data: any) => void>();
  ...
  describe('retry logic', () => {
    it('should retry failed executions with linear backoff', async () => {
      ...
      let attempts = 0;
      const failingEmitFn = vi.fn<(event: string, data: any) => Promise<void>>(async () => {
        attempts++;
        if (attempts < 3) {
  ...
  describe('recurring events error handling', () => {
    it('should continue recurring even if individual execution fails', () => {
      let callCount = 0;
      const sometimesFailingFn = vi.fn<(event: string, data: any) => void>(() => {
        callCount++;
        if (callCount === 2) {
