...
export const EmitEvent = createMethodInterceptor<{
...
}>('EmitEvent', async function emitEventInterceptor(this: any, originalMethod, args, context) {
  ...
  try {
    ...
    if (this && typeof this === 'object') {
      ...
      const emitter = (this as any).__eventEmitter__ || (this as any).eventEmitter;
...
export const OnModuleEvent = createDecorator<{
  ...
  .forMethod((context) => {
    ...
    if (filter) {
      context.descriptor!.value = function filteredHandler(...args: any[]) {
        if (filter(args[0])) {
