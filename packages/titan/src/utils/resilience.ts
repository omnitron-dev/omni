...
import { calculateStrategyDelay, addJitter, type RetryStrategyType } from './retry.js';
...
export class CircuitBreaker extends EventEmitter {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    try {
      const result =
        this.config.requestTimeout > 0 ? await this.executeWithTimeout(fn, this.config.requestTimeout) : await fn();
...
export async function withTimeout<T>(
  ...
  const message =
    typeof timeout === 'object' && timeout.errorMessage
...
/**
 * Map BackoffStrategy enum to RetryStrategyType
 */
function mapBackoffToStrategy(backoff: BackoffStrategy): RetryStrategyType {
  switch (backoff) {
...
export async function retry<T>(fn: () => Promise<T>, options: RetryOptions = {}): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      // Calculate delay using unified retry utilities
      const strategy = mapBackoffToStrategy(backoffStrategy);
      let delay = calculateStrategyDelay(strategy, attempt, initialDelay, maxDelay, backoffMultiplier);

      // Add jitter if using ExponentialWithJitter or jitterFactor > 0
      if (backoffStrategy === BackoffStrategy.ExponentialWithJitter || jitterFactor > 0) {
