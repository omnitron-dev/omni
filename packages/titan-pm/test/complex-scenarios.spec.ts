...
import { Process, Public, Workflow, Stage, Supervisor, Child } from '../src/decorators.js';
import { ProcessStatus, SupervisionStrategy } from '../src/types.js';
...
describe('Complex PM Scenarios', () => {
  ...
  describe('Supervisor Complex Scenarios', () => {
    it('should handle supervisor with cascading failures', async () => {
      ...
      const _supervisor = await pm.supervisor(DependentSupervisor);
    ...
    it('should handle supervisor with complex restart patterns', async () => {
      ...
      const _supervisor = await pm.supervisor(TimedSupervisor);
