...
export interface ResolutionState {
  ...
  /** Set of tokens for O(1) circular dependency lookup (performance optimization) */
  chainSet: Set<InjectionToken<any>>;
 ...
 * used (every module in the downstream backends, 130 call sites in this package's
...
export interface IContainer {
   ...
   */
  resolveBatch<T extends Record<string, InjectionToken<unknown>> | InjectionToken<unknown>[]>(
    tokens: T,
   ...
   * Use `loadModuleAsync()` for full eager initialization.
   */
   ...
   */
  loadModuleAsync(module: IModule): Promise<this>;
   ...
   */
  eagerlyInitialize(): Promise<void>;
 ...
 * @since 0.1.0
 */
export function hasStreamOptions<T>(
  provider: ProviderDefinition<T> | StreamProviderOptions<T>
): provider is StreamProviderOptions<T> {
 ...
 * @since 0.1.0
 */
export function isConditionalProviderWithWhen<T>(
  provider: ProviderDefinition<T>
): provider is ConditionalProviderWithWhen<T> {
  return typeof provider === 'object' && provider !== null && 'when' in provider && 'useFactory' in provider;
}
