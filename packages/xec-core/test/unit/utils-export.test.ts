  ...
  describe('Integration', () => {
    ...
    it('should handle retry exhaustion', async () => {
      const testFunction = jest.fn(
        async () => ({ exitCode: 1, stdout: '', stderr: 'Always fails', command: 'test' }) as any
      );
    ...
    it('should test ParallelEngine methods', async () => {
      ...
      const filtered = await parallelEngine.filter(filterItems, (item) =>
        item.includes('fail') ? 'fail' : `echo ${item}`
      );
