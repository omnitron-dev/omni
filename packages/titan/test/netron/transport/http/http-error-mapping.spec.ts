...
describe('HTTP Error Mapping with TitanError', () => {
  ...
  beforeEach(async () => {
    ...
    // Register server with netron so dynamically exposed services are propagated
    netron.transportServers.set('http', server);
    await server.listen();
  });
  ...
  describe('Client-Side Error Reconstruction', () => {
    it('should reconstruct TitanError from HTTP response with context headers', async () => {
      ...
      const _peer = new HttpRemotePeer(connection, netron, baseUrl);
