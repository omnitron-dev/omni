...
import { CONFIG_INJECT_METADATA_KEY, CONFIG_SCHEMA_METADATA_KEY, type AnyZodSchema } from './types.js';
...
export function Config(path?: string, defaultValue?: any): any {
  return function configDecorator(target: any, propertyKey?: string | symbol, parameterIndex?: number) {
 ...
 * ```
 */
export function ConfigSchema<T>(schema: AnyZodSchema) {
  return function configSchemaDecorator(target: any) {
    Reflect.defineMetadata(CONFIG_SCHEMA_METADATA_KEY, schema, target);

    const zodSchema = schema as { safeParse: (v: unknown) => { success: boolean; data?: T; error?: unknown } };

    // Add validation method
    target.prototype.validate = function validateInstance() {
      return zodSchema.safeParse(this);
    };

    // Add static validation method
    target.validate = function validateStatic(data: any) {
...
export function Configuration(prefix?: string) {
  return function configurationDecorator(target: any) {
 ...
 * ```
 */
export function ConfigValidate(schema: AnyZodSchema) {
  return function configValidateDecorator(target: any, propertyKey: string | symbol) {
...
export function ConfigWatch(path: string) {
  return function configWatchDecorator(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
...
export function ConfigDefaults(defaults: Record<string, any>) {
  return function configDefaultsDecorator(target: any) {
...
export function ConfigProvider(name: string) {
  return function configProviderDecorator(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
...
export function ConfigTransform(transformer: (value: any) => any) {
  return function configTransformDecorator(target: any, propertyKey: string | symbol) {
