...
import { createToken, getTokenName } from '@omnitron-dev/titan/nexus';
import {
  Scope,
  IModule,
  Provider,
...
// Vitest mock function type (declared for type checking but may not be used)
  ...
  private async verify(): Promise<void> {
    try {
      ...
      if (this.expectedErrorType && !(error instanceof this.expectedErrorType)) {
        throw new Error(`Expected ${this.expectedErrorType.name}, got ${(error as Error).constructor.name}`, { cause: error });
      }

      if (this.expectedMessage && !(error as Error).message.includes(this.expectedMessage)) {
        throw new Error(`Expected error message to contain "${this.expectedMessage}"`, { cause: error });
      }
    }
  }
}
  ...
  private verify(): void {
    ...
    if (this.expectedToken) {
      try {
        ...
        throw new Error(`Dependency ${String(this.expectedToken)} not found`, { cause: _error });
      }
    }
  }
}
...
export async function assertThrows<E extends Error>(
  ...
  try {
    ...
    if (errorType && !(error instanceof errorType)) {
      throw new Error(`Expected error of type ${errorType.name}, got ${(error as Error).constructor.name}`, { cause: error });
    }
    ...
    if (message) {
      ...
      if (typeof message === 'string') {
        if (!errorMessage.includes(message)) {
          throw new Error(`Expected error message to include "${message}", got "${errorMessage}"`, { cause: error });
        }
      } else if (message instanceof RegExp) {
        if (!message.test(errorMessage)) {
          throw new Error(`Expected error message to match ${message}, got "${errorMessage}"`, { cause: error });
        }
      }
    }
...
export function verifyContainerState(
  ...
  const _metadata = container.getTestMetadata();
