 ...
 *
 * This module uses the canonical ValidationError and ServiceError from
 * `../errors/validation.js`. The error classes exported here are re-exports
 * with backward-compatible interfaces for the validation engine's specific needs.
 *
 * @module validation/validation-engine
 * @see {@link ../errors/validation.js} for the canonical error implementations
 */
 ...
 *
 * This is a lightweight adapter that wraps the canonical ValidationError
 * from `../errors/validation.js` while providing the interface expected
 * by the ValidationEngine (code as string literal, statusCode property).
 *
 * @see {@link ../errors/validation.js#ValidationError} for the canonical implementation
 */
export class ValidationError extends Error {
  /**
   * Error code as a string literal for validation errors
   */
  public readonly code = 'VALIDATION_ERROR' as const;

  /**
   * HTTP status code for validation errors (422 Unprocessable Entity)
   */
  public readonly statusCode = 422;

  /**
   * The underlying Zod error containing detailed validation issues
   */
  public readonly zodError: z.ZodError;

  /**
   * Validation options used when this error was created
   */
  public readonly options?: ValidationOptions;

  constructor(zodError: z.ZodError, options?: ValidationOptions) {
   ...
   *
   * @returns JSON representation suitable for API responses
   */
  toJSON(): {
    code: string;
    message: string;
    errors: Array<{ path: string; message: string; code: string; expected?: string; received?: string } | string>;
  } {
  ...
  /**
   * Convert to the canonical ValidationError from errors/validation.js.
   *
   * Use this when you need the rich interface of the canonical error
   * (e.g., hasFieldError, getFieldErrors, etc.).
   *
   * @returns Canonical ValidationError instance
   */
  toCanonical(): CanonicalValidationError {
    return CanonicalValidationError.fromZodError(this.zodError);
  }

  /**
   * Create a ValidationError from a canonical ValidationError.
   *
   * @param canonical - The canonical ValidationError to convert
   * @param options - Optional validation options
   * @returns ValidationError instance for use with ValidationEngine
   */
  static fromCanonical(canonical: CanonicalValidationError, options?: ValidationOptions): ValidationError {
    if (canonical.zodError) {
      return new ValidationError(canonical.zodError, options);
    }
    // If no zodError, create one from the validation errors
    // Use type assertion because Zod v4's internal ZodIssue type requires
    // additional properties that we don't have access to from canonical errors
    const issues = canonical.validationErrors.map((err) => ({
      code: err.code,
      path: err.path.split('.').filter(Boolean),
      message: err.message,
    })) as z.ZodIssue[];
    return new ValidationError(new z.ZodError(issues), options);
  }
}
 ...
 *
 * This class provides a simplified interface for service errors that is
 * compatible with the validation engine's needs. For richer functionality
 * including validation support, use the canonical ServiceError from
 * `../errors/validation.js`.
 *
 * @see {@link ../errors/validation.js#ServiceError} for the canonical implementation
 */
export class ServiceError<TCode extends number = number> extends Error {
  /**
   * HTTP status code for this service error
   */
  ...
  constructor(statusCode: TCode, data: Record<string, unknown>) {
  ...
  /**
   * Convert to the canonical ServiceError from errors/validation.js.
   *
   * Use this when you need the rich interface of the canonical error
   * (e.g., withValidation static method).
   *
   * @returns Canonical ServiceError instance
   */
  toCanonical(): CanonicalServiceError<TCode> {
    return new CanonicalServiceError(this.statusCode, this.data);
  }
}
 ...
 * High-performance validation engine with caching.
 *
 * This engine compiles Zod schemas into optimized validators that can be
 * reused across multiple validations. It supports various optimization
 * modes including coercion, stripping unknown properties, and strict
 * validation.
 *
 * @example
 * ```typescript
 ...
 *
 * // Async validation
 * const asyncResult = await validator.validateAsync({ name: 'John', age: 30 });
 *
 * // Type guard
...
export class ValidationEngine {
  ...
  private applyCoercionRecursive(
    ...
    if (typeName === 'number' || constructorName === 'ZodNumber') {
      ...
      // Outside arrays: use transform + pipe pattern (Zod 4 preferred over deprecated preprocess)
