...
const getPathParameters = (path: string, zodObject?: unknown) => {
  ...
  const paramsFromPath = path
    ...
    .filter((param) => zodShape[param] === undefined);
...
const extractReferenceSchemas = (
  ...
  if (schema.properties) {
    ...
    }>((prev, [propertyName, schema_]) => {
      prev[propertyName] = extractReferenceSchemas(schema_, referenceSchemas);
      return prev;
    }, {});
  }
