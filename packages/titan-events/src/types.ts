...
/**
 * Backpressure strategy for bounded message queues
 */
export type BackpressureStrategy = 'DROP_OLDEST' | 'DROP_NEWEST' | 'BLOCK' | 'REJECT';

/**
 * Configuration for bounded message queues with backpressure handling
 */
export interface IMessageQueueConfig {
  /**
   * Maximum number of messages per queue (per target)
   * @default 10000
   */
  maxSize?: number;
   ...
   * - DROP_OLDEST: Remove oldest message to make room for new one
   * - DROP_NEWEST: Discard the incoming message
   * - BLOCK: Wait until space is available (with timeout)
   * - REJECT: Throw an error when queue is full
   * @default 'DROP_OLDEST'
   */
  strategy?: BackpressureStrategy;

  /**
   * Timeout in milliseconds for BLOCK strategy
  ...
  /**
   * Threshold percentage (0-1) for capacity warnings
   * Logs a warning when queue reaches this percentage of maxSize
   * @default 0.8 (80%)
   */
...
/**
 * Metrics for message queue operations
 */
export interface IMessageQueueMetrics {
  ...
  /**
   * Number of messages dropped per target
   */
  droppedByTarget: Map<string, number>;

  /**
   * Number of messages rejected (REJECT strategy)
   */
  rejectedMessages: number;

  /**
   * Number of block timeouts (BLOCK strategy)
   */
  blockTimeouts: number;

  /**
   * High watermark - maximum queue size reached
   */
...
/**
 * Result of adding a message to the queue
 */
export interface IQueueAddResult {
  /**
  ...
  /**
   * If dropped, the reason for dropping
   */
  dropReason?: 'queue_full_drop_newest' | 'block_timeout' | 'rejected';

  /**
   * ID of the dropped message (if DROP_OLDEST was used)
   */
