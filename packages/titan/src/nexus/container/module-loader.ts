...
export class ModuleLoaderService {
  ...
  private tokenModuleIndex = new Map<string, { moduleName: string; isGlobal: boolean; isExported: boolean }>();
  // Cache for getTokenKey results to avoid repeated computation
  private tokenKeyCache = new WeakMap<object, string>();
  // Cache for module export keys - enables O(1) export checking instead of O(n)
  private moduleExportKeyCache = new WeakMap<IModule, Set<string>>();
   ...
   * Uses Set-based caching for O(1) lookup instead of O(n) iteration
   */
  isTokenExported(module: IModule, token: InjectionToken<any>): boolean {
    ...
    // Get target key once
    const targetKey = this.getTokenKey(token);

    // Check if we have a cached Set of export keys
    let exportKeySet = this.moduleExportKeyCache.get(module);
    if (!exportKeySet) {
      // Build Set of export keys on first access
      exportKeySet = new Set(module.exports.map((t) => this.getTokenKey(t)));
      this.moduleExportKeyCache.set(module, exportKeySet);
    }

    // O(1) lookup instead of O(n) iteration
    return exportKeySet.has(targetKey);
  }

  /**
   * Invalidate export cache for a module (call if module.exports changes)
   */
  invalidateExportCache(module: IModule): void {
    this.moduleExportKeyCache.delete(module);
  }
}
