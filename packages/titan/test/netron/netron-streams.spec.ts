...
describe('Netron Streams', () => {
  ...
  describe('NetronReadableStream', () => {
    it('should correctly handle ordered packets', () =>
    ...
    it('should handle inactivity timeout correctly', () =>
      new Promise<void>((done) => {
    ...
    it('should close the stream after receiving the last packet', () =>
      new Promise<void>((done) => {
    ...
    it('should handle destroy with error', () =>
      new Promise<void>((done) => {
        ...
        const errorSpy = vi.spyOn(peer.logger, 'error').mockImplementation(() => {});
  ...
  describe('NetronWritableStream', () => {
    ...
    beforeEach(() => {
      ...
      mockPeer.sendStreamChunk = vi.fn<typeof mockPeer.sendStreamChunk>().mockResolvedValue(undefined);
    });
    ...
    it('should handle sendStreamChunk errors correctly', async () => {
      ...
      mockPeer.sendStreamChunk = vi.fn<typeof mockPeer.sendStreamChunk>().mockRejectedValue(error);
      mockPeer.sendPacket = vi.fn<typeof mockPeer.sendPacket>().mockResolvedValue();
    ...
    it('should handle destroy with error', async () => {
      ...
      const infoSpy = vi.spyOn(mockPeer.logger, 'info').mockImplementation(() => {});
  ...
  describe('Stream Integration', () => {
    it('should handle bidirectional streaming', async () => {
      ...
      peer1.sendStreamChunk = vi.fn<typeof peer1.sendStreamChunk>(async (streamId, chunk, index, isLast) => {
        stream1to2Data.push({ streamId, chunk, index, isLast });
      });

      peer2.sendStreamChunk = vi.fn<typeof peer2.sendStreamChunk>(async (streamId, chunk, index, isLast) => {
