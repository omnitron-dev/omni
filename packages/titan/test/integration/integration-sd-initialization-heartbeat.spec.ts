...
if (skipTests) {
  console.log('⏭️ Skipping integration-sd-initialization-heartbeat.spec.ts - requires real Redis');
}
...
describeOrSkip('ServiceDiscovery Integration - Initialization & Heartbeat', () => {
  ...
  it('should not register node in client mode', async () => {
    const clientNetron = await Netron.create(createTestLogger(), {
      discoveryEnabled: true,
      discoveryRedisUrl: getTestRedisUrl(2),
    });
  ...
  it('should allow client to discover server services', async () => {
    ...
    const serverNetron = await Netron.create(createTestLogger(), {
      listenHost: 'localhost',
      listenPort: 4001,
      discoveryEnabled: true,
      discoveryRedisUrl: getTestRedisUrl(2),
