...
const LIVE_ERROR_DAY = [
  { app: 'acme/dev/payments', message: 'webhook-delivery-worker: tick failed', count: 4203 },
  { app: 'acme/dev/pricing', message: 'Aggregation error', count: 2544 },
  { app: 'acme/dev/messaging', message: 'All 3 heartbeat attempts failed', count: 1691 },
  { app: 'acme/dev/messaging', message: 'Scheduled messages processing failed', count: 848 },
  { app: 'acme/dev/payments', message: 'Confirmation check failed in poll cycle', count: 566 },
  { app: 'acme/dev/payments', message: 'Withdrawal reorg sweep failed in poll cycle', count: 566 },
  { app: 'acme/dev/payments', message: 'BTC scan failed in poll cycle', count: 566 },
  { app: 'acme/dev/payments', message: 'XMR credited-reorg sweep failed in poll cycle', count: 566 },
  ...
  { app: 'acme/dev/main', message: 'one-off failure 44', count: 24 },
];
...
describe('findDuplicatedLogs', () => {
  it('reports lines stored under two app names', () => {
    const found = findDuplicatedLogs(
      [{ app: 'acme/dev/pricing', otherApp: 'omnitron', count: 40 }],
      104
    );
  ...
  it('ignores the coincidences that remain after the defect is fixed', () => {
    ...
    // to ignore it.
    expect(findDuplicatedLogs([{ app: 'acme/dev/main', otherApp: 'acme/dev/payments', count: 18 }], 1260)).toEqual([]);
  });

  it('still reports duplication at the scale the defect produced', () => {
    // The systematic case was around half the table.
    const found = findDuplicatedLogs([{ app: 'acme/dev/pricing', otherApp: 'omnitron', count: 500 }], 1000);
    expect(found).toHaveLength(1);
