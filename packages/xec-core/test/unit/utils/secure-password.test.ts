...
describe('SecurePasswordHandler', () => {
  ...
  describe('maskPassword', () => {
    test('should mask password in command string', () => {
      const masked = SecurePasswordHandler.maskPassword('echo mypassword | sudo -S command', 'mypassword');

      expect(masked).toBe('echo ***MASKED*** | sudo -S command');
    });

    test('should mask multiple occurrences', () => {
      const masked = SecurePasswordHandler.maskPassword('echo secret && echo secret again', 'secret');
    ...
    test('should escape regex special characters', () => {
      const masked = SecurePasswordHandler.maskPassword('echo pass.word$test | command', 'pass.word$test');
