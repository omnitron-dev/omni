...
function displayPostgresPlan(planData: any, options: ExplainOptions): void {
  ...
  function displayNode(node: any, indent: number = 0): void {
    ...
    if (node['Sort Key']) {
      ...
      if (node['Sort Method']) {
        console.log(
          `${prefix}  ${prism.magenta('Sort Method: ' + node['Sort Method'] + ' Memory: ' + node['Sort Space Used'] + 'kB')}`
        );
      }
    }
...
function analyzePostgresNode(node: any, issues: string[], suggestions: string[]): void {
  ...
  if (node['Plan Rows'] && node['Actual Rows']) {
    ...
    if (estimateError > 10) {
      issues.push(
        `Large estimation error on ${node['Node Type']} (estimated ${node['Plan Rows']}, actual ${node['Actual Rows']})`
      );
