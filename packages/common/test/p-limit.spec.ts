...
describe('p-limit', () => {
  ...
  describe('Edge cases and compatibility', () => {
    ...
    it('should preserve this context in map', async () => {
      const limit = pLimit(2);
      // Note: p-limit.map doesn't preserve 'this' context, this test verifies basic array mapping
      const _context = { multiplier: 2 };

      const results = await limit.map([1, 2, 3], async function (this: typeof _context, value: number) {
        return value * 2;
      });
