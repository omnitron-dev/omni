...
describe('Icon Management Integration', () => {
          ...
          return {
            querySelector: (selector: string) => {
              if (selector === '#sprite-icon-1') {
                return {
                  outerHTML:
                    '<symbol id="sprite-icon-1" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></symbol>',
  ...
  describe('Icon Caching Strategies', () => {
    ...
    it('should prevent duplicate concurrent requests', async () => {
      ...
      await Promise.all([registry.get('concurrent'), registry.get('concurrent'), registry.get('concurrent')]);
  ...
  describe('Sprite Extraction and Rendering', () => {
          ...
          return {
            querySelector: (selector: string) => {
              const symbols: Record<string, any> = {
                ...
                '#settings': {
                  outerHTML:
                    '<symbol id="settings" viewBox="0 0 24 24"><path d="M19.14,12.94c0.04-0.3,0.06-0.61,0.06-0.94"/></symbol>',
                  getAttribute: () => '0 0 24 24',
                },
              };
    ...
    it('should optimize sprite loading', async () => {
      ...
      await Promise.all([registry.get('icon1'), registry.get('icon2'), registry.get('icon3')]);
