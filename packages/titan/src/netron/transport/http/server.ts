...
export class HttpServer extends EventEmitter implements ITransportServer {
   ...
   * Validate method input against contract.
   *
   * Delegates to the shared helper. This was a forty-line private copy of it,
   * and the copy is what every dispatch path actually called while the shared
   * one sat exported and unused — so a fix applied to either was invisible to
   * the other.
   */
  private validateMethodInput(input: unknown, contract?: MethodContract): unknown {
    return validateInputAgainstContract(input, contract, this.netronPeer?.logger);
  }
