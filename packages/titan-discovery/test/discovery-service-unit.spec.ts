...
if (skipTests) {
  console.log('⏭️ Skipping discovery-service-unit.spec.ts - requires real Redis');
}
...
describeOrSkip('DiscoveryService - Core Unit Tests', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle Redis errors in findNodes', async () => {
      redis.smembers = vi.fn().mockRejectedValue(new Error('Redis error'));
    ...
    it('should handle errors in nodeExists', async () => {
      redis.exists = vi.fn().mockRejectedValue(new Error('Redis error'));
    ...
    it('should handle errors in getNodeInfo', async () => {
      redis.hgetall = vi.fn().mockRejectedValue(new Error('Redis error'));
