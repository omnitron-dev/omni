...
import { WebSocketTransport, WebSocketConnection } from '../../../src/netron/transport/websocket/index.js';
...
describe('WebSocket Advanced Tests', () => {
  ...
  afterEach(async () => {
    ...
    // Reset the shared KeepAliveManager singleton between tests
  ...
  describe('Keep-Alive Ping/Pong Timeout', () => {
    it('should handle keep-alive ping timeout and cleanup', async () => {
      ...
      _serverWs = await connectionPromise;

      // Verify connection is registered with shared KeepAliveManager
      // (The implementation uses a shared timer wheel pattern)
      const keepAliveManager = KeepAliveManager.getInstance({ interval: 100, timeout: 200 });
      expect(keepAliveManager.isRegistered(client as any)).toBe(true);
      ...
      // Verify connection is unregistered from KeepAliveManager
      expect(keepAliveManager.isRegistered(client as any)).toBe(false);
    });
  ...
  describe('Coverage Gap Tests', () => {
    it('should handle connection errors during setup', async () => {
      ...
      (_serverWs as any)._socket.write(Buffer.from([0xff, 0xff, 0xff, 0xff]));
    ...
    it('should cleanup ping interval and pong timeout on disconnect', async () => {
      ...
      // Verify connection is registered with shared KeepAliveManager
      const keepAliveManager = KeepAliveManager.getInstance({ interval: 100, timeout: 50 });
      expect(keepAliveManager.isRegistered(client as any)).toBe(true);
