...
export interface ModuleAsyncOptions<
  TOptions,
  TFactory extends AsyncOptionsFactory<TOptions> = AsyncOptionsFactory<TOptions>,
> {
  /**
...
export function createAsyncOptionsProvider<TOptions>(
  ...
  if (asyncOptions.useFactory) {
    ...
    providers.push([
      optionsToken,
      {
        useFactory: async (...args: any[]) => Promise.resolve(asyncOptions.useFactory!(...args)),
    ...
    providers.push([
      optionsToken,
      {
        useFactory: async (factory: AsyncOptionsFactory<TOptions>) => factory.createOptions(),
 ...
 * ```
 */
export function defineValue<T>(token: InjectionToken<T>, provider: { useValue: T }): ModuleProviderTuple<T> {
  return [token, provider as ProviderDefinition<T>];
}
