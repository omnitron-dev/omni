...
describe('NetronWritableStream', () => {
  ...
  beforeEach(async () => {
    ...
    peer.sendStreamChunk = vi.fn<typeof peer.sendStreamChunk>().mockResolvedValue(undefined);
    peer.sendPacket = vi.fn<typeof peer.sendPacket>().mockResolvedValue();
  });
  ...
  describe('Stream Creation and Initialization', () => {
    ...
    it('should log stream creation', () => {
      ...
      expect(peer.logger.info).toHaveBeenCalledWith({ streamId: 101, isLive: false }, 'Creating writable stream');
    });
  ...
  describe('Write Operations', () => {
    ...
    it('should call callback with error when _write called on closed stream', () =>
      new Promise<void>((done) => {
        ...
        (stream as any)._write('data', 'utf8', (err: Error | null) => {
          ...
          expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 207 }, 'Attempt to write to closed stream');
          done();
        });
      }));
  });
  ...
  describe('Write Error Handling', () => {
    it('should handle sendStreamChunk error', async () => {
      ...
      peer.sendStreamChunk = vi.fn<typeof peer.sendStreamChunk>().mockRejectedValue(sendError);
    ...
    it('should log error when sending error packet fails', async () => {
      const sendError = new Error('Write error');
      peer.sendStreamChunk = vi.fn<typeof peer.sendStreamChunk>().mockRejectedValue(sendError);
      peer.sendPacket = vi.fn<typeof peer.sendPacket>().mockRejectedValue(new Error('Packet send failed'));
  ...
  describe('Final Chunk Handling', () => {
    ...
    it('should handle final chunk error', async () => {
      let callCount = 0;
      peer.sendStreamChunk = vi.fn<typeof peer.sendStreamChunk>().mockImplementation(async () => {
        callCount++;
        if (callCount > 1) {
  ...
  describe('Pipe Operations', () => {
    ...
    it('should handle pipe backpressure', async () => {
      ...
      peer.sendStreamChunk = vi.fn<typeof peer.sendStreamChunk>().mockImplementation(async (...args) => {
        // Simulate slow write
    ...
    it('should handle pipe error', async () => {
      ...
      expect(peer.logger.error).toHaveBeenCalledWith({ streamId: 233, error: pipeError }, 'Pipe operation failed');
    });
    ...
    it('should handle pipe error with non-Error thrown', async () => {
      ...
      const loggedError = vi.spyOn(peer.logger, 'error');
      ...
      expect(loggedError).toHaveBeenCalledWith({ streamId: 235, error: 'string error' }, 'Pipe operation failed');
    });
  ...
  describe('Destroy Operations', () => {
    ...
    it('should log error when sending close packet fails', async () => {
      peer.sendPacket = vi.fn<typeof peer.sendPacket>().mockRejectedValue(new Error('Send failed'));
    ...
    it('should not destroy already closed stream', () => {
      ...
      expect(peer.logger.warn).toHaveBeenCalledWith({ streamId: 255 }, 'Attempt to destroy already closed stream');
    });
  ...
  describe('Backpressure Handling', () => {
    ...
    it('should log backpressure during pipe', async () => {
      ...
      stream.write = function mockWrite(...args: any[]) {
      ...
      // After pipeFrom completes, stream goes through end() → _final() → closeStream()
      // The stream is expected to be finished/destroyed after pipe completion
      expect(peer.sendStreamChunk).toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handler Method', () => {
    it('should have handleError method that cleans up', () => {
      ...
      expect(peer.logger.error).toHaveBeenCalledWith({ streamId: 270, error: testError }, 'Stream error occurred');
