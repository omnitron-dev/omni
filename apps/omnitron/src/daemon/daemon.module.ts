...
export function createDaemonModule(ecosystemConfig: IEcosystemConfig, dc: IDaemonConfig) {
  ...
  @Module({
    imports: [
      ...
      // Auth — JWT verification + caching via titan-auth (same as main/storage/messaging/payments)
    ...
    providers: [
      ...
      ...(!isSlave ? [[
        OMNITRON_DB_TOKEN,
        {
          // Connection parameters (and the ResilientPgClient wiring that
          // closes the mid-handshake 'error'-listener race) come from
          // `database/connection.ts` — the same resolution the migration
          // runner and the migrate CLI use, so `OMNITRON_DATABASE_URL`
          // moves every consumer at once instead of only some of them.
          useFactory: async () => {
            const { createOmnitronDb } = await import('../database/connection.js');
            return createOmnitronDb({ max: 10 });
          },
          scope: Scope.Singleton,
        },
      ] as any] : []),
