...
describe('ConfigValidator', () => {
  ...
  describe('tasks validation', () => {
    ...
    it('should validate timeout format', async () => {
      ...
      const timeoutErrors = errors.filter(
        (e) => e.path.includes('tasks.valid1') || e.path.includes('tasks.valid2') || e.path.includes('tasks.valid3')
      );
    ...
    it('should validate target references', async () => {
      ...
      expect(errors).toContainEqual({
        ...
        message:
          "Target reference 'invalid-target' may not be valid. Expected format: hosts.name, containers.name, pods.name, or local",
        rule: 'warning',
      });
    });
  });
  ...
  describe('profiles validation', () => {
    ...
    it('should detect circular profile inheritance', async () => {
      ...
      const circularErrors = errors.filter((e) => e.message === 'Circular profile inheritance detected');
