...
- [Transport Adapters](#transport-adapters)
  - [BinaryTransportAdapter](#binarytransportadapter)
...
Windows inter-process communication via Named Pipes.
...
**Use Cases:**
- Windows IPC
- Cross-process communication on Windows
- Windows service integration
- Legacy Windows application communication

**Platform Support:**
- ✅ Windows (primary)
- ⚠️ Limited support on Unix-like systems (emulated via Unix sockets)

**Configuration:**
```typescript
const netron = new Netron({
  transport: 'pipe',
  listenPath: 'my-service-pipe',  // Will be converted to \\.\pipe\my-service-pipe on Windows
});
```
...
// Server (Windows)
const server = await transport.createServer({
  name: 'my-service'  // Creates \\.\pipe\my-service
});

// Client (Windows)
const connection = await transport.connect('pipe://my-service');
// or
const connection = await transport.connect('pipe://\\\\.\\pipe\\my-service');
```
...
// Full Windows path
'pipe://\\\\.\\pipe\\my-service'

// Direct path (will be prefixed automatically)
'my-service'
```
...
The `BinaryTransportAdapter` wraps any transport connection to provide a standard WebSocket-like interface, making it compatible with existing code that expects WebSocket objects.

**Key Features:**
- Standard socket interface (readyState, send, close, ping, pong)
- Works with all binary transports (WebSocket, TCP, Unix Socket, Named Pipe)
- Automatic event mapping (connect → open, disconnect → close, etc.)
- Binary type support (nodebuffer, arraybuffer)

**Usage:**
```typescript
import { BinaryTransportAdapter, TransportConnectionFactory } from '@omnitron-dev/titan/netron';

// Create adapter from transport connection
const transport = getTransport('tcp');
const connection = await transport.connect('tcp://localhost:9000');
const adapter = new BinaryTransportAdapter(connection, 'tcp://localhost:9000');

// Use like a WebSocket
adapter.on('open', () => {
  console.log('Connected!');
});

adapter.on('message', (data, isBinary) => {
  console.log('Received:', data);
});

adapter.send(Buffer.from('Hello!'));

// Check connection state (WebSocket compatible)
console.log(adapter.readyState); // 0=CONNECTING, 1=OPEN, 2=CLOSING, 3=CLOSED
```

**Properties:**
```typescript
class BinaryTransportAdapter {
  // WebSocket-compatible constants
  ...
  // Methods
  send(data: Buffer | ArrayBuffer | Uint8Array | string, callback?: (err?: Error) => void): void;
  ...
  on(event: 'message', handler: (data: Buffer | ArrayBuffer, isBinary: boolean) => void): this;
...
Factory for creating transport connections with automatic protocol detection and adapter wrapping.

**Key Features:**
- Automatic transport selection based on URL
- Returns BinaryTransportAdapter for uniform interface
- Supports all registered transports
- Backward compatibility with native WebSocket

**Usage:**
```typescript
import { TransportConnectionFactory } from '@omnitron-dev/titan/netron';

// Connect to any transport via URL
const wsConn = await TransportConnectionFactory.connect('ws://localhost:8080');
const tcpConn = await TransportConnectionFactory.connect('tcp://localhost:9000');
const unixConn = await TransportConnectionFactory.connect('unix:///tmp/service.sock');
const pipeConn = await TransportConnectionFactory.connect('pipe://my-service');

// All return BinaryTransportAdapter with uniform interface
wsConn.on('message', (data) => console.log('WS:', data));
tcpConn.on('message', (data) => console.log('TCP:', data));

// Create adapter from existing connection
const transport = getTransport('tcp');
const rawConnection = await transport.connect('tcp://localhost:9000');
const adapter = TransportConnectionFactory.fromConnection(rawConnection, 'tcp://localhost:9000');
```
...
class TransportConnectionFactory {
  ...
  // Create adapter from existing transport connection
  static fromConnection(connection: ITransportConnection, url?: string): BinaryTransportAdapter;

  // Check if object is native WebSocket
  static isNativeWebSocket(socket: any): socket is WebSocket;

  // Get adapter from WebSocket or ITransportConnection
  static getAdapter(socket: WebSocket | ITransportConnection): BinaryTransportAdapter;
}
```

**Example - Migrating from WebSocket:**
```typescript
// Old code using WebSocket
const ws = new WebSocket('ws://localhost:8080');
ws.on('message', (data) => console.log(data));

// New code using TransportConnectionFactory (works with any transport!)
const conn = await TransportConnectionFactory.connect('tcp://localhost:9000');
...
// Register globally (correct API: register(name, factory))
...
Pre-registered transports (automatically available):

```typescript
// Auto-registered transports:
// - WebSocketTransport (websocket, ws, wss)
// - TcpTransport (tcp)
// - UnixSocketTransport (unix)
// - NamedPipeTransport (pipe) - Windows support

// Note: HttpTransport is NOT auto-registered and must be registered manually:
import { HttpTransport } from '@omnitron-dev/titan/netron';
import { TransportRegistry } from '@omnitron-dev/titan/netron';
...
> **⚠️ NOT IMPLEMENTED**: Connection pooling is not currently implemented in the transport layer. The ConnectionPool class shown in earlier examples does not exist. Connection pooling may be added in a future version.

For now, manage connections manually:

```typescript
// Manual connection management
const connections = new Map<string, ITransportConnection>();

async function getConnection(address: string): Promise<ITransportConnection> {
  let conn = connections.get(address);
...
For high-throughput scenarios, implement your own connection management:

```typescript
// Connection cache for reuse
const connectionCache = new Map<string, ITransportConnection>();

async function getOrCreateConnection(address: string): Promise<ITransportConnection> {
  let conn = connectionCache.get(address);
...
const conn = await getOrCreateConnection('tcp://server:9000');
