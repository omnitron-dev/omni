...
describeOrSkip('HttpTransportClient - Comprehensive Coverage', () => {
  ...
  describe('Timeout Handling', () => {
    ...
    it('should use hint timeout if provided', async () => {
      // Note: For direct HTTP path testing (no peer), create client without Netron
      client = new HttpTransportClient(baseUrl);
      await client.initialize();
    ...
    it('should use default timeout if not specified', async () => {
      ...
      await expect(client.invoke('TestService', 'echo', [{ fast: true }])).resolves.toBeDefined();
    });
  });
  ...
  describe('Connection Management', () => {
    ...
    it('should close peer if present', async () => {
      ...
      const peerCloseSpy = vi.spyOn(peer, 'close');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle abort signal', async () => {
      client = new HttpTransportClient(baseUrl, mockNetron as any, {
        requestTimeout: 10, // Very short timeout to trigger abort
      });
      await client.initialize();
