# HTTP Transport for Netron v1.0
...
- ✅ **Rate Limiting** - Built-in sliding window rate limiter for DoS protection
- ✅ **Memory Protection** - Configurable limits for async generator items
...
- ⚠️ **Streaming** - HTTP transport doesn't support true streaming (async generators are collected into arrays)
  - Server-Sent Events (SSE) not implemented
  - Use WebSocket transport for real-time streaming needs
...
HTTP v1.0 uses native JSON messages for all communication.
...
**Note:** The HTTP server is exported as both `HttpServer` (from the direct file import) and `HttpNativeServer` (from the package index). Both refer to the same implementation.
...
// Option 2: Import directly from server file
// import { HttpServer } from '@omnitron-dev/titan/netron/transport/http/server';

// Using HttpNativeServer (package export)
...
interface TransportOptions {
  ...
  // Memory protection (server-side)
  maxAsyncGeneratorItems?: number;  // Max items from async generators (default: 1000)
}
```
...
The HTTP server includes a built-in sliding window rate limiter to protect against DoS attacks:
...
const server = new HttpNativeServer({
  ...
  rateLimit: {
    ...
    maxRequests: 100,        // Max 100 requests per window (default: 100)
    windowMs: 60000          // 60-second window (default: 60000)
  }
});
...
// X-RateLimit-Reset: 1735679400000
```
...
Configure maximum items collected from async generators to prevent memory exhaustion:

```typescript
const server = new HttpNativeServer({
  port: 3000,
  maxAsyncGeneratorItems: 1000  // Default: 1000 items
});

// If a service method returns an async generator with >1000 items,
// the server will throw an error with suggestions for:
// - Using pagination
// - Streaming over WebSocket
// - Increasing the limit if appropriate
```
...
### TypedContract (Type-Safe Client) - ⚠️ NOT YET IMPLEMENTED

**Note:** TypedContract is planned but not yet implemented in the current version. Use HttpTransportClient with TypeScript interfaces for type safety:

```typescript
// Current approach - Use TypeScript interfaces for type safety
...
const result = await client.invoke<CalculatorOutput>(
  'Calculator@1.0.0',
  'add',
  [{ a: 5, b: 3 } as CalculatorInput]
);

console.log(result.result); // Type-safe!
```
...
### Optimistic Update Manager - ⚠️ NOT YET IMPLEMENTED

**Note:** OptimisticUpdateManager is not exported in the current version. For optimistic updates, implement a custom solution using HttpCacheManager:
...
async function optimisticUpdate<T>(
  ...
  cache.set(key, optimisticValue, { maxAge: 60000 });

  try {
    // Perform actual update
    const result = await updateFn();
    // Update cache with real value
    cache.set(key, result, { maxAge: 60000 });
    return result;
  } catch (error) {
    // Rollback on error
    cache.delete(key);
...
const optimisticUser = { id: userId, name: 'New Name' };
...
- **HttpCacheManager** - Cache configuration with TTL and tag-based invalidation
- **RetryManager** - Retry and circuit breaker configuration
- **RequestBatcher** - Automatic request batching configuration
- **SlidingWindowRateLimiter** - Rate limiting and DoS protection (server-side)
...
HTTP transport uses the `ErrorCode` enum with numeric values that match HTTP status codes:
...
**Important Notes:**
- Error codes are **numeric values** that align with HTTP status codes (e.g., `404`, `500`)
- The `ErrorCode` enum provides named constants for type safety
- Response `error.code` field contains the numeric code (e.g., `404`, not the string `"NOT_FOUND"`)
- Use `TitanError.code` to check error codes programmatically
...
```typescript
// ✅ Good - Type-safe with TypeScript interfaces
interface MyInput {
  value: string;
}
...
const result = await client.invoke<MyOutput>(
  'MyService@1.0.0',
  'myMethod',
  [{ value: 'input' } as MyInput]
);
...
const getData = async (id: string) => {
  return cache.get(
    `item:${id}`,
    async () => client.invoke('Service', 'get', [{ id }]),
    { maxAge: 60000, staleWhileRevalidate: 30000 }
  );
};

// ❌ Avoid - Fetch every time
const result = await client.invoke('Service', 'get', [{ id }]);
...
### From Pre-1.0 (Packet Protocol)

HTTP v1.0 uses native JSON messages instead of binary packet protocol:

**Before (pre-1.0):**
```typescript
// Pre-1.0 used packet serialization
const packet = {
...
HTTP v1.0 does NOT support REST endpoint mapping. Use POST /netron/invoke:
...
**Note:** HttpInterface is exported as `EnhancedHttpInterface` from the package index.

```typescript
import { EnhancedHttpInterface } from '@omnitron-dev/titan/netron/transport/http';

const interface = new EnhancedHttpInterface(transport, definition);
...
### TypedHttpServer (OpenAPI Generation) - ⚠️ NOT YET IMPLEMENTED

**Note:** TypedHttpServer with automatic OpenAPI generation is planned but not yet implemented. Use the standard HttpNativeServer:
...
const server = new HttpNativeServer({ port: 3000 });
server.setPeer(peer);
await server.listen();

// For OpenAPI documentation, consider using external tools
// or manual OpenAPI spec generation based on your service definitions
```
