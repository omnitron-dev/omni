...
3. Automatically starts the application
...
**Note**: The application is automatically started. There is no separate `startOptions` parameter.
...
**Creates service with automatic registration**:
- Creates a service class from the implementation object
- Applies Injectable decorator with the specified scope
- Auto-registers with the global Titan application if it exists
- Sets scope (singleton by default)
- Returns the service instance

**Key Differences from Documentation**:
- `name` is now OPTIONAL and inside the options object (not a required first parameter)
- Uses `scope` instead of `singleton` for more flexibility
- Automatically registers with the application
...
**Note**: Type conversion checks number BEFORE boolean for correct behavior.
...
1. Returns a Proxy object that throws a descriptive error when accessed
2. The actual injection happens at runtime by the DI container
3. This is a placeholder for property injection

**Current Implementation**:
The function returns a Proxy that throws a TitanError when any property is accessed. This ensures that if the dependency is not properly injected, you get a clear error message:
...
**Note**: This is a placeholder implementation. The actual dependency injection is handled by the Nexus DI container through constructor injection or the `@Inject()` decorator. Use constructor injection or `@Inject()` for proper dependency injection instead of calling `inject()` directly.
...
**HTTP controller helper**:
- Creates a controller with automatic route registration
- Normalizes the base path
- Auto-registers with the global Titan application if it exists
- Returns a ControllerRegistration object
...
- `params`: Route parameters extracted from the path
- `headers`: Request headers as key-value pairs
- `body`: Parsed request body (typically JSON)
- `raw`: Native Request object from the HTTP transport
...
**Response methods**:
- `status(code)`: Sets the HTTP status code (chainable)
- `json(data)`: Sends JSON response with Content-Type: application/json
- `text(data)`: Sends text response with Content-Type: text/plain
- `html(data)`: Sends HTML response with Content-Type: text/html
- `header(name, value)`: Sets a single response header (chainable)
- `headers(headers)`: Sets multiple response headers (chainable)
- `send(statusCode, body, contentType?)`: Sends response with custom status, body, and content type

**Important**: Only one response method (json/text/html/send) should be called per request. Calling multiple will throw an error.
...
**Handler function**:
- Takes a `ControllerRequest` and `ControllerResponse` as parameters
- Can be async or sync
- Should call one of the response methods (json/text/html/send)
- Errors are automatically caught and converted to appropriate HTTP responses

#### Integration with Application

Controllers are automatically integrated with the Titan application when created:
1. When `controller()` is called, it registers the controller with the global application
2. The controller handlers are internally converted to Netron services
3. Routes are mapped based on the base path and handler names
4. Request routing follows the pattern: `{basePath}/{handlerName}`

**Internal Functions**:
- `getControllers(app)`: Returns all controllers registered for an application
- `createControllerService(registration)`: Creates a Netron service from a controller registration (bridges the simple controller API with Netron's HTTP transport)
