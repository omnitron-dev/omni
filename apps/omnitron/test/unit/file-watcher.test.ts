...
describe('FileWatcher', () => {
  ...
  it('watches nothing when the entry file is not where the base says', () => {
    ...
    //
    // Every downstream app ended up watching `apps/omnitron`. One edit to
  ...
  it('refuses to restart an app that only shares a name', async () => {
    ...
    // apps called `main`, `storage`, `payments`) point at somebody else's
    // running app. A change under that config's tree then restarts an app it
    // has nothing to do with.
    //
    // Observed: four downstream apps restarted in the same millisecond from one
    // edit to `apps/omnitron/src/commands/doctor.ts`.
    const restartApp = vi.fn().mockResolvedValue(undefined);
    orchestrator = createMockOrchestrator({
      restartApp,
      getAppStatus: () => 'online',
      resolveAppName: () => 'acme/dev/test-app',
      getHandle: () => ({ entry: { name: 'acme/dev/test-app', bootstrap: '/elsewhere/src/bootstrap.ts' } }),
    });
  ...
  it('restarts when the entry points agree', async () => {
    ...
    orchestrator = createMockOrchestrator({
      ...
      getAppStatus: () => 'online',
      resolveAppName: () => 'acme/dev/test-app',
      getHandle: () => ({ entry: { name: 'acme/dev/test-app', script } }),
    });
    ...
    expect(restartApp).toHaveBeenCalledWith('acme/dev/test-app');
  });
