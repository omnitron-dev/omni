...
The PM module supports two spawning modes for testing:

#### MockSpawner (Default in Tests)
Runs **REAL** service logic in the same process - ideal for testing service implementations:

```typescript
// Default behavior in test environment (NODE_ENV=test)
const pm = new ProcessManager(logger);

// Your service class is instantiated and methods are called directly
const service = await pm.spawn(CalculatorService);
const result = await service.add(2, 3); // Real method call!
expect(result).toBe(5);
```

**What MockSpawner tests:**
- ✅ Real service class instantiation
- ✅ Real method execution and return values
- ✅ State management across calls
- ✅ Error handling and propagation
- ✅ Pool load balancing strategies
- ✅ Health check implementations
- ❌ Actual worker thread/process spawning
- ❌ IPC serialization/deserialization
- ❌ Network transport protocols

#### Real Process Spawner
For testing actual worker threads with file-based services:

```typescript
// Explicitly use real spawner
const pm = new ProcessManager(logger, {
  testing: { useMockSpawner: false }
});

// Requires file-based service with default export
const service = await pm.spawn('/path/to/compiled/service.js');
```

**Important:** Real process spawning requires:
1. Compiled JavaScript files (not TypeScript)
2. Services with `export default class`
3. Separate process files

### Testing Best Practices

```typescript
// For unit tests - use MockSpawner (default)
describe('Service Logic', () => {
  let pm: ProcessManager;

  beforeEach(() => {
    pm = new ProcessManager(logger); // MockSpawner in test env
  });

  it('should test real service methods', async () => {
    const service = await pm.spawn(MyService);
    expect(await service.calculate()).toBe(42);
  });
});

// For integration tests with in-process isolation
...
- `allMethodsPublic` - **NEW** Mark all methods as public (no need for @Public())
...
**Zero-Boilerplate Services with `allMethodsPublic`:**

For services where all methods should be accessible via RPC, use `allMethodsPublic` to eliminate @Public() decorators:
...
Access circuit breaker state for monitoring and alerting:

```typescript
const pool = await pm.pool(ExternalService, {
  size: 4,
  circuitBreaker: { enabled: true, threshold: 5, timeout: 60000 }
});

// Get current state
const state = pool.circuitBreakerState;
console.log(state);
// {
...
pool.on('circuitbreaker:open', () => alertOps('Circuit breaker opened!'));
pool.on('circuitbreaker:halfopen', () => log('Testing recovery...'));
pool.on('circuitbreaker:close', () => log('Service recovered'));
```
