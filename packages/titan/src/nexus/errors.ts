 ...
 * ## Error Hierarchy
 *
 * The error system follows this hierarchy:
 *
 * - `Error` (native JavaScript error)
 *   - `NexusError` (base class for all Nexus DI errors)
 *     - `ResolutionError` - dependency cannot be resolved
 *     - `CircularDependencyError` - circular dependency detected
 *     - `RegistrationError` - provider registration failed
 *     - `DuplicateRegistrationError` - token already registered
 *     - `DependencyNotFoundError` - required dependency missing
 *     - `ScopeMismatchError` - scope configuration mismatch
 *     - `InvalidProviderError` - invalid provider definition
 *     - `AsyncResolutionError` - async resolution required
 *     - `DisposalError` - disposal operation failed
 *     - `ModuleError` - module operation failed
 *     - `InitializationError` - service initialization failed
 *     - `ContainerDisposedError` - container already disposed
 *     - `NotInjectableError` - class not decorated as injectable
 *     - `NexusAggregateError` - multiple errors occurred (Nexus-specific)
 *
 * For aggregating `TitanError` instances with HTTP status codes and deduplication,
 * use `AggregateError` from `../errors/core.js`.
 */

import { getTokenName } from './token.js';
import { InjectionToken } from './types.js';
import { buildActionableMessage, DOC_LINKS } from '../errors/formatting.js';
import { createNullLogger, type ILogger } from '../types/logger.js';

// Re-export the canonical AggregateError for TitanError aggregation
export { AggregateError } from '../errors/core.js';
...
export class ResolutionError extends NexusError {
  ...
  private static formatMessage(token: InjectionToken<any>, chain: InjectionToken<any>[], cause?: Error): string {
    ...
    if (cause) {
      ...
      suggestions.push(
        'Register the provider: container.register(' + tokenName + ', { useClass: ' + tokenName + 'Impl })'
      );
    ...
    return buildActionableMessage({
      title:
        ...
        tokenName +
        "'" +
        (cause ? ': Factory or initialization failed' : ': Token is not registered'),
      description: cause
        ? cause.message
        : 'The requested dependency was not found in the container or any parent containers.',
      context: {
        Token: tokenName,
...
export class ScopeMismatchError extends NexusError {
  constructor(
    ...
    const message = buildActionableMessage({
      ...
      description:
        "Expected scope '" + expectedScope + "' but the provider is registered with scope '" + actualScope + "'.",
      context: {
        Token: tokenName,
...
export class AsyncResolutionError extends NexusError {
  constructor(public readonly token: InjectionToken<any>) {
    ...
    const message = buildActionableMessage({
      ...
      description:
        'This provider uses an async factory or has async dependencies. Synchronous resolve() cannot be used.',
      context: {
        Token: tokenName,
      },
      suggestions: [
...
/**
 * Nexus-specific error aggregator for multiple errors.
 *
 * This class aggregates multiple errors that occurred during DI operations
 * (resolution, disposal, etc.) and provides actionable error messages.
 *
 * For aggregating `TitanError` instances with HTTP status codes and
 * deduplication support, use the re-exported `AggregateError` from
 * `../errors/core.js` instead.
 *
 * @example
 * ```typescript
 * const errors = [
 *   new ResolutionError(TokenA),
 *   new ResolutionError(TokenB),
 * ];
 * throw new NexusAggregateError(errors);
 * ```
 */
export class NexusAggregateError extends NexusError {
...
export class ErrorHandler {
  ...
  static handle(error: Error): void {
    ...
    this.logger.error({ err: error, code: isNexusError(error) ? error.code : 'UNKNOWN' }, 'Nexus error occurred');
