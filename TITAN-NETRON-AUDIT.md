...
- [ ] **U-P1 extract shared infra (biggest win, ~1000 lines dedup) — DESIGNED, ready to build:** create `@acme/titan-kit` (mirror auth-utils scaffold: type:module, peerDeps on titan-*, src/index.ts; downstream workspace globs `packages/*`). API from the survey:
  ...
  - ⏳ **Remaining U-P1:** roll out to storage / payments (+worker) / pricing / messaging (note: messaging's divergent fn names — keep wrappers or update its app.module) / geo (+ its missing config schema, U-P0); each per-app `tsc` verified. Then `wireNetronAuth()` + `ACME_REDIS_DB` pinning.
