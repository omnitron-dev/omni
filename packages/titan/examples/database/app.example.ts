/**
 * Example Titan Application with Database Module (Golden Path)
 *
 * Demonstrates the recommended Golden Path architecture:
 * - Uses @AutoTransactional() decorator for automatic transaction handling
 * - Uses runInTransaction() for functional transaction control
 * - All repositories automatically share the same transaction
 * - Shows service-level transaction coordination across multiple repositories
 */

import { Application } from '../../src/application.js';
import { Module, Injectable } from '../../src/decorators/index.js';
import { TitanDatabaseModule } from '../../src/modules/database/database.module.js';
import { InjectRepository } from '../../src/modules/database/database.decorators.js';
import { AutoTransactional, runInTransaction } from '../../src/modules/database/transaction/transaction.context.js';
...
export class OrderService {
  ...
  async cancelOrder(orderId: number, reason: string): Promise<void> {
    // All repository calls automatically share the same transaction
    const order = await this.orderRepo.findWithItems(orderId);
    if (!order) {
      throw new Error('Order not found');
    }
    ...
    // Update order status
    await this.orderRepo.updateOrderStatus(orderId, OrderStatus.CANCELLED);
  }
