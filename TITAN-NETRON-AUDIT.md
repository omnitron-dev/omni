...
- [ ] **NET-14** (discovered during SEC-2) `ServiceStub.call/get/set` dispatch by method name straight to `this.instance[method]` with NO public-surface whitelist — only `isValidPropertyName` (prototype-pollution guard) + `enforceMethodAccess` gate it. So ANY instance method/property (lifecycle hooks, private helpers, anything on the prototype that isn't `__proto__`/`constructor`) is wire-reachable by a peer holding the service's defId, even though it's not in `definition.meta.methods` (the `@Public` surface). SEC-2 `authDefaultDeny` closes this ONLY when opted-in (no ACL + no decorator ⇒ deny). **Fundamental non-opt-in fix:** in `enforceMethodAccess` (and the HTTP invoke path), reject any `methodName`/`prop` not present in `stub.definition.meta.methods` / `.properties` with a uniform NOT_FOUND (SEC-6 posture) — only the explicitly-`@Public` surface is callable, always. Needs: confirm `meta.methods` is built solely from `@Public` (so no legitimate caller invokes an undeclared method), verify local/nested-service paths (callerPeer null) are exempt, test undeclared-method rejection + declared-method pass on WS+HTTP. Potentially breaking → own task, careful tests.
