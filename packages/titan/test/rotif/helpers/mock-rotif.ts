...
export class MockRotifRedis extends EventEmitter {
  ...
  private pendingMessages = new Map<
    string,
    Array<{ id: string; consumer: string; idle: number; deliveryCount: number }>
  >();
  ...
  async xpending(stream: string, group: string, ...args: any[]): Promise<any> {
    ...
    if (args[0] === 'IDLE') {
      ...
      return pending.filter((p) => p.idle >= minIdle).map((p) => [p.id, p.consumer, p.idle, p.deliveryCount]);
    }
  ...
  async xrange(stream: string, start: string, end: string, ...args: any[]): Promise<[string, string[]][]> {
    const streamData = this.streams.get(stream) || [];
  ...
  async xrevrange(stream: string, end: string, start: string, ...args: any[]): Promise<[string, string[]][]> {
    ...
    if (cmd === 'GROUPS') {
      ...
      for (const [key, value] of this.consumerGroups) {
        if (value.stream === stream) {
          groups.push(['name', value.group, 'consumers', 1, 'pending', 0, 'last-delivered-id', value.lastId]);
        }
      }
      return groups;
    }
...
export class MockNotificationManager {
  ...
  private handlers = new Map<
    string,
    Array<{ pattern: string; handler: (msg: RotifMessage) => Promise<void>; subscription: Subscription }>
  >();
