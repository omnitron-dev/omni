...
import { createToken, type DynamicModule, type Token } from '@omnitron-dev/titan/nexus';
    ...
    return {
      ...
      providers: [
        ...
        [
          PM_SPAWNER_TOKEN,
          {
            useFactory: (loggerModule: any, pmConfig: IProcessManagerConfig) =>
              ProcessSpawnerFactory.create(loggerModule.logger ?? loggerModule, pmConfig),
        ...
        [
          PM_METRICS_TOKEN,
          {
            useFactory: (loggerModule: any) => new ProcessMetricsCollector(loggerModule.logger ?? loggerModule),
        ...
        [
          PM_HEALTH_TOKEN,
          {
            useFactory: (loggerModule: any) => new ProcessHealthChecker(loggerModule.logger ?? loggerModule),
            inject: [LOGGER_SERVICE_TOKEN],
          },
        ],
        ...
        [
          ...
          {
            useFactory: (loggerModule: any, pmConfig: IProcessManagerConfig) =>
              new ProcessManager(loggerModule.logger ?? loggerModule, pmConfig),
            inject: [LOGGER_SERVICE_TOKEN, PM_CONFIG_TOKEN],
          },
        ],
  ...
  static forRootAsync(options: {
    ...
    providers.push(
      ...
      [
        PM_SPAWNER_TOKEN,
        {
          useFactory: (loggerModule: any, config: IProcessManagerConfig) =>
            ProcessSpawnerFactory.create(loggerModule.logger ?? loggerModule, config),
          inject: [LOGGER_SERVICE_TOKEN, PM_CONFIG_TOKEN],
        },
      ],
      [
      ...
      [
        ...
        {
          useFactory: (loggerModule: any, config: IProcessManagerConfig) =>
            new ProcessManager(loggerModule.logger ?? loggerModule, config),
          inject: [LOGGER_SERVICE_TOKEN, PM_CONFIG_TOKEN],
        },
      ],
      [ProcessManager, { useExisting: PM_MANAGER_TOKEN }]
