/**
 * HTTP-specific built-in middleware for Netron
 *
 * Middleware that operates specifically on HTTP contexts
 */

import type { HttpMiddlewareContext, MiddlewareFunction, CorsOptions } from './types.js';
...
export class HttpBuiltinMiddleware {
   ...
   * OPTIMIZATION: Uses async compression to avoid blocking the event loop
  ...
  static compressionMiddleware(
    ...
    return async (ctx, next) => {
      ...
      // OPTIMIZATION: Use async compression to avoid blocking event loop
      // For small payloads (<16KB), sync is faster due to less overhead
      const useAsync = data.length > 16384;
  ...
  static requestIdMiddleware(
    ...
    const { header = 'X-Request-Id', generator = () => generateUuidV7() } = options;
