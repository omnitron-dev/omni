...
describe('PM Module - HTTP Cluster Integration', () => {
  describe('Phase 3.1: Multi-Process HTTP Server Pool', () => {
    ...
    it('should spawn multiple workers with HTTP transport', async () => {
    ...
    it('should verify each worker gets unique port (simulated)', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should distribute load using round-robin strategy', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should distribute load using least-loaded strategy', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 4,
    ...
    it('should handle high concurrent load across workers', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 4,
        strategy: PoolStrategy.ROUND_ROBIN,
        maxQueueSize: 200
  ...
  describe('Phase 3.2: Worker Crash and Recovery', () => {
    ...
    it('should handle worker crash and auto-restart', async () => {
    ...
    it('should redistribute load after worker failure', async () => {
      const pool = await pm.pool(CrashableWorker, {
        size: 3,
        strategy: PoolStrategy.LEAST_LOADED
      });
    ...
    it('should track restart count after crashes', async () => {
      const pool = await pm.pool(CrashableWorker, {
        size: 2,
        strategy: PoolStrategy.ROUND_ROBIN
      });
  ...
  describe('Phase 3.3: State Isolation', () => {
    ...
    it('should maintain independent state across workers', async () => {
    ...
    it('should isolate data storage between workers', async () => {
      const pool = await pm.pool(StatefulCounterWorker, {
        size: 2,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should verify worker PID isolation', async () => {
      const pool = await pm.pool(StatefulCounterWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should not share memory between workers', async () => {
      const pool = await pm.pool(StatefulCounterWorker, {
        size: 2,
        strategy: PoolStrategy.ROUND_ROBIN
      });
  ...
  describe('Phase 3.4: Health Monitoring', () => {
    ...
    it('should query health from individual workers', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN,
        healthCheck: {
    ...
    it('should aggregate cluster health status', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 4,
        strategy: PoolStrategy.LEAST_LOADED,
        healthCheck: {
    ...
    it('should exclude unhealthy workers from load balancing', async () => {
      const pool = await pm.pool(CrashableWorker, {
        size: 3,
        strategy: PoolStrategy.LEAST_LOADED,
        healthCheck: {
  ...
  describe('Phase 3.5: Metrics Collection', () => {
    ...
    it('should collect metrics from each worker', async () => {
    ...
    it('should aggregate cluster-wide metrics', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 4,
        strategy: PoolStrategy.ROUND_ROBIN,
        metrics: true
    ...
    it('should track HTTP-specific metrics', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 3,
        strategy: PoolStrategy.LEAST_LOADED,
        metrics: true
    ...
    it('should calculate throughput and saturation', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 4,
        strategy: PoolStrategy.ROUND_ROBIN,
        metrics: true
    ...
    it('should track error rates across workers', async () => {
      const pool = await pm.pool(CrashableWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN,
        metrics: true
    ...
    it('should collect latency percentiles', async () => {
  ...
  describe('Additional Integration Scenarios', () => {
    ...
    it('should support dynamic pool scaling', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 2,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should handle auto-scaling based on load', async () => {
      const pool = await pm.pool(HeavyComputeWorker, {
        size: 2,
        strategy: PoolStrategy.LEAST_LOADED,
        autoScale: {
    ...
    it('should handle graceful pool shutdown', async () => {
      const pool = await pm.pool(CalculatorWorker, {
        size: 3,
        strategy: PoolStrategy.ROUND_ROBIN
      });
    ...
    it('should support request queuing when workers are busy', async () => {
      const pool = await pm.pool(HeavyComputeWorker, {
        size: 2,
        strategy: PoolStrategy.ROUND_ROBIN,
        maxQueueSize: 50,
