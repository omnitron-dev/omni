...
describe('Enhanced Error System', () => {
  ...
  describe('EnhancedCommandError', () => {
    ...
    it('should generate adapter-specific suggestions for SSH', () => {
      const error = new EnhancedCommandError('npm test', 127, undefined, '', 'npm: command not found', 500, 'ssh');
    ...
    it('should generate suggestions for exit code 2 (misuse)', () => {
      const error = new EnhancedCommandError('grep', 2, undefined, '', 'grep: invalid option', 50, 'local');
    ...
    it('should generate suggestions for exit code 126 (cannot execute)', () => {
      const error = new EnhancedCommandError('./script.sh', 126, undefined, '', 'Permission denied', 100, 'local');
  ...
  describe('EnhancedConnectionError', () => {
    ...
    it('should generate suggestions for connection refused', () => {
      const error = new EnhancedConnectionError('localhost', new Error('connect ECONNREFUSED 127.0.0.1:22'), 22, 'ssh');
    ...
    it('should generate suggestions for timeout errors', () => {
      const error = new EnhancedConnectionError('remote.server.com', new Error('Connection timeout'), 22);
    ...
    it('should generate suggestions for authentication errors', () => {
      const error = new EnhancedConnectionError('secure.server.com', new Error('Authentication failed'), 22, 'ssh');
  ...
  describe('EnhancedTimeoutError', () => {
    it('should suggest increasing timeout', () => {
      const error = new EnhancedTimeoutError('npm install', 30000, 'local');
    ...
    it('should suggest background execution for long tasks', () => {
      const error = new EnhancedTimeoutError('apt-get install large-package', 60000, 'local');
    ...
    it('should suggest SSH-specific fixes', () => {
      const error = new EnhancedTimeoutError('long-running-script.sh', 30000, 'ssh');
    ...
    it('should suggest container resource checks', () => {
      const error = new EnhancedTimeoutError('heavy-computation', 45000, 'docker');
  ...
  describe('enhanceError', () => {
    it('should enhance standard CommandError', () => {
      const originalError = new CommandError('test-cmd', 1, undefined, 'output', 'error output', 100);
    ...
    it('should enhance standard ConnectionError', () => {
      const originalError = new ConnectionError('test.host.com', new Error('Connection failed'));
    ...
    it('should enhance standard TimeoutError', () => {
      const originalError = new TimeoutError('slow-command', 5000);
    ...
    it('should handle already enhanced errors', () => {
      const enhanced = new EnhancedExecutionError('Already enhanced', 'ENHANCED', { command: 'test' });
  ...
  describe('Edge cases', () => {
    it('should handle errors with no suggestions', () => {
      const error = new EnhancedExecutionError('Unknown error', 'UNKNOWN', { command: 'test' });
    ...
    it('should format error with all context fields', () => {
      const error = new EnhancedExecutionError('Complex error', 'COMPLEX', {
    ...
    it('should handle suggestions with only documentation', () => {
      const error = new EnhancedExecutionError('Error with doc', 'DOC_ERROR', {}, [
        {
  ...
  describe('Error suggestions quality', () => {
    it('should provide helpful install commands', () => {
      const error = new EnhancedCommandError('git', 127, undefined, '', 'git: command not found', 100);

      // Find the suggestion that has the actual install command (not the 'which' command)
      const installSuggestion = error.suggestions.find(
        (s) => s.message.includes('install') && s.command && !s.command.includes('which')
      );
    ...
    it('should handle signal-based termination', () => {
      const error = new EnhancedCommandError('sleep 100', null as any, 'SIGTERM', '', '', 5000);
