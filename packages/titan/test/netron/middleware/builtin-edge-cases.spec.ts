...
describe('NetronBuiltinMiddleware - Edge Cases', () => {
  ...
  describe('cachingMiddleware - edge cases', () => {
    ...
    it('should expire cache after TTL', async () => {
      ...
      // Create new context for second call after TTL expired
      const newContext = {
        ...mockContext,
        result: undefined, // Reset result
      };
      const newNext = vi.fn().mockImplementation(async () => {
        newContext.result = { data: 'new data' };
      });

      // Lazy expiration: the expired entry should be treated as cache miss
      await middleware(newContext, newNext);

      // newNext should be called because cache entry expired (cache miss)
      expect(newNext).toHaveBeenCalledTimes(1);
      expect(newContext.result).toEqual({ data: 'new data' });
