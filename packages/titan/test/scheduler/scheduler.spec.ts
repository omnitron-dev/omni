...
describe('Titan Scheduler Module', () => {
  ...
  afterEach(async () => {
    ...
    if (schedulerService?.isRunning()) {
      await schedulerService.onStop();
    }

    // Clear container to remove all singleton instances
    if (container) {
      // Force clear all registered instances
  ...
  describe('SchedulerService Integration', () => {
    ...
    it('should start and stop scheduler', async () => {
      ...
      await schedulerService.onStart();
    ...
    it('should trigger job manually', async () => {
      const handler = vi.fn().mockResolvedValue({ result: 'manual' });
    ...
    it('should handle graceful shutdown', async () => {
      // Add some jobs
      schedulerService.addInterval('interval-1', 1000, vi.fn());
      schedulerService.addInterval('interval-2', 2000, vi.fn());
