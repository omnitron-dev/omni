...
- [ ] **U-P2 module-consumption uniformity (sensible default = unify on Titan) — INVESTIGATED, next focused pass:**
  ...
  - ⏳ **ONLY remaining U-P2 piece — health factory:** contract-sensitive (dynamic `@Service` name + per-app response shape, called by monitoring/LBs). Two viable approaches: (a) `createHealthRpcService({name, toResponse})` factory emitting a dynamically-decorated `@Service` class — needs netron-registration verification (a running-netron test, can't be tsc-only); (b) safer — extract shared `convertHealthStatus`/`toHealthResponse` helpers, keep each app's concrete thin `@Service` class. Warrants a focused pass with proper verification.
