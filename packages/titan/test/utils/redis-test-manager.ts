import { execSync, execFileSync, ChildProcess } from 'node:child_process';
...
export class RedisTestManager {
  ...
  private constructor(options: RedisTestManagerOptions = {}) {
    ...
    if (this.verbose) {
      console.log(
        `[RedisTestManager] Initialized for Jest worker ${workerId} with port range ${this.basePort}-${this.basePort + 1000}`
      );
    }
