...
describe('HttpServer - Edge Cases & Error Paths', () => {
  ...
  beforeEach(async () => {
    ...
    const stub = {
      ...
      call: vi.fn(async (methodName: string, args: any[], peer: any) => {
  ...
  describe('Fast-Path Optimization (handleSimpleInvocation)', () => {
    ...
    it('should bypass fast-path when Authorization header is present', async () => {
      ...
      // The server goes through middleware path when Authorization header is present
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    it('should bypass fast-path when Origin header is present with CORS enabled', async () => {
      ...
      // The server applies CORS headers via applyCorsHeaders() when Origin is present
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      expect(response.status).toBe(201);
      // Check for CORS headers - applyCorsHeaders sets Access-Control-Allow-Origin to the Origin value
  ...
  describe('Input Validation Errors', () => {
    ...
    it('should return 400 error for validation failure in middleware pipeline', async () => {
      ...
      // Include Authorization to trigger middleware pipeline
      // Validation is performed in both fast-path and middleware pipeline via validateMethodInput()
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('CORS Handling', () => {
    ...
    it('should include CORS headers in response when Origin header present', async () => {
      ...
      // CORS headers are applied via applyCorsHeaders() which checks for Origin header
      // and cors option being enabled (set to true in beforeEach)
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  describe('Authentication Middleware', () => {
    it('should extract Bearer token from Authorization header', async () => {
      ...
      // The server's http-auth middleware extracts Bearer token from Authorization header
      // Without an AuthenticationManager configured, the token is stored in metadata
      // but the request still proceeds successfully
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    it('should handle malformed Authorization header gracefully', async () => {
      ...
      // The server handles malformed Authorization headers gracefully
      // extractBearerToken returns null for non-Bearer tokens, so no auth context is set
      // but the request still proceeds without authentication
      const response = await fetch(`${baseUrl}/netron/invoke`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
