...
Consolidated audit of `app → @acme/titan-kit → titan-database → @kysera/* → kysely`
across titan-database internals and 8 backends (main, portal, portal-seed, geo,
messaging, storage, payments, pricing). Goal: each concern lives at exactly
...
null)` sites in payments, and one real bug: payments `asset.repository.ts:192-214`
...
5. Idempotency helper (`unique index + catch 23505 + refetch`) — payments ×5.
6. Atomic guarded column arithmetic (`SET col = col ± x WHERE col ± x >= 0
   RETURNING`) — payments money paths.
7. Transactional outbox primitive (`FOR UPDATE SKIP LOCKED` claim, backoff,
   GC) — payments ×3 (~1130 LOC), storage ×1, same shape.
8. RLS `filter()`: subquery/joined-table predicates (documented limitation
   drove ~150 LOC of ad hoc visibility filtering in payments).
9. `JsonColumn<T>` ColumnType convention (pricing has a live type/runtime
   drift bug); runtime table-family routing helper (pricing backfill).
...
- **payments** (~600–750 LOC of free wins): switch CAS methods to
...
- **pricing**: delete `createMany` reinvention + duplicate
...
- **main** (largest: 128k LOC, 82 repos, 130 migrations; ~3160 LOC
  ...
  (`deletedAtColumn` is configurable) — same false comment as payments.
- **titan-kit**: the proven push-down vehicle — add the error-taxonomy
  scaffold (`defineErrorTaxonomy()`; payments+pricing have identical
...
4. App sweeps in order: messaging (correctness), payments (money paths),
   storage, geo/pricing, portal-seed rebuild.
...
and payments plugins.ts:9 both reject softDeletePlugin for a limitation that
...
TAR redesign must support table families (pricing OhlcvRepository serves
3 identical candle tables via runtime dispatch and has to ignore all
inherited CRUD). payments carries two mutually contradictory soft-delete
