...
describe('MetricsBridge', () => {
  ...
  it('refreshAppGauges sets app status gauge to 1 only for the current status', () => {
    ...
    const text = registry.toPrometheusText();
    expect(text).toMatch(/omnitron_app_status{app="payments",status="online"} 1/);
    expect(text).toMatch(/omnitron_app_status{app="payments",status="errored"} 0/);
    ...
    expect(text).toMatch(/omnitron_app_restarts{app="payments"} 5/);
  ...
  it('recordEsbuildBuild populates the duration histogram + outcome counter', () => {
    bridge.recordEsbuildBuild('payments', 'build', 1500, true);
    bridge.recordEsbuildBuild('payments', 'rebuild', 200, false);

    const text = registry.toPrometheusText();
    expect(text).toMatch(/omnitron_esbuild_duration_seconds_bucket{[^}]*app="payments"/);
    expect(text).toMatch(
      /omnitron_esbuild_builds_total{[^}]*app="payments",[^}]*kind="build",[^}]*outcome="success"/,
    );
    expect(text).toMatch(
      /omnitron_esbuild_builds_total{[^}]*app="payments",[^}]*kind="rebuild",[^}]*outcome="failure"/,
    );
  });

  it('recordAppRestart bumps the per-reason counter and gauge', () => {
    bridge.recordAppRestart('payments', 3, 'crash');
    bridge.recordAppRestart('payments', 4, 'crash');

    const text = registry.toPrometheusText();
    expect(text).toMatch(
      /omnitron_app_restarts_total{app="payments",reason="crash"} 2/,
    );
    expect(text).toMatch(/omnitron_app_restart_attempt{app="payments"} 4/);
  });
