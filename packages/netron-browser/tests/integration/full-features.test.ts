...
import { Application, NetronToken } from '@omnitron-dev/titan';
import { Module, Injectable } from '@omnitron-dev/titan/decorators';
import { Service, Public, type INetron } from '@omnitron-dev/titan/netron';
import { HttpNativeServer } from '@omnitron-dev/titan/netron/transport/http';
import { HttpRemotePeer, HttpCacheManager } from '../../src/transport/http/index.js';
...
import { isAuthenticateResponse, createInvalidateCacheRequest } from '../../src/core-tasks/index.js';
...
describe('Full Features Integration', () => {
  let app: Application;
  let httpServer: HttpNativeServer;
  ...
  beforeEach(async () => {
    ...
    app = await Application.create(TestServicesModule, {
      ...
      // Don't disable core modules - we need Logger for Netron to be registered
    });

    await app.start();

    // Get Netron instance using the NetronToken
    netron = app.get<INetron>(NetronToken);

    // Services are auto-exposed by Titan when they have @Service decorator
    // No need to manually expose them

    // Create HTTP server
    httpServer = new HttpNativeServer({ port: 0, host: 'localhost' });
    httpServer.setPeer(netron.peer as any);
    await httpServer.listen();

    // Get actual port assigned by the OS (when port 0 is used)
    // Access the underlying Node.js server to get the actual port
    const server = (httpServer as any).server;
    const address = server?.address?.() || null;
    const port = typeof address === 'object' && address !== null ? address.port : httpServer.port || 3000;
    ...
    // Create client
    client = new HttpRemotePeer(`http://localhost:${port}`);

    // Set up cache manager for HTTP responses
    const cacheManager = new HttpCacheManager({
      maxEntries: 1000,
      defaultMaxAge: 60000, // 60 seconds
    });
  ...
  afterEach(async () => {
    // Cleanup - handle undefined values
    if (client) {
      client.close();
    }
    if (authClient) {
  ...
  describe('Authentication Flow', () => {
    it('should authenticate user and store token', async () => {
      ...
      const response = await client.call('AuthService@1.0.0', 'authenticate', [credentials]);
  ...
  describe('Middleware Integration', () => {
    it('should inject auth token via middleware', async () => {
      ...
      const authResponse = await client.call('AuthService@1.0.0', 'authenticate', [credentials]);
  ...
  describe('Auth-Aware Service Discovery', () => {
    it('should query interface and discover service methods', async () => {
      // HTTP transport doesn't fetch definitions - just creates a proxy
      const userService = await client.queryInterface('UserService@1.0.0');

      // Verify we can call methods (which proves they exist)
      const publicInfo = await client.call('UserService@1.0.0', 'getPublicInfo');
      expect(publicInfo).toBeDefined();
      expect(publicInfo.info).toBe('This is public information');

      // HTTP transport discovers methods by calling them, not by fetching metadata
      // This is by design - HTTP is stateless and doesn't require definition exchange
    });
    ...
    it('should query interface with auth token in middleware', async () => {
      ...
      // Verify we can call methods with auth middleware
      const publicInfo = await client.call('UserService@1.0.0', 'getPublicInfo');
      expect(publicInfo).toBeDefined();
  ...
  describe('Error Handling Integration', () => {
    ...
    it('should handle missing service gracefully', async () => {
      // HTTP transport will create a proxy even for non-existent services
      // The error only happens when you try to call a method
      await expect(async () => {
        await client.call('NonExistentService@1.0.0', 'someMethod');
      }).rejects.toThrow();
    });

    it('should handle network errors with proper error context', async () => {
      // Close the client to simulate network error
      const testClient = new HttpRemotePeer('http://localhost:9999'); // Non-existent port
