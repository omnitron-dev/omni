...
describe('Core Container', () => {
  ...
  describe('Lifecycle Management', () => {
    it('should call onInit lifecycle hook', async () => {
      ...
      const token = createToken<TestService>('TestService');
      // Lifecycle hooks only work with non-transient scopes (singleton/scoped)
      // because transient instances are not tracked to avoid memory leaks
      container.register(token, { useClass: TestService, scope: Scope.Singleton });

      const _instance = container.resolve(token);
  ...
  describe('Performance Optimizations', () => {
    it('should cache singleton instances', () => {
      const factory = vi.fn(() => ({ value: 'cached' }));
