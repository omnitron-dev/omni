...
export class UnixSocketConnection extends TcpConnection {
  ...
  constructor(socket: net.Socket, options: UnixSocketOptions, socketPath: string) {
...
export class UnixSocketServer extends TcpServer {
  ...
  constructor(server: net.Server, options: UnixSocketOptions, socketPath: string) {
...
export class UnixSocketTransport extends TcpTransport {
  ...
  override async connect(address: string, options: UnixSocketOptions = {}): Promise<ITransportConnection> {
    ...
    // Strip query parameters (Netron appends ?id=... which is invalid for filesystem paths)
    const rawPath = parsed.path || address;
  ...
  override parseAddress(address: string): { protocol: string; path?: string; params?: Record<string, string> } {
    // Extract query parameters (Netron may append ?id=...)
    const [addressPart, queryPart] = address.split('?') as [string, string | undefined];
    ...
    if (addressPart.startsWith('unix://')) {
      return { protocol: 'unix', path: addressPart.substring(7), params };
    }

    if (addressPart.startsWith('unix:')) {
      return { protocol: 'unix', path: addressPart.substring(5), params };
    }
    ...
    // Assume it's a path
    return { protocol: 'unix', path: addressPart, params };
  }
...
export class NamedPipeTransport extends TcpTransport {
  ...
  override parseAddress(address: string): { protocol: string; path?: string; params?: Record<string, string> } {
    if (address.startsWith('pipe://')) {
      const pipePath = address.substring(7);
      return {
        protocol: 'pipe',
        path: pipePath.startsWith('\\\\.\\pipe\\') ? pipePath : `\\\\.\\pipe\\${pipePath}`,
        params: {},
      };
    }

    if (address.startsWith('pipe:')) {
      const pipePath = address.substring(5);
      return {
        protocol: 'pipe',
        path: pipePath.startsWith('\\\\.\\pipe\\') ? pipePath : `\\\\.\\pipe\\${pipePath}`,
        params: {},
