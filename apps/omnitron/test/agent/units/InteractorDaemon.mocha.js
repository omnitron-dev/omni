...
const cst = require('../../../dist/agent/constants');
const InteractorDaemon = require('../../../dist/agent/InteractorDaemon');
...
describe('InteractorDaemon', () => {
  describe('construct', (_) => {
    it('should retrieve conf, init transport and http client', (done) => {
      let _httpInit = 0;
      let Daemon = getDaemon();
      let utility = new ModuleMocker('../../../dist/agent/Utility');
      utility.mock({
      ...
      assert(daemon.transport instanceof require('../../../dist/agent/TransporterInterface'));
      daemon.transport.transporters.forEach((transport) => {
        clearInterval(transport._worker);
      });
      assert(_httpInit === 1);
