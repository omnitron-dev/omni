...
mod tests {
    ...
    async fn test_end_to_end_metrics_flow() -> Result<()> {
        ...
        let storage = Arc::new(MetricsStorage::new(temp_dir.path(), Some(30))?);
        ...
        collector.record_tool_call("progress.create_task", 5.2, true);
        collector.record_tokens("progress.create_task", 50, 25);
    ...
    async fn test_metrics_persistence_across_restarts() -> Result<()> {
        ...
        {
            let new_storage = Arc::new(MetricsStorage::new(temp_dir.path(), Some(30))?);
