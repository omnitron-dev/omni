...
Marks a class as a controller with an optional base path. This decorator is primarily used for
organizational purposes and works with the `@Injectable` decorator to register the controller
in the DI container.

> **Note**: Titan uses Netron's RPC-over-HTTP pattern rather than traditional REST routing.
> For HTTP endpoints, use `@Service` and `@Method` decorators instead. The `@Controller`
> decorator stores path metadata but does not provide REST routing (@Get, @Post, etc.).
...
describe('Service decorators', () => {
  it('should apply service metadata', () => {
    ...
    const hasMethod = hasDecorator('Public', TestService.prototype, 'testMethod');
...
```typescript
// NestJS (REST-style routing with decorators)
@Injectable()
@Controller('users')
...
// Titan equivalent (RPC-style via Netron)
// Note: Titan uses RPC-over-HTTP, not REST routing.
// @Get/@Post/@Param decorators do not exist in Titan.
// Use @Service + @Public for exposing methods over HTTP.
@Service('users@1.0.0')
class TitanService {
  @Public()
  getUser(id: string) {}
