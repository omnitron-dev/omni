 ...
 * {
 *  'process_name': {
 *    process : {},         // OMNITRON process meta data
 *    data : {
...
module.exports = class TransactionAggregator {
  ...
  init() {
    ...
    this.pushInteractor._iomnitron.bus.on('process:event', (data) => {
      if (data.event !== 'online' || !this.processes[data.process.name]) return false;

      let rev = data.process.versioning && data.process.versioning.revision ? data.process.versioning.revision : null;
  ...
  aggregate(packet) {
    ...
    newTrace.spans = newTrace.spans.filter((span) => span.endTime !== span.startTime);
  ...
  prepareAggregationforShipping() {
    ...
    Object.keys(this.processes).forEach((appName) => {
      ...
      Object.keys(routes).forEach((path) => {
        ...
        const variances = data.variances.sort((a, b) => b.count - a.count).slice(0, 5);
