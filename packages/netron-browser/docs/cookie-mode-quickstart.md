...
The bootstrap (`apps/*/src/bootstrap.ts` in the downstream project) reads
`selectTokenTransport()` from `@acme/auth-utils` and threads the
...
- **Cross-origin browser fetch**: a portal at `omni.example` calling a
  ...
  same-origin gateway (the downstream deployment template does this) or
