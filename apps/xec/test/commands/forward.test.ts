...
describe('Forward Command', () => {
  ...
  describeSSH(
    ...
    () => {
      it('should forward SSH ports correctly', async () => {
        ...
        await command.execute(['hosts.test', `${localPort}:22`, { background: true, quiet: true }]);
      ...
      it('should forward multiple ports simultaneously', async () => {
        ...
        await command.execute(['hosts.test', `${localPort1}:22,${localPort2}:22`, { background: true, quiet: true }]);
      ...
      it('should handle auto port selection', async () => {
        ...
        await command.execute(['hosts.test', '0:22', { background: true, quiet: false }]);
  ...
  describe('Session Management', () => {
    ...
    it('should prevent duplicate port forwards on same local port', async () => {
      ...
      await expect(command.execute(['hosts.test', '8080:80', { quiet: true }])).rejects.toThrow(
        'Port forwarding already active'
      );
    });
  });
  ...
  describe('Dry Run Mode', () => {
    it('should not forward ports in dry run mode', async () => {
      ...
      try {
        await command.execute(['hosts.db', '5432', { dryRun: true, quiet: false }]);
  ...
  describe('Error Handling', () => {
    ...
    it('should validate port numbers', async () => {
      ...
      await expect(command.execute(['hosts.test', '99999', { quiet: true }])).rejects.toThrow(
        'Invalid remote port: 99999'
      );
    ...
    it('should handle connection failures gracefully', async () => {
      ...
      await expect(command.execute(['hosts.nonexistent', '8080', { quiet: true }])).rejects.toThrow(
  ...
  describe('Reverse Tunneling', () => {
    it('should indicate reverse tunneling is not yet supported for SSH', async () => {
      ...
      await expect(command.execute(['hosts.remote', '8080:80', { reverse: true, quiet: true }])).rejects.toThrow(
        'Reverse tunneling is not yet implemented in this version'
      );
    });
    ...
    it('should not support reverse forwarding for Docker', async () => {
      ...
      await expect(command.execute(['containers.app', '80', { reverse: true, quiet: true }])).rejects.toThrow(
        'Reverse port forwarding is not supported for Docker'
      );
    });
    ...
    it('should not support reverse forwarding for Kubernetes', async () => {
      ...
      await expect(command.execute(['pods.db', '5432', { reverse: true, quiet: true }])).rejects.toThrow(
        'Reverse port forwarding is not supported for Kubernetes'
      );
    });
  });
});
