...
function createMockContainer(): IContainer {
  return {
    ...
    resolveOptional: vi.fn(),
    registerStream: vi.fn().mockReturnThis(),
    resolveStream: vi.fn(),
    resolveParallel: vi.fn(),
    resolveParallelSettled: vi.fn(),
    resolveBatch: vi.fn(),
    ...
    dispose: vi.fn(),
    clearCache: vi.fn(),
    getMetadata: vi.fn().mockReturnValue({ registrations: 0, cached: 0, scopes: 0 }),
    getContext: vi.fn(),
    resolveLazy: vi.fn(),
    resolveLazyAsync: vi.fn(),
    addMiddleware: vi.fn().mockReturnThis(),
...
describe('FeatureFlagStrategy', () => {
  ...
  describe('select', () => {
    it('should select provider with matching feature', () => {
      ...
      const providers = [{ feature: 'oldUI', value: 'old' }, { feature: 'newUI', value: 'new' }, { value: 'default' }];
    ...
    it('should fallback to provider without feature when no match', () => {
      ...
      const providers = [{ feature: 'feature1', value: 'featured' }, { value: 'default' }];
...
describe('RoleBasedStrategy', () => {
  ...
  describe('select', () => {
    ...
    it('should fallback to public provider when no role match', () => {
      ...
      const providers = [{ requiredRole: 'admin', value: 'admin' }, { value: 'public' }];
...
describe('ContextManager', () => {
    ...
    it('should allow registering custom strategy', () => {
      const customStrategy: ResolutionStrategy = {
        ...
        select: (providers, context) =>
          providers.find((p) => p.custom === context.metadata?.['custom']) || providers[0],
      };
  ...
  describe('runWithContext', () => {
    ...
    it('should maintain context across promise chains', async () => {
      ...
      const result = await manager.runWithContext(testContext, async () => {
        const value1 = await Promise.resolve().then(() => manager.getCurrentContext().get(testKey));

        const value2 = await Promise.resolve().then(() => manager.getCurrentContext().get(testKey));
  ...
  describe('selectProvider', () => {
    ...
    it('should fallback to first provider when no strategy applies', () => {
      const context = createMockResolutionContext({});
      const providers = [{ value: 'first' }, { value: 'second' }];
...
describe('InjectContext decorator', () => {
  ...
  it('should handle multiple context parameters', () => {
    class TestClass {
      method(@InjectContext(ContextKeys.User) user: any, @InjectContext(ContextKeys.Tenant) tenant: any) {
        return { user, tenant };
      }
    }
...
describe('getCurrentAuthContext', () => {
  it('should return undefined when no auth context is set', () => {
    ...
    const result = manager.runWithContext(emptyContext, () => getCurrentAuthContext());
  ...
  it('should return auth context when set in current context', () => {
    ...
    const result = manager.runWithContext(contextWithAuth, () => getCurrentAuthContext());
  ...
  it('should return auth context from nested context', () => {
    ...
    const result = manager.runWithContext(childContext, () => getCurrentAuthContext());
...
describe('Concurrent Access', () => {
  ...
  it('should maintain context across multiple nested async calls', async () => {
    ...
    const result = await manager.runWithContext(context, async () => {
      const level1 = await Promise.resolve().then(() => manager.getCurrentContext().get(ContextKeys.CorrelationId));

      const level2 = await Promise.resolve().then(
        async () => await Promise.resolve().then(() => manager.getCurrentContext().get(ContextKeys.CorrelationId))
      );
