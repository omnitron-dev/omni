...
export class DockerTestManager extends EventEmitter {
      ...
      try {
        ...
        if (options?.healthcheck) {
          if (status.health === 'healthy') {
            ...
            // Don't return immediately - also verify port if specified
        ...
        if (options?.port) {
          ...
          if (container) {
            ...
            if (hostPort && (await this.isPortListening('127.0.0.1', hostPort))) {
              ...
              // If healthcheck was required and passed, or no healthcheck required, we're done
              if (!options?.healthcheck || healthCheckPassed) {
        ...
        // If healthcheck passed but we're still here, wait for port
        if (healthCheckPassed && !options?.port) {
          return;
        }
  ...
  private async isPortListening(host: string, port: number): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      socket.setTimeout(500); // Increased from 100ms to 500ms for more reliable checks
      socket.on('connect', () => {
        socket.destroy();
        resolve(true);
      });
...
export class DatabaseTestManager {
  ...
  static async createPostgresContainer(options?: {
    ...
    const container = await DatabaseTestManager.getDockerManager().createContainer({
      name: options?.name,
    ...
    // Additional verification: try to connect to the database with more retries
    await DatabaseTestManager.verifyPostgresConnection(container, database, user, password, 15);

    return container;
  }

  /**
   * Verify PostgreSQL connection by attempting a simple query
   * This ensures the database is truly ready beyond just the health check
   */
  private static async verifyPostgresConnection(
    ...
    if (!port) {
      throw new Error('PostgreSQL port not found in container');
    }
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (i === maxRetries - 1) {
          throw new Error(`Failed to verify PostgreSQL connection after ${maxRetries} attempts: ${error}`);
        }
        // Exponential backoff: 1s, 2s, 3s, 4s...
        const delay = Math.min(1000 + i * 1000, 4000);
  ...
  static async createMySQLContainer(options?: {
    ...
    const container = await DatabaseTestManager.getDockerManager().createContainer({
      ...
      waitFor: {
        healthcheck: true,
        timeout: 180000, // Increased from 90s to 180s for MySQL 8.0
      },
    });

    // Additional verification: try to connect to the database with more retries
    await DatabaseTestManager.verifyMySQLConnection(container, database, user, password, 20);

    return container;
  }

  /**
   * Verify MySQL connection by attempting a simple query
   * This ensures the database is truly ready beyond just the health check
   */
  private static async verifyMySQLConnection(
    container: DockerContainer,
    database: string,
    user: string,
    password: string,
    ...
    if (!port) {
      throw new Error('MySQL port not found in container');
    }

    for (let i = 0; i < maxRetries; i++) {
      try {
        // Dynamic import to avoid requiring mysql2 at module load time
        ...
        const connection = await mysql.createConnection({
          ...
          user,
          password,
          // Connection options to prevent "Connection lost" errors
        ...
        if (i === maxRetries - 1) {
          throw new Error(`Failed to verify MySQL connection after ${maxRetries} attempts: ${error}`);
        }
