...
import type {
  ...
  APIRequest,
  APIResponse,
} from '../../src/enterprise/graphql-federation.js';
import {
  SchemaParser,
  GraphQLService,
...
describe('GraphQLService', () => {
  ...
  describe('addResolver', () => {
    it('should add query resolvers', () => {
      const resolver = vi.fn().mockResolvedValue({ id: '1', name: 'John', email: 'john@example.com' });
    ...
    it('should emit resolver:added event', () => {
      ...
      service.addResolver('Query', 'user', vi.fn());
  ...
  describe('execute', () => {
    ...
    it('should execute simple queries', async () => {
      const result = await service.execute(`query { user(id: "1") { id name email } }`, { id: '1' });
    ...
    it('should handle query variables', async () => {
      const result = await service.execute(`query GetUser($id: ID!) { user(id: $id) { id name } }`, { id: '123' });

      expect(result.errors).toBeUndefined();
    });

    it('should return errors for missing resolvers', async () => {
      const result = await service.execute(`query { unknownField { id } }`);
    ...
    it('should provide execution context', async () => {
      let _capturedContext: GraphQLContext | undefined;
      ...
      await service.execute(`query { contextTest { success } }`, {}, context);
  ...
  describe('registerEntityResolver', () => {
    it('should register entity resolvers', () => {
      const resolver = vi.fn().mockResolvedValue([{ id: '1', name: 'John' }]);
    ...
    it('should emit entity:registered event', () => {
      ...
      service.registerEntityResolver('User', ['id'], vi.fn());
  ...
  describe('resolveEntities', () => {
    ...
    it('should handle missing entity resolvers', async () => {
      const representations: EntityRepresentation[] = [{ __typename: 'UnknownType', id: '1' }];
    ...
    it('should group representations by type', async () => {
      const userResolver = vi.fn().mockResolvedValue([{ id: '1', name: 'John' }]);
...
describe('GraphQLFederationGateway', () => {
  ...
  describe('execute', () => {
    beforeEach(() => {
      ...
      userService?.addResolver('Query', 'users', async () => [{ id: '1', name: 'John', email: 'john@example.com' }]);
      ...
      productService?.addResolver('Query', 'products', async () => [{ id: '100', name: 'Widget', price: 29.99 }]);
    });

    it('should execute queries across services', async () => {
      const result = await gateway.execute(`query { user(id: "1") { id name } }`);
...
describe('APIGateway', () => {
  ...
  describe('use middleware', () => {
    it('should allow adding custom middleware', async () => {
      const customMiddleware = vi.fn().mockResolvedValue(null);
    ...
    it('should allow middleware to short-circuit request', async () => {
      ...
      const customMiddleware = vi.fn().mockResolvedValue(customResponse);
      apiGateway.use(customMiddleware);
...
describe('Federation Integration', () => {
  describe('Cross-service entity resolution', () => {
    ...
    it('should resolve entities from owning service', async () => {
      ...
      const userResults = await gateway.resolveEntities([{ __typename: 'User', id: '1' }], context);
      ...
      apiGateway = new APIGateway(
        {
          ...
          graphql: [{ name: 'users', schema: userServiceSchema }],
          cors: { origin: '*' },
