#!/usr/bin/env bash
#
# rebuild-backends.sh — one-command rebuild of the downstream backend
...
#      the dist artefacts when bundling each downstream backend at spawn.
#   2. Clear `.omnitron-build/` caches in every downstream app. Without
...
#
# Usage:
#   scripts/rebuild-backends.sh
#   scripts/rebuild-backends.sh --skip-titan         # skip step 1
#   scripts/rebuild-backends.sh --apps main,storage  # subset

set -euo pipefail

OMNI_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BACKENDS_ROOT="${BACKENDS_ROOT:-$HOME/projects/acme}"

# Defaults — all downstream backends managed by omnitron.
DEFAULT_APPS="main,storage,pricing,payments,messaging"
APPS=""
SKIP_TITAN=0
...
for app in "${APP_LIST[@]}"; do
  cache="$BACKENDS_ROOT/apps/$app/.omnitron-build"
  if [[ -d "$cache" ]]; then
