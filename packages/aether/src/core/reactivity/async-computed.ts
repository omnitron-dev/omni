 ...
 */
export function asyncComputed<T>(fn: () => Promise<T>, options: AsyncComputedOptions<T> = {}): AsyncComputed<T> {
  ...
  const executor = options.debounce ? debounce(fn, options.debounce) : fn;
...
export function asyncComputedGroup<T extends Record<string, () => Promise<any>>>(
  ...
  const loading = computed(() => loadingSignals.some((s) => s()));
