...
import { ProcessStatus } from '../../../src/modules/pm/index.js';
...
describe('Real-World Scenarios - E-Commerce Order Processing', () => {
  ...
  it('should process a complete order successfully', async () => {
    // Disable payment failures for this deterministic test
    await paymentService.setSimulateFailures(false);
  ...
  it('should handle payment failures with compensation', async () => {
    // Enable guaranteed payment failures for this test
    await paymentService.setSimulateFailures(true, 1.0); // 100% failure rate
    ...
    // The workflow should fail due to 100% payment failure rate
    let failed = false;
    try {
    ...
    // Verify inventory was released (compensation was called)
...
describe('Real-World Scenarios - Real-Time Analytics', () => {
  ...
  it('should process event stream and aggregate statistics [MockSpawner: async generator proxy support]', async () => {
  ...
  it('should reset statistics [MockSpawner: service dependencies]', async () => {
    const service = await pm.spawn<AnalyticsAggregatorService>(AnalyticsAggregatorService);
