/**
 * Query Builder for HTTP Interface (shared)
 *
 ...
 * @omnitron-dev/netron-browser. Depends on the structural {@link IHttpRequestExecutor}
 * (each package's concrete HttpTransportClient satisfies it) rather than a concrete
 * transport class.
 */
...
/**
 * Local factory mirroring titan's `Errors.badRequest` / `Errors.internal` exactly
 * (the only two used here) — netron-protocol exposes TitanError but not the full
 * titan `Errors` namespace. Behaviour-identical: same code + message + details.
 */
const Errors = {
  badRequest: (message: string, details?: any): TitanError =>
    new TitanError({ code: ErrorCode.BAD_REQUEST, message, details }),
  internal: (message: string): TitanError => new TitanError({ code: ErrorCode.INTERNAL_ERROR, message }),
};
...
export class QueryBuilder<TService = any, TMethod extends keyof TService = keyof TService> {
  ...
  private static backgroundIntervals = new Map<string, ReturnType<typeof setInterval>>();

  constructor(
    private transport: IHttpRequestExecutor,
