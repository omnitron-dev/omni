...
describeOrSkip('Netron - Comprehensive Tests', () => {
  ...
  describe('Task Management', () => {
    ...
    it('should track task execution', async () => {
      ...
      const result = await netron.taskManager.runTask('tracked_task');
  ...
  describe('Logger Integration', () => {
    ...
    it('should accept custom logger context', () => {
      ...
      const _customNetron = new Netron(customLogger, {
  ...
  describe('Error Handling', () => {
    it('should handle transport server start failure gracefully', async () => {
      const failingTransport = {
        ...
        createServer: vi.fn().mockRejectedValue(new Error('Server start failed')),
      };
  ...
  describe('Integration Points', () => {
    it('should integrate with task manager', async () => {
      ...
      netron.addTask(testTask);
      const result = await netron.taskManager.runTask('test_task');
    ...
    it('should handle peer connect events', async () => {
      ...
      expect(connectHandler).toHaveBeenCalledWith(expect.objectContaining({ peerId: 'peer-456' }));
    });
    ...
    it('should handle peer disconnect events', async () => {
      ...
      expect(disconnectHandler).toHaveBeenCalledWith(expect.objectContaining({ peerId: 'peer-789' }));
    });
  });
  ...
  describe('Memory and Resource Management', () => {
    it('should clean up resources on stop', async () => {
      ...
      const _initialPeers = netron.peers.size;
      const _initialServers = netron.transportServers.size;
