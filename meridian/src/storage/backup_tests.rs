...
mod tests {
    ...
    fn create_test_db(path: &std::path::Path, num_keys: usize) {
    ...
    async fn test_concurrent_backups() {
        ...
        let manager = std::sync::Arc::new(tokio::sync::RwLock::new(
            BackupManager::new(db_path.clone(), config).unwrap()
        ));
        ...
        for i in 0..3 {
            ...
            let handle = tokio::spawn(async move {
                manager_clone.write().await.create_backup(
                    BackupType::Manual,
                    Some(format!("Concurrent backup {}", i)),
                    vec![],
                ).await
            });
            handles.push(handle);
        ...
        let backups = manager.read().await.list_backups().await.unwrap();
        assert_eq!(backups.len(), 3);
    }
    ...
    async fn test_backup_with_empty_database() {
        ...
        let _db = DB::open_default(&db_path).unwrap();
        ...
        assert!(metadata.size_bytes > 0); // RocksDB has metadata even when empty
        assert!(metadata.verified);
    }
    ...
    async fn test_error_invalid_backup_id() {
        ...
        // Note: delete is idempotent, so it succeeds even if backup doesn't exist
        assert!(result.is_ok());
    }
}
