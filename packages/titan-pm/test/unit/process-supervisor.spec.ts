...
import { SupervisionStrategy, RestartDecision, ProcessStatus } from '../../src/types.js';
import type {
  IProcessManager,
  ISupervisorOptions,
  ISupervisorChild,
...
import { SUPERVISOR_METADATA_KEY } from '../../src/decorators.js';
...
const createMockProxy = (processId: string) => ({
  ...
  __getMetrics: vi.fn().mockResolvedValue({ cpu: 0, memory: 0 }),
  __getHealth: vi.fn().mockResolvedValue({ status: 'healthy', checks: [], timestamp: 0 }),
...
const createMockProcessManager = (): IProcessManager & EventEmitter => {
  ...
  (emitter as any).spawn = vi.fn().mockImplementation(() => {
    const processId = `proc-${++processIdCounter}`;
    return Promise.resolve(createMockProxy(processId));
  });

  (emitter as any).pool = vi.fn().mockImplementation(() => {
    const processId = `pool-${++processIdCounter}`;
    return Promise.resolve(createMockProxy(processId));
  });

  (emitter as any).discover = vi.fn().mockResolvedValue(null);
  (emitter as any).workflow = vi.fn().mockResolvedValue({});
  (emitter as any).supervisor = vi.fn().mockResolvedValue({});
  (emitter as any).getProcess = vi.fn().mockReturnValue(undefined);
  (emitter as any).listProcesses = vi.fn().mockReturnValue([]);
  (emitter as any).kill = vi.fn().mockResolvedValue(true);
  (emitter as any).getMetrics = vi.fn().mockResolvedValue(null);
  (emitter as any).getHealth = vi.fn().mockResolvedValue(null);
  (emitter as any).shutdown = vi.fn().mockResolvedValue(undefined);

  return emitter;
};

// Helper to create supervisor class with metadata
const createSupervisorClass = (
  children: Map<string, ISupervisorChild>,
...
describe('ProcessSupervisor', () => {
  ...
  describe('Supervision Strategies', () => {
    ...
    describe('REST_FOR_ONE strategy', () => {
      it('should restart failed child and all children started after it', async () => {
        ...
        const databaseSpawnCall = (mockManager.spawn as vi.Mock).mock.results[1];
  ...
  describe('Restart Decision Logic', () => {
    ...
    it('should respect IGNORE decision from custom crash handler for non-critical child', async () => {
      ...
      let crashCount = 0;
      const customCrashHandler = vi.fn().mockImplementation(async () => {
        crashCount++;
      ...
      const triggerCrash = async () => {
        const spawnResults = (mockManager.spawn as vi.Mock).mock.results;
      ...
      expect((mockManager.spawn as vi.Mock).mock.calls.length).toBe(initialSpawnCount + 2);
    });

    it('should use custom onChildCrash handler when provided', async () => {
      const customCrashHandler = vi.fn().mockResolvedValue(RestartDecision.IGNORE);
    ...
    it('should handle SHUTDOWN decision', async () => {
      const customCrashHandler = vi.fn().mockResolvedValue(RestartDecision.SHUTDOWN);
  ...
  describe('Child Management', () => {
    ...
    it('should differentiate critical vs non-critical children on spawn failure', async () => {
      ...
      // Make spawn fail
      (mockManager.spawn as vi.Mock).mockRejectedValue(new Error('Spawn failed'));
  ...
  describe('Crash Handling', () => {
    ...
    it('should use O(1) lookup via processIdToName map', async () => {
      ...
      const cacheSpawnCall = (mockManager.spawn as vi.Mock).mock.results[2];
    ...
    it('should track restart counts', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const lastSpawnCall = (mockManager.spawn as vi.Mock).mock.results[
          (mockManager.spawn as vi.Mock).mock.results.length - 1
        ];
        if (lastSpawnCall) {
    ...
    it('should handle errors in crash handler gracefully', async () => {
      const customCrashHandler = vi.fn().mockRejectedValue(new Error('Handler error'));
  ...
  describe('Edge Cases', () => {
    ...
    it('should clean up processIdToName map on stop', async () => {
      ...
      const _processInfo: IProcessInfo = {
        id: workerProcessId,
        name: 'worker',
        status: ProcessStatus.CRASHED,
        startTime: 1000,
