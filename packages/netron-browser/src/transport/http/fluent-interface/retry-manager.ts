/**
 * Retry Manager for HTTP requests - Browser implementation
 *
...
export interface RetryOptions {
   ...
   * timeout, HTTP 5xx / 408) — when this is `true`. Without it those are NOT
   * retried, so a non-idempotent RPC (e.g. a mutation) can't be double-executed
   * by an automatic retry. Failures that prove the request never reached the
   * server (connection refused / DNS) and an explicit 429 are retried
   * regardless. A custom `shouldRetry` bypasses this gate entirely (the caller
   * takes responsibility). Default: `false`.
   */
  idempotent?: boolean;
}
...
/**
 * Retry Manager implementation for browser
 */
