...
import {
  ...
  type ChaosSchedule,
} from '../../src/enterprise/chaos-engineering.js';
...
describe('Chaos Monkey', () => {
  ...
  describe('Basic Operations', () => {
    ...
    it('should clear active experiments on stop', () => {
      ...
      (monkey as any).activeExperiments.set(
        'exp-1',
        setTimeout(() => {}, 10000)
      );
      (monkey as any).activeExperiments.set(
  ...
  describe('Fault Injection', () => {
    it('should inject latency', () => new Promise<void>((done) => {
    ...
    it('should inject errors', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
    ...
    it('should simulate process kills', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
    ...
    it('should spike CPU', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
    ...
    it('should leak memory', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
  ...
  describe('Network Chaos', () => {
    it('should inject network partition', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
    ...
    it('should inject IO delay', () => new Promise<void>((done) => {
      monkey = new ChaosMonkey({ enabled: true });
  ...
  describe('Safety Limits', () => {
    ...
    it('should skip chaos outside allowed hours', async () => {
      ...
      Date.prototype.getHours = vi.fn().mockReturnValue(22); // 10 PM
    ...
    it('should skip excluded targets', async () => {
      ...
      (monkey as any).randomTarget = vi.fn().mockReturnValue({
...
describe('Chaos Orchestrator', () => {
  ...
  describe('Experiment Registration', () => {
    ...
    it('should emit registration event', () => new Promise<void>((done) => {
      const experiment: ChaosExperiment = {
        id: 'exp-2',
        name: 'Test',
        description: 'Test',
  ...
  describe('Method Execution', () => {
    it('should execute methods with probability', async () => {
      const experiment: ChaosExperiment = {
        id: 'exp-probability',
        ...
        method: [
          {
            ...
            parameters: {},
            // Use 0.001 (0.1% chance) - small enough to almost always skip
            // but truthy so the probability check is evaluated
            probability: 0.001,
            duration: 10,
          },
        ],
      ...
      // With 0.1% probability, the method is almost certainly skipped.
      // But there's a 0.1% chance it executes. Check that either:
      // 1. Method was skipped due to probability, OR
      // 2. Method was executed (in the rare case it wasn't skipped)
      const skipped = result.observations.some((o) => o.type === 'method-skipped' && o.reason === 'probability');
      const executed = result.observations.some((o) => o.type === 'method-start');
      expect(skipped || executed).toBe(true);
    });

    it('should execute all chaos types', async () => {
      const chaosTypes = [
        ChaosType.LATENCY,
        ChaosType.ERROR,
        ChaosType.KILL,
  ...
  describe('Rollback', () => {
    it('should execute rollback on exception', async () => {
      ...
      const experiment: ChaosExperiment = {
        ...
        steadyState: {
          metrics: [],
          validate: async () => true, // Validation succeeds
        },
        method: [
          {
            type: ChaosType.CUSTOM,
            target: { type: 'service', selector: 'test' },
            parameters: {
              // Custom handler that throws an exception
              handler: async () => {
                throw new Error('Simulated chaos failure to trigger rollback');
              },
            },
          },
        ],
      ...
      // The experiment will fail due to the exception in custom handler
      const result = await orchestrator.runExperiment('exp-rollback');

      // Rollback should be executed when an exception occurs
      // Either the rollback observation is recorded OR the rollback callback was called
      const rollbackObs = result.observations.find((o) => o.type === 'rollback');
      expect(rollbackObs || rolledBack).toBeTruthy();
    });
  });
...
describe('Chaos Testing Framework', () => {
  ...
  describe('Default Experiments', () => {
    ...
    it('should run default experiments', async () => {
  ...
  describe('Scheduled Experiments', () => {
    it('should schedule experiment', () => new Promise<void>((done) => {
      let runCount = 0;

      const originalRun = framework.runExperiment.bind(framework);
      framework.runExperiment = vi.fn(async (id: string) => {
        runCount++;
        if (runCount >= 2) {
  ...
  describe('Reporting', () => {
    ...
    it('should generate recommendations for low success rate', async () => {
      ...
      const hasLowSuccessRateRec = report.recommendations.some((r) => r.includes('low success rate'));
