...
async function processMonitor(processName?: string) {
  ...
  if (platform === 'darwin') {
    command = processName ? `ps aux | grep "${processName}" | grep -v grep` : 'ps aux | head -20';
  } else {
    command = processName ? `ps aux | grep "${processName}" | grep -v grep` : 'ps aux --sort=-%cpu | head -20';
  }
...
async function dockerMonitor() {
  ...
  const stats =
    await $`docker stats --no-stream --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}"`;
  console.log(stats.stdout);

  // Проверяем здоровье контейнеров
  const healthChecks =
    await $`docker ps --format "{{.Names}}" | xargs -I {} docker inspect {} --format '{{.Name}}: {{.State.Health.Status}}'`.nothrow();
