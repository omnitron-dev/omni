# Titan / Netron / downstream — Deep Audit & Remediation Tracker
...
- [x] **Phase 3 AUDIT done** — downstream Backends Architecture Uniformity Blueprint produced (bg agent). Backends are already ~85% uniform (shared `@acme/auth-utils` + `defineSystem` bootstrap; ZERO `@ts-nocheck`/legacy shims; ZERO version-suffixed service names). Canonical app = `apps/main`. Blueprint + roadmap below. **downstream-app changes commit to the separate `internal/downstream` repo.**

### PHASE 3 — UNIFICATION ROADMAP (from blueprint)
**✅ SESSION PROGRESS (downstream commits 36741e9→c769fbf):** U-P0 DONE · U-P1 DONE (titan-kit + 6-backend migration + config-path-bug fix + wireNetronAuth + ACME_REDIS_DB pinning) · cookie-mode `__Host-` hardening (omni b462cc9 + downstream d1a1368). Next: **U-P2** (health/metrics unify). All changes per-app `tsc`-verified; framework source committed (rebuild propagates to backends).
- [x] **U-P0 cleanup — DONE:** deleted `apps/payments/Oops.rej` (36741e9); storage `./bootstrap` export added (36741e9); geo `src/config/config.schema.ts`+`index.ts` added (c769fbf). (Stale `apps/storage/AUTH_MODULE_README.md` deletion still pending — low priority doc.)
- [x] **U-P1 — DONE (all sub-parts):** `@acme/titan-kit` + all 6 backends migrated (8943842) + config-path bug fixed; `wireNetronAuth` collapses the 6 duplicated bootstrap auth blocks (c449e1c); `ACME_REDIS_DB` pins the Redis split in code (ae38748).
- [x] **U-P1 ✅ DONE (downstream `8943842`) — all 6 backends on `@acme/titan-kit`; config-path bug FIXED:** every `shared/module-factories.ts` (+ payments deposit-worker's 7th parser copy) now thin wrappers over titan-kit (parsers + parameterized Config/Redis/Database factories). Per-app specifics preserved via params (main gateway+audit, storage sessions, pricing transform:false+migrations, geo timestamps:false+no-schema, payments rotif+pricing+rls, messaging forRoot-wrappers). **NET FIX:** titan-kit resolves config from absolute `appRoot` (+local.json+priorities) — fixes the latent relative-path config-load-under-supervision bug across 5 backends. All 6 apps `tsc --noEmit`: 0 errors. ~1000 lines deduped. **Remaining U-P1 sub-parts (next focused passes — both behavior-sensitive):**
  - **`wireNetronAuth()` (intricate — investigated):** auth wiring is duplicated near-identically across storage/pricing/payments/messaging; main adds `sessionLookup` (Postgres), geo is minimal (anonymous-first, no CSRF). BUT it spans TWO bootstrap hooks — `afterCreate` (createJwtAuthManager + `app.netron.configureAuth` + `setAuthBootState`) and `afterStart` (origin + CSRF middleware via `getAuthBootState`) — plus module-level `deferredPermVerRedis` and storage's token-transport handoff to custom routes. → extract `wireNetronAuthCreate(app, {jwtService, sessionRedis, logger, sessionLookup?, cnfHmacSecret?}) → bootState` + `wireNetronAuthStart(app)` into `@acme/auth-utils` (reuse its existing `set/getAuthBootState` + `boot-state.ts`). Security-critical → migrate per-app, behavior-preserving, tsc-verify each. Do as a dedicated pass (NOT a tail-of-session rush).
  - **`ACME_REDIS_DB` code-pinning:** define a shared app→db map (main=0…payments=4, geo=5) and pin each app's primary Redis `db` in code rather than relying on the `REDIS_URL` path convention (collision-safety). Behavior-sensitive (overrides URL-derived db) → confirm deployment expectation first.
- [ ] **U-P1 (orig design note, kept for reference):** create `@acme/titan-kit` (mirror auth-utils scaffold: type:module, peerDeps on titan-*, src/index.ts; downstream workspace globs `packages/*`). API from the survey:
  - `parseRedisUrl`/`parseDatabaseUrl` — pure, BYTE-IDENTICAL across all 6 backends (+ payments deposit-worker = 7 copies).
  - `createConfigModuleOptions({ app, envPrefix, schema, transform=true })` — varies only by prefix (MAIN_/STORAGE_/PAYMENTS_/PRICING_/MSG_/GEO_) + path `apps/{app}/config/default.json` + schema.
  - `createRedisModuleOptions({ sessionsClient?, extraClients? })` — shared `sessions` client (SESSION_REDIS_URL||REDIS_URL db0) used by 5 apps; main adds gateway, payments adds pricing client.
  ...
  - Migrate 6 apps → thin wrappers; needs `pnpm install` (downstream) + per-app `tsc` verify. Then `wireNetronAuth()` in auth-utils + pin Redis DB via shared `ACME_REDIS_DB` map.
  - **Execution note:** cross-repo + build-system + 6-app + per-app-verify → do as a focused build (not rushed), each app kept green.
  - ✅ **U-P1 part 1 DONE (downstream `ead7092`):** `@acme/titan-kit` created (parsers + parameterized config/redis/db factories) + **main migrated** to thin wrappers (gateway client + data-audit preserved, app.module.ts untouched). `pnpm install` linked it; **main tsc 0 errors**. Proven end-to-end on the reference app.
  - ⏳ **Remaining U-P1 — DIVERGENCE MAP captured (rollout is a deliberate convergence, not a blind swap):**
    - **CONFIG — convergence is a NET FIX:** geo uses absolute `APP_ROOT`-resolved paths + `local.json` + explicit priorities, deliberately fixing a cwd-under-omnitron-supervision bug; main/storage/payments/pricing/messaging still use RELATIVE `apps/<app>/config/default.json` (the latent bug geo fixed). → titan-kit `createConfigModuleOptions` should take `appRoot` + adopt geo's robust pattern → FIXES the bug across all 6 (re-migrate main too). Also: pricing omits `transform:camelCase`; geo passes NO schema.
    - **REDIS:** the `sessions` client (SESSION_REDIS_URL||REDIS_URL db0, `lazyConnect:false`, `maxRetriesPerRequest:1`) is canonical (5 apps); **storage is the OUTLIER** lacking logger-inject/readyLog/errorLog/healthCheck → converge it up. Extra clients: main=`gateway`, payments=`rotif`+`pricing`.
    - **DB:** vary by `timestamps` (geo:false), `migrations.directory` (pricing/geo), `dataAudit` (main), `poolOverrides`; `healthCheck:true` canonical (storage lacks it).
...
- [ ] **U-P2 module-consumption uniformity (sensible default = unify on Titan) — INVESTIGATED, next focused pass:**
  ...
  - ✅ **Optional deps — ASSESSED, both KEEP (confirmed used):** messaging wires `titan-discovery` via `.forRoot()` in app.module; payments uses `titan-pm`'s `CircuitBreaker`/`RateLimit`/`Supervisor`/`Child`/`SupervisionStrategy` primitives across 5 files (wallet-supervisor + BTC/XMR hot-wallets). Blueprint's "declared-unused titan-pm" was wrong — only the full `PMModule.forRoot()` supervision tree isn't enabled; the primitives ARE used. No change.
  - ✅ **Health — ASSESSED, NO HEAVY CHANGE (correct call):** read all 5 services. They already share the titan-health SUBSTRATE; the RPC adapters legitimately VARY — `@Service` name (`HealthService`×4 / `GeoHealth`), rate limit (1000 / messaging 120), delegation (pricing/geo/messaging are thin pass-throughs to a domain `HealthService`; storage/payments inline a converter), and response shape (`HealthResponse` / `GeoHealthResponse` / inline). The one true duplication is storage+payments's `convertToHealthResponse`, BUT their `HealthResponse` types are part of the FRONTEND WIRE CONTRACT (exported via `./dto`), so sharing them = cross-package DTO coupling for ~40 lines. A dynamic-`@Service` factory would over-abstract legitimately-varied, contract-bound code (+ framework risk). Substrate already unified; thin adapters correctly differ → leave as-is.
...
- [ ] **U-P3 naming/docs:** one `@Service` naming convention (recommend bare domain nouns, drop `Service` suffix + `OmnitronStorage*`/`Payments*` prefixes); portal-seed read DB creds from env.
...
Bring all 8 downstream backends to ONE consistent Titan-based architecture — demonstrating Titan's
...
- **Repo note**: downstream-app changes commit to the SEPARATE `internal/downstream` git repo; framework
...
- **MEM-FIX ✅** — corrected `infra_invariants` memory (Nexus default is **Singleton**, not Transient — was inverted; verified registration.ts:69).
...
- [ ] **MEM-FIX** Correct `acme-dev-principles` memory: Nexus defaults providers to **Singleton**, not Transient (NX-1). (verify first)
...
- [~] **RL-3** PARTIALLY FIXED (FixedWindow/counter done) — added an atomic `IRateLimitStorage.checkAndConsume(key, limit, ttl)` primitive: Redis does the compare-and-increment in ONE Lua `EVAL` (PEXPIRE only on first creation, so a fixed window isn't extended), memory does it synchronously (no `await` between read and write). `FixedWindowAlgorithm.check` now uses it on the consume path, closing the get()+increment() TOCTOU (concurrent callers can no longer all pass `count<limit` then all increment). Also fixed a latent ttl unit bug there (was SECONDS passed to the ms storage API). Test ratelimit-atomicity.test.ts: 100 concurrent → exactly `limit` admitted, counter never overshoots; peek consumes nothing. **REMAINING (RL-3b):** SlidingWindow (sorted-set ZREMRANGEBYSCORE+ZCARD+ZADD+PEXPIRE) still races across its 4 awaits → needs its own Lua; TokenBucket (RL-2 refill) get/set race → needs refill-in-Lua.
...
Boundary map: titan-auth = JWTService(used) + shared-session preset(the bridge, downstream-facing) + DEAD HTTP stack(AU-3). netron/auth = the RPC engine (Authn/Authz/Policy). Two incompatible AuthContext models (`role` vs `roles[]`).
