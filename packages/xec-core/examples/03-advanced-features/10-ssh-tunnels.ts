...
async function main() {
  ...
  try {
    ...
    console.log(
      `✅ Dynamic tunnel created on localhost:${dynamicTunnel.localPort} -> ${dynamicTunnel.remoteHost}:${dynamicTunnel.remotePort}`
    );
  ...
  try {
    ...
    // Close all tunnels
    await Promise.all([tunnel1.close(), tunnel2.close(), tunnel3.close()]);
