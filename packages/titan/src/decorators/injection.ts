...
export function Optional() {
  return function optionalDecorator(target: any, propertyKey: string | symbol | undefined, parameterIndex?: number) {
    if (propertyKey === undefined && parameterIndex !== undefined) {
...
export function Lazy<T>(tokenFactory: () => InjectionToken<T>) {
  return function lazyDecorator(target: any, propertyKey: string) {
...
export function InjectEnv(key: string, defaultValue?: any) {
  return function injectEnvDecorator(target: any, propertyKey: string | symbol | undefined, parameterIndex?: number) {
    if (propertyKey === undefined && parameterIndex !== undefined) {
...
export function InjectConfig(path: string) {
  return function injectConfigDecorator(
    target: any,
    propertyKey: string | symbol | undefined,
    parameterIndex?: number
  ) {
...
export function ConditionalInject<T>(token: InjectionToken<T>, condition: () => boolean, fallback?: T | (() => T)) {
  return function conditionalInjectDecorator(
    target: any,
    propertyKey: string | symbol | undefined,
    parameterIndex?: number
  ) {
