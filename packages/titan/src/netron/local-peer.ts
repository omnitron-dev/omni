...
export class LocalPeer extends AbstractPeer {
  ...
  async exposeService(instance: any): Promise<Definition> {
    this.logger.debug('Exposing service');
    ...
    if (extendedMeta.transports && extendedMeta.transports.length > 0) {
      this.logger.info(
        {
          serviceName: meta.name,
          transportCount: extendedMeta.transports.length,
          transports: extendedMeta.transports,
        },
    ...
    const isTransportWithServices = (s: unknown): s is ITransportServerWithServices =>
      s !== null &&
      typeof s === 'object' &&
      'registerService' in s &&
      typeof (s as ITransportServerWithServices).registerService === 'function';
    ...
    await this.netron.emitSpecial(NETRON_EVENT_SERVICE_EXPOSE, getServiceEventName(serviceKey), {
      name: def.meta.name,
   ...
   */
  async exposeRemoteService(peer: RemotePeer, meta: ServiceMetadata) {
    // Collision protection: reject if a different peer already exposed this service name
    const existingStub = this.netron.services.get(meta.name) as ServiceStub | undefined;
    if (existingStub && existingStub.definition?.peerId !== peer.id) {
      throw Errors.conflict(`Service '${meta.name}' already exposed by a different peer`);
    }
    ...
    await this.netron.emitSpecial(NETRON_EVENT_SERVICE_EXPOSE, getServiceEventName(def.meta.name), {
      name: def.meta.name,
      version: def.meta.version,
  ...
  async unexposeService(serviceName: string) {
    ...
    const hasUnregisterService = (s: unknown): s is ITransportServerWithServices =>
      s !== null &&
      typeof s === 'object' &&
      'unregisterService' in s &&
      typeof (s as ITransportServerWithServices).unregisterService === 'function';
    ...
    await this.netron.emitSpecial(NETRON_EVENT_SERVICE_UNEXPOSE, getServiceEventName(serviceName), {
      name: def.meta.name,
      version: def.meta.version,
  ...
  protected getDefinitionByServiceName(name: string): Definition {
    const stub = this.netron.services.get(name) as ServiceStub | undefined;
    if (stub === void 0) {
