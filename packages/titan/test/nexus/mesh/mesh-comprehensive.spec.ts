  ...
  describe('Load Balancer', () => {
    ...
    describe('Round Robin Strategy', () => {
      ...
      it('should distribute requests evenly', () => {
        const selections = [loadBalancer.next(), loadBalancer.next(), loadBalancer.next(), loadBalancer.next()];
    ...
    describe('Response Time Strategy', () => {
      ...
      it('should select endpoint with no data first', () => {
        ...
        const next = loadBalancer.next();
        // When only one endpoint has data, the algorithm prefers exploring
        // endpoints with no data, returning the first one encountered
        expect(next?.id).toBe('endpoint2');
      });
    ...
    it('should open after threshold failures', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        try {
          ...
          // Expected failure to open the circuit
        }
      }
