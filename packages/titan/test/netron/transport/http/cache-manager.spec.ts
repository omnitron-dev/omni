...
describe('HttpCacheManager', () => {
  ...
  describe('Cache Statistics', () => {
    ...
    it('should track cache size', async () => {
      const fetcher1 = vi.fn(async () => ({ data: 'short' }));
      const fetcher2 = vi.fn(async () => ({ data: 'a'.repeat(1000) }));
  ...
  describe('Events', () => {
    it('should emit cache events', async () => {
      const events: any[] = [];
      const handlers = {
        hit: vi.fn((data) => events.push({ type: 'hit', data })),
        miss: vi.fn((data) => events.push({ type: 'miss', data })),
        set: vi.fn((data) => events.push({ type: 'set', data })),
        invalidate: vi.fn((data) => events.push({ type: 'invalidate', data })),
      };
  ...
  describe('Debug Mode', () => {
    ...
    it('should log during background revalidation', async () => {
      ...
      const fetcher = vi.fn(async () => ({ data: 'test', timestamp: Date.now() }));
  ...
  describe('Size-based Eviction', () => {
    it('should evict entries when max size is exceeded', async () => {
      ...
      const fetcher1 = vi.fn(async () => ({ data: 'x'.repeat(400) })); // ~400 bytes
      const fetcher2 = vi.fn(async () => ({ data: 'y'.repeat(400) })); // ~400 bytes
      const fetcher3 = vi.fn(async () => ({ data: 'z'.repeat(400) })); // ~400 bytes

      // Add entries
      await cache.get('size-key1', fetcher1, { maxAge: 10000 });
      const statsBefore = cache.getStats();
      const _sizeBefore = statsBefore.sizeBytes;
    ...
    it('should calculate size correctly', async () => {
      ...
      const fetcher = vi.fn(async () => smallData);
  ...
  describe('Edge Cases', () => {
    it('should handle concurrent gets for same key from cache', async () => {
      const fetcher = vi.fn(async () => ({ data: 'concurrent' }));
