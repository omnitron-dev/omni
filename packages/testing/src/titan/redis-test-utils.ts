...
export async function createRedisTestFixture(options: RedisTestFixtureOptions = {}): Promise<RedisTestFixture> {
  ...
  if (withManager) {
    // Use type assertion to avoid ioredis version compatibility issues
    const clientConfigs = [
      {
        namespace: 'default',
        ...redisOptions,
    ...
    manager = new RedisManager({ clients: clientConfigs }, createTestNullLogger());
...
export function createMockRedis(): any {
  ...
  const mock = {
    ...
    exists: vi.fn().mockImplementation(async (...keys: string[]) => keys.filter((k) => data.has(k)).length),
    ...
    ttl: vi.fn().mockImplementation(async (key: string) => (data.has(key) ? -1 : -2)),
    ...
    // Pub/Sub
    subscribe: vi.fn().mockImplementation(async (...channels: string[]) => {
    ...
    unsubscribe: vi.fn().mockImplementation(async (...channels: string[]) => {
      for (const channel of channels) {
    ...
    eval: vi.fn().mockImplementation(async (script: string, ...args: any[]) => {
