  ...
  describe('Integration: Real-world testing pattern', () => {
    it('should support complex test scenario', async () => {
      ...
      await testInTransaction(db, async (trx) => {
        ...
        const user1 = await trx.insertInto('users').values(user1Data).returningAll().executeTakeFirst();

        const user2 = await trx.insertInto('users').values(user2Data).returningAll().executeTakeFirst();
        ...
        expect(postsWithUsers.map((p) => (p as any).author_name).sort()).toEqual(['Alice', 'Bob']);
      });
