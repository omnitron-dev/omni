...
describe('error-handler', () => {
  ...
  describe('handleError', () => {
    ...
    it('should show debug info when XEC_DEBUG is set', () => {
      ...
      const hasDebugInfo = errorCalls.some(
        (call) => call[0] && typeof call[0] === 'string' && call[0].includes('=== DEBUG ERROR INFO ===')
      );
    ...
    it('should display verbose error info', () => {
      ...
      expect(
        mockConsoleError.mock.calls.some((call) => call[0]?.includes && call[0].includes('Run with --verbose'))
      ).toBe(false);
    });
  });
