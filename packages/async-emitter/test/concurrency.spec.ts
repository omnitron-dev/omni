...
describe("AsyncEventEmitter - Concurrency with pLimit", () => {
  const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));
  
  describe("Constructor concurrency", () => {
    it("should accept concurrency in constructor", () => {
      const emitter = new AsyncEventEmitter(5);
    ...
    it("should not set limiter when concurrency is invalid", () => {
      const emitter1 = new AsyncEventEmitter(0);
      const emitter2 = new AsyncEventEmitter(-1);
      const emitter3 = new AsyncEventEmitter(0.5);
  ...
  describe("Concurrent execution control", () => {
    it("should limit concurrent execution to specified number", async () => {
      const concurrency = 2;
      const emitter = new AsyncEventEmitter(concurrency);
  ...
  describe("Performance characteristics", () => {
    it("should complete faster with higher concurrency", async () => {
      ...
      const emitter1 = new AsyncEventEmitter(1);
      ...
      const emitter2 = new AsyncEventEmitter(3);
      for (let i = 0; i < taskCount; i++) {
    ...
    it("should handle no concurrency limit efficiently", async () => {
      const emitter = new AsyncEventEmitter(); // No concurrency limit
      const taskCount = 10;
      const results: number[] = [];
  ...
  describe("Edge cases", () => {
    ...
    it("should handle very high concurrency values", () => {
      const emitter = new AsyncEventEmitter(1000);
      expect((emitter as any).limiter).toBeDefined();
    });

    it("should handle Infinity concurrency", () => {
      const emitter = new AsyncEventEmitter(Infinity);
      expect((emitter as any).limiter).toBeDefined();
    });
  });
