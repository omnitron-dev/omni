  ...
  describe('ParallelEngine', () => {
    describe('all', () => {
      it('should execute all commands and return results', async () => {
        ...
        const results = await $mock.parallel.all(['echo "1"', 'echo "2"', 'echo "3"']);
      ...
      it('should throw if any command fails', async () => {
        ...
        await expect($mock.parallel.all(['echo "1"', 'fail', 'echo "3"'])).rejects.toThrow();
      });
    });
    ...
    describe('race', () => {
      it('should return the first successful result', async () => {
        ...
        const result = await $mock.parallel.race(['slow', 'fast', 'medium']);
    ...
    describe('map', () => {
      it('should map over items and execute commands', async () => {
        ...
        const result = await $mock.parallel.map(items, (item) => `process ${item}`);
      ...
      it('should respect maxConcurrency', async () => {
        ...
        const result = await $mock.parallel.map(items, (i) => `echo ${i}`, { maxConcurrency: 2 });
    ...
    describe('filter', () => {
      it('should filter items based on command success', async () => {
        ...
        const validFiles = await $mock.parallel.filter(files, (file) => `test -f ${file}`);
    ...
    describe('some', () => {
      it('should return true if at least one command succeeds', async () => {
        ...
        const result = await $mock.parallel.some(['test1', 'test2', 'test3']);
    ...
    describe('every', () => {
      it('should return true if all commands succeed', async () => {
        ...
        const result = await $mock.parallel.every(['test1', 'test2', 'test3']);
    ...
    describe('ProcessPromise support', () => {
      it('should handle ProcessPromise in all()', async () => {
        ...
        const results = await $mock.parallel.all([promise1, promise2, 'echo "direct"']);
      ...
      it('should handle ProcessPromise with nothrow()', async () => {
        ...
        const result = await $mock.parallel.settled([okPromise, failPromise]);
      ...
      it('should handle ProcessPromise in some()', async () => {
        ...
        const promises = [$mock`fail1`.nothrow(), $mock`success`.nothrow(), $mock`fail2`.nothrow()];
      ...
      it('should handle ProcessPromise in every()', async () => {
        ...
        const promises = [$mock`ok1`, $mock`ok2`, $mock`ok3`];
      ...
      it('should handle mixed types in parallel execution', async () => {
        ...
        const result = await $mock.parallel.settled(['from-string', command, promise]);
