...
class TestConnection extends BaseConnection {
  ...
  public testHandleData(data: Buffer | ArrayBuffer): void {
    this.handleData(data);
  }

  public testResetBinaryMode(): void {
    this.resetBinaryMode();
  }

  public getBinaryMode(): boolean {
    return (this as any).binaryMode;
  }
}
...
describe('BaseTransport Classes', () => {
  ...
  describe('BaseServer', () => {
    ...
    it('should close all connections when server closes', async () => {
      ...
      const disconnect1Spy = vi.fn();
      const disconnect2Spy = vi.fn();
  ...
  describe('Binary Mode Optimization', () => {
    let connection: TestConnection;
    ...
    it('should start with binary mode disabled', () => {
      expect(connection.getBinaryMode()).toBe(false);
    });

    it('should enable binary mode after first binary packet', async () => {
      await connection.connect();

      // Import packet encoder to create a valid binary packet
      const { Packet, encodePacket, TYPE_CALL } = await import('../../../src/netron/packet/index.js');

      const packet = new Packet(1);
      packet.setType(TYPE_CALL); // Set non-PING type
      packet.data = { test: 'data' };
      const encodedPacket = encodePacket(packet);

      expect(connection.getBinaryMode()).toBe(false);

      // Simulate receiving binary packet
      connection.testHandleData(encodedPacket);

      // Binary mode should now be enabled
      expect(connection.getBinaryMode()).toBe(true);
    });

    it('should skip text detection heuristic in binary mode', async () => {
      await connection.connect();

      // Import packet encoder
      const { Packet, encodePacket, TYPE_CALL } = await import('../../../src/netron/packet/index.js');

      // Register spy before sending any packets to catch all emissions
      const packetSpy = vi.fn();
      const dataSpy = vi.fn();
      connection.on('packet', packetSpy);
      connection.on('data', dataSpy);

      // First packet enables binary mode
      const packet1 = new Packet(1);
      packet1.setType(TYPE_CALL); // Set non-PING type
      packet1.data = { first: 'packet' };
      connection.testHandleData(encodePacket(packet1));

      expect(connection.getBinaryMode()).toBe(true);
      expect(packetSpy).toHaveBeenCalledTimes(1);

      // Second packet should use fast path (no text detection)
      const packet2 = new Packet(2);
      packet2.setType(TYPE_CALL); // Set non-PING type
      packet2.data = { second: 'packet' };

      connection.testHandleData(encodePacket(packet2));

      // Verify both packets were processed
      expect(packetSpy).toHaveBeenCalledTimes(2);
      expect(packetSpy).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({
          id: 2,
        })
      );
      // Ensure no data events were emitted (only packet events)
      expect(dataSpy).not.toHaveBeenCalled();
    });

    it('should handle text during handshake before binary mode', async () => {
      await connection.connect();

      const dataSpy = vi.fn();
      connection.on('data', dataSpy);

      // Send text data (handshake message)
      const textData = Buffer.from('{"handshake": "hello"}');
      connection.testHandleData(textData);

      expect(connection.getBinaryMode()).toBe(false);
      expect(dataSpy).toHaveBeenCalledWith(textData);
    });

    it('should reset binary mode on disconnect', async () => {
      await connection.connect();
      ...
      packet.setType(TYPE_CALL); // Set non-PING type
      packet.data = { test: 'data' };
      connection.testHandleData(encodePacket(packet));
      ...
      // Binary mode should be reset
      expect(connection.getBinaryMode()).toBe(false);
    });

    it('should allow manual reset of binary mode', async () => {
      await connection.connect();
      ...
      // Reset manually
      connection.testResetBinaryMode();

      expect(connection.getBinaryMode()).toBe(false);
    });

    it('should handle binary packets correctly in binary mode', async () => {
      await connection.connect();

      // Import packet encoder
      const { Packet, encodePacket, TYPE_CALL } = await import('../../../src/netron/packet/index.js');

      // Register spy before sending packets
      const packetSpy = vi.fn();
      connection.on('packet', packetSpy);

      // Enable binary mode with first packet
      const packet1 = new Packet(1);
      packet1.setType(TYPE_CALL); // Set non-PING type
      packet1.data = { test: 'data' };
      connection.testHandleData(encodePacket(packet1));

      expect(connection.getBinaryMode()).toBe(true);
      expect(packetSpy).toHaveBeenCalledTimes(1);

      // Send multiple packets in binary mode
      for (let i = 2; i <= 10; i++) {
        const packet = new Packet(i);
        packet.setType(TYPE_CALL); // Set non-PING type
        packet.data = { index: i };
        connection.testHandleData(encodePacket(packet));
      }

      // All 10 packets should be processed via fast path
      expect(packetSpy).toHaveBeenCalledTimes(10);
    });

    it('should handle PING packets in binary mode', async () => {
      await connection.connect();

      // Import packet encoder
      const { Packet, encodePacket, TYPE_PING } = await import('../../../src/netron/packet/index.js');
      ...
      // Send PING request
      const pingPacket = new Packet(100);
      pingPacket.setType(TYPE_PING);
      pingPacket.setImpulse(1); // Request
      pingPacket.data = Date.now();

      connection.testHandleData(encodePacket(pingPacket));

      // PING should be handled internally (no packet event)
      const packetSpy = vi.fn();
      connection.on('packet', packetSpy);
    ...
    it('should fall back to data emission on decode error in binary mode', async () => {
      await connection.connect();
      ...
      connection.testHandleData(invalidData);

      // Should emit as raw data even in binary mode
      expect(dataSpy).toHaveBeenCalledWith(invalidData);
    });

    it('should not enable binary mode for text-like data', async () => {
      await connection.connect();

      const dataSpy = vi.fn();
      connection.on('data', dataSpy);

      // Send various text formats
      const textFormats = [
        Buffer.from('{"json": "data"}'), // JSON object
        Buffer.from('"string"'), // JSON string
        Buffer.from('plain text'), // Plain text
      ];

      for (const textData of textFormats) {
        connection.testHandleData(textData);
      }

      // Binary mode should still be disabled
      expect(connection.getBinaryMode()).toBe(false);
