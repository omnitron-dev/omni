 ...
 */
export type SubscriptionCallback<T = any> = (value: T, oldValue: T | undefined, path: string[]) => void;
...
export class StoreSubscriptionManager<T extends object> {
   ...
   */
  subscribe(path: string | string[], callback: SubscriptionCallback, options: SubscriptionOptions = {}): () => void {
    const pathArray = Array.isArray(path) ? path : path.split('.');
    // const pathStr = pathArray.join('.'); // Unused for now, may be useful for debugging
  ...
  subscribeMany(
    ...
    const unsubscribes = subscriptions.map((sub) => this.subscribe(sub.path, sub.callback, sub.options));
   ...
   */
  notify(path: string[], newValue: any, _oldValue: any): void {
   ...
   */
  private shouldNotify(subscription: SubscriptionRecord, _changePath: string[], changePathStr: string): boolean {
   ...
   */
  private triggerCallback(subscription: SubscriptionRecord, value: any, oldValue: any, path: string[]): void {
    const trigger = () => {
      try {
...
export function storeComputed<T extends object, R>(
  ...
  if (manager) {
    for (const path of paths) {
      const unsub = manager.subscribe(path, () => recompute(), options);
