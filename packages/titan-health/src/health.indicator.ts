...
export abstract class HealthIndicator implements IHealthIndicator {
   ...
   */
  protected unhealthy(message?: string, details?: Record<string, unknown>, error?: Error): HealthIndicatorResult {
  ...
  protected createResultFromCondition(
    ...
    return isHealthy ? this.healthy(healthyMessage, details) : this.unhealthy(unhealthyMessage, details);
  }
   ...
   */
  protected async withLatency<T>(operation: () => Promise<T>): Promise<{ result: T; latency: number }> {
    const start = Date.now();
  ...
  protected getWorstStatus(statuses: HealthStatus[]): HealthStatus {
    ...
    return statuses.reduce(
      (worst, current) => (this.getStatusSeverity(current) > this.getStatusSeverity(worst) ? current : worst),
