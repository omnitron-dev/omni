...
import { isCluster, createRedisClient } from '../src/redis.utils.js';
import {
...
describeOrSkip('Redis Cluster Support', () => {
  ...
  describe('Cluster Service Operations', () => {
    ...
    it('should handle cluster operations through service', async () => {
      ...
      const cluster = client as Cluster;
      cluster.set = vi.fn().mockResolvedValue('OK');
      cluster.get = vi.fn().mockResolvedValue('test-value');
    ...
    it('should handle cluster pipeline operations', async () => {
      ...
      cluster.pipeline = vi.fn().mockReturnValue(mockPipeline);
  ...
  describe('Cluster Health Checks', () => {
    it('should check health of cluster clients', async () => {
      ...
      const cluster = client as Cluster;
      cluster.ping = vi.fn().mockResolvedValue('PONG');
  ...
  describe('Cluster Script Execution', () => {
    it('should handle script execution in cluster mode', async () => {
      ...
      const cluster = client as Cluster;
      cluster.evalsha = vi.fn().mockResolvedValue('script-result');
      cluster.script = vi.fn().mockImplementation((cmd: string) => {
        if (cmd === 'load') {
  ...
  describe('Cluster Pub/Sub', () => {
    it('should handle pub/sub in cluster mode', async () => {
      ...
      cluster.duplicate = vi.fn().mockReturnValue(mockSubscriber);
      cluster.publish = vi.fn().mockResolvedValue(1);
