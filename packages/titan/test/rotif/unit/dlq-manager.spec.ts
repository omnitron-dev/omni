...
import type { ILogger } from '../../../src/rotif/types.js';
...
describe('Rotif - DLQManager', () => {
  ...
  describe('auto-cleanup', () => {
    ...
    it('should log auto-cleanup start and stop', () => {
      ...
      manager.startAutoCleanup();
      // The logger is called with (object, message) format: ({ interval }, 'DLQ auto-cleanup started')
      expect(logger.info).toHaveBeenCalled();
      const infoCall = (logger.info as vi.Mock).mock.calls.find((call: any[]) =>
        call.some((arg: any) => typeof arg === 'string' && arg.includes('DLQ auto-cleanup started'))
      );
