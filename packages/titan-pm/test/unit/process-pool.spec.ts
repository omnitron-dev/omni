...
const createMockProxy = (overrides: Record<string, unknown> = {}) => ({
  ...
  failingMethod: vi.fn().mockRejectedValue(new Error('Method failed')),
  ...overrides,
});
...
const createMockManager = () => {
  ...
  manager.spawn = vi.fn().mockImplementation(async () => createMockProxy());
...
describe('ProcessPool', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should handle spawn failures gracefully', async () => {
      mockManager.spawn.mockResolvedValueOnce(createMockProxy()).mockRejectedValueOnce(new Error('Spawn failed'));
  ...
  describe('Health Monitoring', () => {
    ...
    it('should mark worker as unhealthy after repeated failures', async () => {
      const failingProxy = createMockProxy({
        __getHealth: vi.fn().mockRejectedValue(new Error('Health check failed')),
        testMethod: vi.fn().mockRejectedValue(new Error('Always fails')),
      });
      mockManager.spawn.mockResolvedValue(failingProxy);
