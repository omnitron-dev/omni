...
/**
 * Maximum number of wildcards allowed in permission patterns
 * Prevents ReDoS attacks through exponential backtracking
 */
const MAX_WILDCARDS = 5;

/**
 * Cache for compiled permission patterns to avoid repeated regex compilation
 */
const permissionPatternCache = new Map<string, RegExp>();
 ...
 *
 * Includes protection against ReDoS attacks through:
 * - Limiting maximum number of wildcards
...
function matchPermissionPattern(pattern: string, permission: string): boolean {
  ...
  // Validate inputs
  if (typeof pattern !== 'string' || typeof permission !== 'string') {
    return false;
  }
  ...
  if (wildcardCount > MAX_WILDCARDS) {
    // Too many wildcards - potential ReDoS, reject
    return false;
  }

  // Check cache for compiled regex
  let regex = permissionPatternCache.get(pattern);

  if (!regex) {
    // Convert pattern to regex (escape special chars except *)
    const regexPattern = pattern
      .replace(/[.+?^${}()|[\]\\]/g, '\\$&') // Escape special chars
      .replace(/\*/g, '[^:]*'); // Replace * with [^:]* (non-greedy, matches within segment only)

    regex = new RegExp(`^${regexPattern}$`);

    // LRU-style cache eviction
    if (permissionPatternCache.size >= MAX_PATTERN_CACHE_SIZE) {
      const firstKey = permissionPatternCache.keys().next().value;
      if (firstKey !== undefined) {
        permissionPatternCache.delete(firstKey);
      }
    }
    permissionPatternCache.set(pattern, regex);
  }
...
export const BuiltInPolicies = {
  ...
  requirePermission: (permission: string): PolicyDefinition => ({
    ...
    evaluate: (context) => {
      const hasPerm = matchPermission(context.auth?.permissions ?? [], permission);
      return {
        allowed: hasPerm,
        reason: hasPerm ? `Has required permission: ${permission}` : `Missing permission: ${permission}`,
      };
    },
  }),
  ...
  requireAnyPermission: (permissions: string[]): PolicyDefinition => ({
    ...
    tags: ['rbac', 'permission'],
    evaluate: (context) => {
      const hasAnyPermission = permissions.some((perm) => matchPermission(context.auth?.permissions ?? [], perm));
      return {
  ...
  requireResourceOwner: (): PolicyDefinition => ({
    ...
    evaluate: (context) => {
      ...
      const isOwner = safeCompare(userId, ownerId);
      return {
        allowed: isOwner,
  ...
  requireTenantIsolation: (): PolicyDefinition => ({
    ...
    evaluate: (context) => {
      ...
      // Use constant-time comparison to prevent timing attacks
      const allowed = safeCompare(userTenant, resourceTenant);
      return {
        allowed,
